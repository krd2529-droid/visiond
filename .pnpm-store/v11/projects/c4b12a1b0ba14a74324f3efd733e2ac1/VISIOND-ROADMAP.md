...
- ศูนย์จัดการคอร์สต้องแสดงตะกร้าคอร์สทั้งหมดที่เป็นของเจ้าของบัญชี
...
# อัปเดต v0.14.305 — EP เดียวต่อการเพิ่มหนึ่งครั้งและไม่นับสล็อตว่าง (IMPLEMENTED)

- หน้า Course Seller เหลือปุ่มเพิ่ม EP เพียงปุ่มเดียว และแจ้ง validation ภายในหน้าแทน tooltip บังคับของเบราว์เซอร์
- การเพิ่ม EP จะสร้างเฉพาะรายการที่มีชื่อและมีคลิปหรือเอกสารจริง ไม่เติมหรือคืนชีพสล็อตว่างจากโครงสร้างเก่า
- การส่งตรวจและการอนุมัติหลังบ้านนับเฉพาะ EP ที่มีชื่อและสื่อจริง สล็อตว่างเก่าไม่ทำให้จำนวน EP ผิด
- Event Case นี้เสร็จเมื่อทดสอบ v0.14.305 ผ่านและ Commit ในเครื่อง โดยไม่ Push/Deploy

# อัปเดต v0.14.306 — ปุ่มเพิ่ม EP ทำหน้าที่เพิ่มทันที (IMPLEMENTED)

- แก้ข้อกำหนด v0.14.298 และ v0.14.305 เฉพาะจุด: ปุ่ม `เพิ่ม EP` ต้องเพิ่ม EP ใหม่ทันทีโดยไม่บังคับกรอกชื่อ คลิป หรือเอกสารก่อน
- ห้ามใช้ validation ของฟอร์มหรือ API ขวางการกดเพิ่ม EP
- กฎส่งตรวจแยกจากการเพิ่ม EP: นับเฉพาะ EP ที่มีชื่อพร้อมคลิปหรือเอกสารจริง
- ไม่เปลี่ยนกฎแก้ไขหรือลบ EP ใน Event Case นี้
# v0.14.307 — ตะกร้าคอร์สและ EP เป็นงานเดียวกัน

- ปุ่ม `+ เพิ่ม EP` เพิ่มกล่อง EP ในหน้าเดิมทันที โดยไม่เรียก API ไม่ตรวจฟอร์ม และไม่เลื่อนไปช่องอื่น
- EP ว่างถูกข้ามตอนส่งตรวจ ส่วน EP ที่มีข้อมูลต้องมีชื่อและมีคลิปหรือเอกสาร
- ปุ่ม `ส่งตรวจ` จุดเดียวสร้างตะกร้า อัปโหลด EP ที่มีข้อมูลทั้งหมด และส่งให้ Boss ตรวจ

# v0.14.308 — ตัดข้อมูลเก่าออกจากหน้าสร้างคอร์ส

- `/course-seller?type=1` เป็นหน้าสร้างใหม่โดยเฉพาะ ไม่ render Dashboard และไม่เปิด draft เก่าอัตโนมัติ
- รีเฟรชแล้วต้องเห็นฟอร์มตะกร้าคอร์สและส่วนเพิ่ม EP พร้อมกันเสมอ
- การตั้งค่าหน้าซ้ำต้องไม่ปิดปุ่มส่งตรวจและไม่คืนข้อความ “กำลังโหลด”
- Feature: `COURSE-EP-CONTINUOUS-002`

# v0.14.310 — เส้นทางคอร์สพาร์ตเนอร์หลังอนุมัติถึงการชำระเงิน

- คอร์สที่ Boss อนุมัติต้อง active และตะกร้าต้อง published ก่อนแสดงในแคตตาล็อกคอร์ส
- ผู้ซื้อคอร์สพาร์ตเนอร์ชำระผ่านบัญชี VisionD และระบบตรวจ EasySlip อัตโนมัติ
- เมื่อ EasySlip ตรวจไม่สำเร็จ ระบบต้องส่งให้ Boss ตรวจแมนนวล อนุมัติเพื่อปลดล็อก หรือปฏิเสธพร้อมเหตุผล
- ห้ามเปิดสิทธิ์ Boss ให้ตรวจแทนคอร์สผู้ขายประเภทอื่น
- Feature: `COURSE-PARTNER-CHECKOUT-001`

# v0.14.312 — สร้างตะกร้าตุ๊กตากระดาษจาก PDF หลายไฟล์

- เพิ่มปุ่มในหน้าการจัดการสินค้าเพื่ออัปโหลด PDF หลายไฟล์ โดยถือ PDF แต่ละไฟล์เป็นหนึ่งกลุ่มโดยไม่ต้องระบุประเภท
- แบ่งหน้าของ PDF ทุกกลุ่มตามจำนวนตะกร้า กระจายเศษให้ตะกร้าแรกตามลำดับ และตรวจว่าไม่มีหน้าซ้ำหรือตกหล่น
- ทุกตะกร้าต้องได้รับหน้าจาก PDF ทุกไฟล์; ถ้า PDF ใดมีหน้าน้อยกว่าจำนวนตะกร้าต้องหยุดก่อนสร้าง
- แสดง Preview ช่วงหน้า จำนวนหน้ารวม รูปปก และราคาเดียว/ราคาแยกก่อนสร้าง
- สร้างเฉพาะร่างหมวด `paper-doll` ด้วยตัวสร้าง Slug ปกติ `paper-doll-NNN`; ไม่สร้างรูปแบบ Slug ใหม่และไม่เผยแพร่อัตโนมัติ
- Feature: `PD-SET-001`

# v0.14.315 — ยกเลิกเพดานไฟล์ต้นทางของตัวสร้างตะกร้าตุ๊กตากระดาษ

- เอาข้อจำกัด 200 MB ออกจาก PNG, PDF และ ZIP ต้นทาง เพราะระบบอ่านและแบ่งหน้าในเครื่องผู้ใช้ก่อนอัปโหลด
- คงการตรวจไฟล์ผลลัพธ์ต่อตะกร้าให้ต่ำกว่าเพดาน API; หากผลลัพธ์ใหญ่เกินให้เพิ่มจำนวนตะกร้าแล้ว Preview ใหม่
- ไม่เปลี่ยนกติกาการกระจายหน้า การสร้างร่าง หมวด และ Slug เดิม

# v0.14.316 — ยกเลิกเพดานจำนวนรายการภายใน ZIP ตุ๊กตากระดาษ

- เอาข้อจำกัด 500 รายการออกจาก ZIP ต้นทาง เพื่อให้ระบบอ่านและแบ่งทุกรายการตามจำนวนจริง
- คงการตรวจ ZIP ว่าต้องมีเฉพาะ PNG หรือเฉพาะ PDF และคงเพดานไฟล์ผลลัพธ์ต่อตะกร้าก่อนเรียก API
- ไม่เปลี่ยนกติกาการกระจายหน้า การสร้างร่าง หมวด และ Slug เดิม

# v0.14.317 — คิวสร้างตะกร้าตุ๊กตากระดาษแบบต่อเนื่อง

- หลัง Preview ระบบสร้างตะกร้าร่างทีละตะกร้าตามลำดับ ห้ามยิงคำขอสร้างพร้อมกัน
- แต่ละตะกร้าแสดงสถานะรอคิว กำลังสร้าง สำเร็จ หรือไม่สำเร็จ
- ถ้าตะกร้าหนึ่งล้มเหลว ระบบทำตะกร้าถัดไปต่อและสรุปผลเมื่อครบคิว
- ปุ่มลองใหม่ส่งเฉพาะตะกร้าที่ไม่สำเร็จ โดยห้ามสร้างตะกร้าที่สำเร็จแล้วซ้ำ
- Feature: `PD-SET-001`

# v0.14.318 — พรีวิวลิงก์สินค้าดิจิทัลและพาร์ทเนอร์คอร์ส

- เปลี่ยน Open Graph และ Twitter Card ของหน้าแรกให้สื่อทั้งสินค้าดิจิทัลหลายหมวดและการเปิดรับพาร์ทเนอร์ลงคอร์สออนไลน์ในภาพเดียว
- ระบุหมวดตัวอย่าง ได้แก่ ใบงาน ระบายสี เกมเสริมพัฒนาการ ตุ๊กตากระดาษ และไฟล์ PDF
- คง canonical URL, robots และ Facebook domain verification เดิม
- Feature: `SHARE-OG-001`

# v0.14.313 — ใช้ Slug ปกติของหมวดตุ๊กตากระดาษ

- เครื่องมือแบ่งชุดตุ๊กตากระดาษต้องเรียกตัวสร้าง Slug เดิมของหมวด `paper-doll` โดยตรง
- รูปแบบคือ `paper-doll-NNN` และต้องนับต่อจากทั้งสินค้าและประวัติ Slug ที่มีอยู่
- ห้ามแยกสาย Slug ใหม่ เช่น `paper-doll-set-NNN`
- Feature: `PD-SET-001`

# v0.14.314 — รองรับ PNG/PDF/ZIP ในตัวสร้างตะกร้าตุ๊กตากระดาษ

- ช่องไฟล์ต้นทางรองรับ PNG, PDF, ZIP ที่มี PNG และ ZIP ที่มี PDF
- PNG ที่เลือกพร้อมกันเป็นหนึ่งกลุ่ม ส่วน PDF/ZIP แต่ละไฟล์เป็นหนึ่งกลุ่ม
- ระบบแปลงทุกกลุ่มเป็นหน้ากลางก่อนแบ่ง: ทุกตะกร้าได้หน้าจากทุกกลุ่ม ไม่มีหน้าซ้ำหรือตกหล่น
- ZIP แบบผสม PNG และ PDF ในไฟล์เดียวไม่อนุญาต เพื่อป้องกันลำดับหน้าไม่ชัดเจน
- ผลลัพธ์ยังเป็น PDF แบบร่าง หมวด `paper-doll` และใช้ Slug ปกติ `paper-doll-NNN`
- Feature: `PD-SET-001`
# v0.14.319 — สร้างรูปตัวอย่างทั้งตะกร้าเดี่ยว

- เพิ่มเครื่องมือหลังบ้านสำหรับแปลงทุกหน้าของ PDF/รูปใน ZIP ของตะกร้าเดี่ยวเป็น JPEG พร้อม SAMPLE
- ดาวน์โหลดผลลัพธ์เป็น ZIP และไม่แก้ไฟล์ต้นฉบับ
- ไม่รวมตะกร้าชุดรวมและตะกร้าคอร์ส

# v0.14.320 — เลือกตะกร้าด้วยการ์ดแคตตาล็อก

- เอา Dropdown เลือกตะกร้าออกจากเครื่องมือสร้างรูปตัวอย่างทั้งตะกร้า
- แสดงเฉพาะตะกร้าเดี่ยวปกติเป็นการ์ดที่มีรูปปก ชื่อ เลขสินค้า Slug และจำนวนไฟล์
...
# v0.14.322 — ตั้งชื่อ ZIP ตามชื่อตะกร้า

- ไฟล์ ZIP จากเครื่องมือสร้างรูปตัวอย่างทั้งตะกร้าต้องใช้ชื่อตะกร้าที่เลือกเป็นชื่อไฟล์
...
# v0.14.324 — แผนที่ระบบคอร์สพาร์ตเนอร์จากโค้ดจริง

- แก้ `COURSE-EP-001` ให้ตรงเส้นทางพาร์ตเนอร์ปัจจุบันและ method API จริง
- เพิ่ม `COURSE-BASKET-001` สำหรับตะกร้าคอร์สและ EP ที่เป็นงานเดียวกัน
- เพิ่ม `COURSE-REVIEW-001` สำหรับส่งตรวจครั้งเดียวและการอนุมัติของ Boss
- ขยาย `COURSE-PARTNER-CHECKOUT-001` ให้ครอบคลุมแคตตาล็อก บัญชีกลาง VisionD ตรวจสลิปอัตโนมัติ และคิวตรวจแมนนวล
- แพตช์นี้แก้เอกสารและตัวตรวจเท่านั้น ไม่เปลี่ยนพฤติกรรมธุรกิจ
