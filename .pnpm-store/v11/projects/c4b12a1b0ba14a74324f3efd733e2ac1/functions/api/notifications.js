...
export async function onRequestGet(ctx){
  ...
  for(const x of issues.results||[])items.push(notice(`seller-slip-${x.id}`,'danger',`มีสลิปยอด ${new Intl.NumberFormat('th-TH').format((Number(x.total)||0)/100)} บาทรอคุณตรวจ`,`ผู้ใช้ @${x.username||'สมาชิก'} · ${x.title} · ${x.order_no}`,'/course-center#slipIssueRows',x.updated_at));
