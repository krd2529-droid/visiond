...
assert.ok(Number(fs.readFileSync('VERSION.txt','utf8').trim().split('.').pop())>=84);
...
assert.match(activation,/verifyPassword/);assert.match(activation,/p\.platform_type='veasy'/);assert.match(activation,/SET user_id=\?,binding_state='bound'/);assert.match(activation,/VEASY_KEY_BOUND_TO_OTHER_ACCOUNT/);assert.match(activation,/ownedShopByLicense/);assert.match(activation,/binding_state='bound'/);assert.match(activation,/VEASY_DEVICE_LIMIT/);assert.match(activation,/issueVision7AppSession/);
