...
- Use the permanent team first: Elon owns implementation and Mark owns reproduction and verification. Jarvis may create the minimum number of temporary specialists genuinely required by the current work, but each must have a distinct, non-duplicated responsibility recorded on the workboard. Do not create extra agents merely because parallel slots are available.
- Every additional agent must start without inherited conversation turns and read `WORKBOARD.md` plus the shared evidence paths. If context is missing, Jarvis must expand the live workboard with every relevant requirement, decision, and reference; there is no fixed limit on necessary workboard detail.
