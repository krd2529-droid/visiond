...
export async function onRequestGet(ctx){
  ...
  const rows=await ctx.env.DB.prepare(`SELECT c.id course_id,p.title,p.status product_status,c.review_status,c.active,u.username owner_username,u.vision5_test_account,
    ...
    FROM courses c JOIN products p ON p.id=c.product_id JOIN users u ON u.id=c.owner_user_id WHERE c.course_origin='seller_rights' AND p.deleted_at IS NULL ORDER BY c.id DESC`).all();
  const items=(rows.results||[]).map(x=>{const paid=Number(x.paid_orders)||0,slips=Number(x.approved_slips)||0,rights=Number(x.learning_rights)||0,invalid=Number(x.invalid_rights)||0,healthy=paid===slips&&slips===rights&&invalid===0;return {...x,paid_orders:paid,approved_slips:slips,learning_rights:rights,invalid_rights:invalid,healthy,difference:rights-slips,event_case_ready:healthy&&paid>0&&x.review_status==='approved'&&Number(x.active)===1&&x.product_status==='published',sample_account:String(x.owner_username||'').toLowerCase()==='user1'||Number(x.vision5_test_account)===1}});
  ...
  const sampleItems=items.filter(x=>x.sample_account),sampleReady=sampleItems.filter(x=>x.event_case_ready).length;
