...
export async function onRequestPost(ctx){
  ...
  const sellerCourse=order.seller_course_id?await ctx.env.DB.prepare('SELECT seller_plan FROM courses WHERE id=?').bind(order.seller_course_id).first():null;
  ...
  try{apiKey=sellerCourse?.seller_plan==='partner_50'?String(ctx.env.EASYSLIP_API_KEY||''):sellerCourse?.seller_plan==='free_manual'?'':order.course_owner_user_id?await loadSellerToken(ctx.env,order.course_owner_user_id):rightsOrder?(paymentSettings.vision5_rights_auto_verify?await loadSellerToken(ctx.env,auth.user.id):''):String(ctx.env.EASYSLIP_API_KEY||'')}catch(error){tokenError=String(error?.message||'TOKEN_DECRYPT_FAILED')}
  const autoEnabled=sellerCourse?.seller_plan==='partner_50'?Boolean(apiKey):sellerCourse?.seller_plan==='free_manual'?false:order.course_owner_user_id?Boolean(apiKey):rightsOrder?paymentSettings.vision5_rights_auto_verify&&Boolean(apiKey):paymentSettings.vision3_auto_verify&&Boolean(apiKey);
  if(!autoEnabled){
