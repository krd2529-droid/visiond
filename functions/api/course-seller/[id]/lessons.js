...
export async function onRequestPost(ctx){
  ...
  try{
    ...
    let lessonId=placeholder?.id,statements=[];
    if(!lessonId){const max=await ctx.env.DB.prepare('SELECT COALESCE(MAX(sort_order),0) n FROM course_lessons WHERE course_id=?').bind(course.id).first();const result=await ctx.env.DB.prepare('INSERT INTO course_lessons(course_id,title,description,sort_order,video_key,video_mime,duration_seconds) VALUES(?,?,?,?,?,?,?)').bind(course.id,title,description,Number(max.n)+10,videoKey,video?.type||null,duration).run();lessonId=result.meta.last_row_id;createdLessonId=lessonId}
    else statements.push(ctx.env.DB.prepare('UPDATE course_lessons SET title=?,description=?,duration_seconds=?,video_key=?,video_mime=?,upload_claim=NULL,upload_claimed_at=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=? AND course_id=? AND upload_claim=?').bind(title,description,duration,videoKey,video?.type||null,lessonId,course.id,claim));
    ...
    statements.push(ctx.env.DB.prepare("UPDATE courses SET expected_episodes=(SELECT COUNT(*) FROM course_lessons WHERE course_id=?),total_minutes=(SELECT CAST(COALESCE(SUM(duration_seconds),0)/60 AS INTEGER) FROM course_lessons WHERE course_id=?),review_status=CASE WHEN license_entitlement_id IS NULL THEN 'draft' ELSE 'pending' END,submitted_at=CASE WHEN license_entitlement_id IS NULL THEN submitted_at ELSE CURRENT_TIMESTAMP END,active=0,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(course.id,course.id,course.id));statements.push(ctx.env.DB.prepare("UPDATE products SET pages=(SELECT COUNT(*) FROM course_lessons WHERE course_id=?),status='draft',updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(course.id,course.product_id));await ctx.env.DB.batch(statements);
    return json({ok:true,id:lessonId,filled_placeholder:Boolean(placeholder),file_count:documents.length,message:`บันทึก EP และไฟล์ประกอบ ${documents.length} ไฟล์แล้ว`},placeholder?200:201);
  }catch(error){if(createdLessonId)await ctx.env.DB.prepare('DELETE FROM course_lessons WHERE id=? AND course_id=?').bind(createdLessonId,course.id).run().catch(()=>{});if(claim)await ctx.env.DB.prepare('UPDATE course_lessons SET upload_claim=NULL,upload_claimed_at=NULL WHERE id=? AND upload_claim=?').bind(placeholder.id,claim).run().catch(()=>{});if(videoKey)await ctx.env.FILES.delete(videoKey).catch(()=>{});for(const item of uploaded)await ctx.env.FILES.delete(item.key).catch(()=>{});return json({error:'อัปโหลด EP ไม่สำเร็จ กรุณาลองใหม่'},500)}
}
