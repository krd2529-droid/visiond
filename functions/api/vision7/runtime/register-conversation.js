...
export async function onRequestPost(ctx){
  ...
  const body=await ctx.request.json().catch(()=>({})),conversationId=cleanConversationId(body.conversation_id),participant=clean(body.platform_participant_id,180),platform=clean(body.platform||'facebook',30).toLowerCase(),displayName=clean(body.display_name||'ลูกค้า',120);
  ...
  try{
    await ctx.env.DB.prepare(`INSERT INTO veasy_conversations(shop_id,id,platform,participant_hash,display_name) VALUES(?,?,?,?,?) ON CONFLICT(shop_id,id) DO UPDATE SET display_name=excluded.display_name,updated_at=CURRENT_TIMESTAMP WHERE veasy_conversations.platform=excluded.platform AND veasy_conversations.participant_hash=excluded.participant_hash`).bind(runtime.shop_id,conversationId,platform,participantHash,displayName).run();
  }catch{return json({error:'ลูกค้าหรือบทสนทนานี้ถูกผูกกับข้อมูลอื่นในร้านแล้ว',code:'VEASY_CONVERSATION_BIND_CONFLICT'},409)}
  const row=await ctx.env.DB.prepare('SELECT shop_id,id,platform,participant_hash,display_name,status,created_at,updated_at FROM veasy_conversations WHERE shop_id=? AND id=?').bind(runtime.shop_id,conversationId).first();
