...
export async function ensureVEasyRuntimeSchema(env){
  ...
  const sql=[
    ...
    `CREATE TABLE IF NOT EXISTS veasy_context_reviews (id TEXT PRIMARY KEY,shop_id TEXT NOT NULL,conversation_id TEXT NOT NULL,question TEXT NOT NULL,bot_answer TEXT NOT NULL,reason TEXT NOT NULL,risk_flag TEXT NOT NULL DEFAULT '',status TEXT NOT NULL DEFAULT 'review' CHECK(status IN ('review','approved','dismissed')),occurrences INTEGER NOT NULL DEFAULT 1,context_id TEXT,reviewed_by TEXT,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,reviewed_at TEXT,FOREIGN KEY(shop_id) REFERENCES veasy_shops(id) ON DELETE CASCADE,FOREIGN KEY(context_id) REFERENCES veasy_sales_contexts(id) ON DELETE SET NULL)`,
    `CREATE TABLE IF NOT EXISTS veasy_context_audit (id TEXT PRIMARY KEY,shop_id TEXT NOT NULL,context_id TEXT,review_id TEXT,action TEXT NOT NULL,actor_user_id TEXT NOT NULL,detail TEXT NOT NULL DEFAULT '',created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(shop_id) REFERENCES veasy_shops(id) ON DELETE CASCADE)`,
    `CREATE INDEX IF NOT EXISTS idx_veasy_sales_context_shop ON veasy_sales_contexts(shop_id,status,category,updated_at DESC)`,
    `CREATE INDEX IF NOT EXISTS idx_veasy_context_review_shop ON veasy_context_reviews(shop_id,status,updated_at DESC)`,
    `CREATE INDEX IF NOT EXISTS idx_veasy_context_audit_shop ON veasy_context_audit(shop_id,created_at DESC)`,
