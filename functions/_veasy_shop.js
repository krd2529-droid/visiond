...
export async function ensureVEasyShopSchema(env){
  ...
  for(const [name,definition] of [['bank_name',"TEXT NOT NULL DEFAULT ''"],['bank_account_name',"TEXT NOT NULL DEFAULT ''"],['bank_account_number',"TEXT NOT NULL DEFAULT ''"],['promptpay_id',"TEXT NOT NULL DEFAULT ''"]])if(!columns.some(column=>column.name===name))await env.DB.prepare(`ALTER TABLE veasy_shops ADD COLUMN ${name} ${definition}`).run();
  ...
  const productColumns=(await env.DB.prepare('PRAGMA table_info(veasy_products)').all()).results||[];
  if(!productColumns.some(column=>column.name==='cover_image_key'))await env.DB.prepare("ALTER TABLE veasy_products ADD COLUMN cover_image_key TEXT NOT NULL DEFAULT ''").run();
