...
async function find(ctx){
  ...
  const publicId=String(ctx.params.publicId||'');
  if(!provider||!/^wh_[a-f0-9]{48}$/.test(publicId))return null;
  return ctx.env.DB.prepare('SELECT id,status,adapter_state FROM veasy_webhook_endpoints WHERE provider=? AND public_id=?').bind(provider,publicId).first();
}
...
export async function onRequestPost(ctx){
  ...
  const credential=await ctx.env.DB.prepare("SELECT secret_ciphertext FROM veasy_channel_credentials WHERE shop_id=(SELECT shop_id FROM veasy_webhook_endpoints WHERE id=?) AND provider='line'").bind(x.id).first();
  if(!credential)return out({error:'LINE ยังไม่พร้อม'},503);

  let valid=false;
  try{valid=await lineSignatureValid(await decryptChannelValue(ctx.env,credential.secret_ciphertext),raw,signature)}catch{}
  ...
  let payload={};
  try{payload=JSON.parse(raw)}catch{return out({error:'JSON ไม่ถูกต้อง'},400)}
  const events=Array.isArray(payload.events)?payload.events:[];

  // LINE Verify sends a signed request with events: []. Return immediately.
  // Schema provisioning belongs to setup/save flows, not the webhook hot path.
