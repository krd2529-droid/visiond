...
- ตรวจเส้นทางจริงของ Channel, Signed Webhook, Inbox, Broadcast และ AI ผู้ช่วยขาย
- เพิ่มรหัส `V12-CHANNEL-001`, `V12-INBOX-001`, `V12-BROADCAST-001`, `V12-SALES-AI-001`
- ไม่เปลี่ยนพฤติกรรมธุรกิจ; Event Case ยังต้องแพตต่อเพื่อทำ Feature Map ระบบที่เหลือ
...
# Patch v0.14.173 — Catalog CPU และเมนูมือถือ

- ลด query หนักแบบ correlated จาก `/api/products` และหยุดล้างถังขยะใน request หน้าร้าน
- API ส่ง `catalog_counts` จากสินค้าที่ `published` และไม่ถูกลบ พร้อม cache ระยะสั้น
- เพิ่มตัวตรวจ mismatch ระหว่างจำนวน API กับการ์ดที่ render จริง
- ปิด backdrop เมนูมือถือทุกครั้งเมื่อโหลดหน้า กลับจาก history หรือสลับแท็บ
- หมวดหน้าร้านรองรับ `โปรยกชุด` (`bundle-deals`)
# Patch v0.14.174 — Vision 5 สามแผน

- ซื้อสิทธิ์: ผู้ซื้อเป็นผู้จ่ายค่า API และผู้สอนได้ 100%
- เริ่มขายฟรี: สูงสุด 3 คอร์ส คอร์สละ 5 EP ไม่มี API และตรวจสลิปเอง
- พาร์ตเนอร์ 50/50: แบ่งยอด VisionD/ผู้สอน 50/50 ก่อน แล้วหักค่า VisionD API 1 บาทจากส่วนผู้สอน
- เพิ่มรูปเงื่อนไขและตัวเลือกแผนตอนสร้างคอร์ส
- งานต่อ: บัญชีพักยอด, รอบจ่ายผู้สอน และการรับเงินเข้าบริษัทสำหรับแผนพาร์ตเนอร์
# Patch v0.14.175 — เงินพาร์ตเนอร์ 50/50

- ออเดอร์แผนพาร์ตเนอร์รับเงินเข้าบัญชีบริษัท VisionD เท่านั้น
- ใช้ EasySlip API กลางของบริษัทตรวจอัตโนมัติ
- เมื่อชำระสำเร็จ สร้างบัญชีพักยอดแบบไม่ซ้ำ: ยอดขาย − 1 บาท แล้วแบ่ง 50/50
- ผู้สอนเห็นยอดรอจ่าย/จ่ายแล้ว และ Boss บันทึกหลักฐานการจ่ายได้
# Patch v0.14.176 — เลขเวอร์ชันกลาง

- ย้ายเลขที่มองเห็นบนหน้าเว็บเข้าสคริปต์กลาง และจัดหน้าแรก หลังบ้าน Dashboard ให้ตรงกัน
- ทุกหน้าที่ใช้เมนูมือถือโหลดเลขกลางพร้อม cache key ใหม่ ป้องกันเลขเก่าค้างกระจายตามหน้า

# Patch v0.14.177 — สูตรพาร์ตเนอร์และรอบจ่าย

- แบ่งยอดขาย VisionD 50% / ผู้สอน 50% ก่อน แล้วหักค่า VisionD API 1 บาทจากส่วนของผู้สอน
- ตัวอย่างยอด 100 บาท: VisionD รับส่วนแบ่ง 50 + API 1 = 51 บาท และผู้สอนรับ 49 บาท
- หลังบ้านรวมรายการรอจ่ายตามผู้สอน บันทึกเป็นรอบจ่ายพร้อมเลขรอบและหลักฐานการโอน
- เปลี่ยนภาพเงื่อนไข 3 แผนและข้อความทุกจุดให้ใช้สูตรเดียวกัน

# Patch v0.14.178 — ปุ่มส่วนแบ่งรายตะกร้าคอร์ส
