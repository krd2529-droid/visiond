...
$("#shopCommissionDashboard").addEventListener("click", async (event) => {
  ...
  try {
    ...
    const model={...card,owner:state.commissionOwnerName,caption:state.commissionCaption,referralUrl:form.elements.referral_url.value};const prepared=await window.VisionDCommissionCard.prepareCommissionCard(model);state.preparedCommission={...prepared,model};const actions=$('#commissionPreparedActions');actions.hidden=false;actions.querySelector('span').textContent=prepared.cached?`ดึงรูปเดิมจากคลังแล้ว ${prepared.files.length} รูป`:`สร้างและบันทึกเข้าคลังแล้ว ${prepared.files.length} รูป`;showToast(actions.querySelector('span').textContent);
  } catch (error) {
