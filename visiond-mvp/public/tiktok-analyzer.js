...
function renderShowcaseProducts(products, orders, demo = false, growthOrders = orders) {
  ...
  const rows = pageProducts.map((product, index) => {
    ...
    return `<tr data-product-id="${escapeHtml(product.product_id)}">${columns.grade ? `<td><span class="type-pill type-${escapeHtml(grade || "unknown")}" title="${grade ? `เกรด ${escapeHtml(grade)}` : "ยังไม่มีข้อมูลเพียงพอสำหรับจัดเกรด"}">${escapeHtml(gradeLabel)}</span></td>` : ""}<td><div class="showcase-product-cell">${picture}<div>${title}<small>${demo ? "ข้อมูลสาธิต" : shopProductLabel(product, orders)}</small><code>${escapeHtml(product.product_id || "ไม่มีรหัสสินค้าในรายงาน")}</code></div></div></td>${columns.sales ? `<td>${(metrics.sales || evidenceSales) ? (metrics.sales || evidenceSales).toLocaleString() : "–"}</td>` : ""}${columns.commission ? `<td>${metrics.commission ? money(metrics.commission) : evidenceCommission ? `${escapeHtml(evidenceCommission)}%` : "–"}</td>` : ""}${columns.gmvLatest ? `<td class="gmv-cell">${product.analysisOnly ? "–" : compactMoney(gmv.latest, gmv.currency)}</td>` : ""}${columns.gmvPrevious ? `<td class="gmv-cell">${product.analysisOnly ? "–" : compactMoney(gmv.previous, gmv.currency)}</td>` : ""}${columns.growth ? `<td>${product.analysisOnly ? "–" : `<span class="gmv-growth ${growthClass}">${growthLabel}</span>`}</td>` : ""}${columns.score ? `<td>${selection.score !== void 0 ? `${Number(selection.score) || 0}/100` : "–"}</td>` : ""}${columns.reason ? `<td class="showcase-reason">${escapeHtml(gradeReason || "–")}</td>` : ""}<td>${productLinkControl(product.product_url)}</td><td class="showcase-row-action">${removeAction}</td></tr>`;
  }).join("");
  const columnCount = 3 + Object.values(columns).filter(Boolean).length, headers = `${columns.grade ? "<th>เกรด</th>" : ""}<th>รูปและสินค้า</th>${columns.sales ? "<th>ขายได้</th>" : ""}${columns.commission ? "<th>ค่าคอม</th>" : ""}${columns.gmvLatest ? "<th>GMV 7 วัน</th>" : ""}${columns.gmvPrevious ? "<th>GMV 7 วันก่อน</th>" : ""}${columns.growth ? "<th>เติบโต</th>" : ""}${columns.score ? "<th>คะแนน</th>" : ""}${columns.reason ? "<th>เหตุผลล่าสุด</th>" : ""}<th>ลิงก์สินค้า</th><th>จัดการ</th>`, gmvNote = columns.gmvLatest || columns.gmvPrevious || columns.growth ? `<p class="gmv-note">GMV เทียบ 7 วันล่าสุดกับ 7 วันก่อนหน้า สิ้นสุดวันที่ ${escapeHtml(state.shopDateTo)} · คำนวณจากรายงานออเดอร์</p>` : "";
