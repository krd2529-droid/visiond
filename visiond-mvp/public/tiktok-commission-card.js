(function(global){
  ...
  function drawCommissionCard(model,pageIndex=0){
    ...
    if(channels.length){channels.forEach((item,index)=>{const column=index<5?0:1,row=index%5,x=68+column*482,y=520+row*132,amount=Math.max(0,Number(item.amount)||0),number=firstNumber+index;roundRect(ctx,x,y,462,112,20,'rgba(255,255,255,.12)','rgba(98,232,223,.45)');ctx.fillStyle='#8ff7ed';ctx.font='800 24px "Noto Sans Thai",sans-serif';ctx.fillText(`ช่องที่ ${number}`,x+22,y+42);ctx.fillStyle='#fff';ctx.font='800 29px "Noto Sans Thai",sans-serif';ctx.fillText(money(amount,model.currency),x+22,y+86)})}
  ...
  async function fingerprint(model){const data=JSON.stringify({privacyVersion:2,owner:model.owner,range:model.range,total:Number(model.total),currency:model.currency,basis:model.basis,channels:(model.channels||[]).map(item=>Number(item.amount)),referralUrl:model.referralUrl});return hex(await crypto.subtle.digest('SHA-256',new TextEncoder().encode(data)))}
  async function shareFiles(files,model){if(navigator.share&&navigator.canShare?.({files})){await navigator.share({title:'สรุปค่าคอม VisionD',text:model.caption||model.referralUrl||'',files});return'shared'}files.forEach((file,index)=>setTimeout(()=>{const link=document.createElement('a');link.download=file.name;link.href=URL.createObjectURL(file);link.click();setTimeout(()=>URL.revokeObjectURL(link.href),1000)},index*250));return'downloaded'}
  async function prepareCommissionCard(model){
  ...
  async function shareCommissionCard(model){const prepared=await prepareCommissionCard(model);return shareFiles(prepared.files,model)}
  global.VisionDCommissionCard={drawCommissionCard,drawCommissionCards,prepareCommissionCard,sharePreparedCommission:shareFiles,shareCommissionCard};
})(window);
