...
const starterCategorySlugs = new Set(["dinosaur", "document"]);
const panels = {
...
let digitalStorefrontPaused=null,digitalStorefrontManualPaused=null;
async function loadDigitalStorefrontEmergency(){
  const button=document.querySelector('#digitalStorefrontEmergency'),status=document.querySelector('#digitalStorefrontEmergencyStatus'),details=document.querySelector('#d1QuotaBreakerDetails');if(!button)return;
  try{const response=await fetch('/api/admin/basket-visibility',{cache:'no-store'}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'อ่านสถานะไม่สำเร็จ');digitalStorefrontPaused=data.storefront_paused===true;digitalStorefrontManualPaused=data.storefront_manual_paused===true;const quota=data.quota_breaker||{},checked=quota.sampled_at?new Intl.DateTimeFormat('th-TH',{dateStyle:'short',timeStyle:'short',timeZone:'Asia/Bangkok'}).format(new Date(quota.sampled_at)):'ยังไม่มีผลวัด',number=value=>typeof value==='number'&&Number.isFinite(value)?new Intl.NumberFormat('th-TH').format(value):'ไม่ทราบ',pct=value=>typeof value==='number'&&Number.isFinite(value)?`${value.toFixed(1)}%`:'ไม่ทราบ',reset=quota.sample_day?new Intl.DateTimeFormat('th-TH',{dateStyle:'short',timeStyle:'short',timeZone:'Asia/Bangkok'}).format(new Date(Date.parse(`${quota.sample_day}T00:00:00.000Z`)+86400000)):'ไม่ทราบ',freshReady=quota.configured===true&&quota.control_available===true&&quota.status==='ok'&&quota.stale!==true,autoLabel=quota.auto_closed?'ปิด':freshReady?'เปิด (ยืนยันล่าสุด)':'ยังไม่ยืนยัน',readiness=freshReady?'พร้อม':quota.storage_configured===false?'ไม่ได้ตั้งค่าที่เก็บสถานะ':'ยังไม่พร้อม';button.querySelector('b').textContent=digitalStorefrontManualPaused?(quota.auto_closed?'ยกเลิกการปิดด้วยตนเอง (Auto ยังปิด)':'เปิดหน้าร้านไฟล์ดิจิทัล'):'ปิดหน้าร้านด้วยตนเอง';status.textContent=`Effective ${digitalStorefrontPaused?'ปิด':'เปิด'} · Manual ${digitalStorefrontManualPaused?'ปิด':'เปิด'} · Auto ${autoLabel}`;if(details)details.textContent=`อ่าน ${number(quota.rows_read)} / ${number(quota.read_limit)} (${pct(quota.read_percent)}) · เขียน ${number(quota.rows_written)} / ${number(quota.write_limit)} (${pct(quota.write_percent)}) · วัด ${checked} · รีเซ็ต UTC รอบถัดไป ${reset} · ${readiness}${quota.stale?' · ข้อมูลเก่าหรือผิดพลาด':''} · ${quota.source||'cloudflare_d1_account_daily'} · ${quota.reason||quota.status||'unknown'}${quota.last_error?` · ${quota.last_error}`:''}`;button.title=status.textContent;button.classList.toggle('is-paused',digitalStorefrontPaused);button.disabled=false}catch(error){button.disabled=true;button.querySelector('b').textContent='ตรวจสถานะไม่สำเร็จ';status.textContent=error.message;if(details)details.textContent=error.message}
}
const d1QuotaBreakerRefresh=document.querySelector('#d1QuotaBreakerRefresh');if(d1QuotaBreakerRefresh)d1QuotaBreakerRefresh.onclick=async()=>{d1QuotaBreakerRefresh.disabled=true;try{await loadDigitalStorefrontEmergency()}finally{d1QuotaBreakerRefresh.disabled=false}};
const digitalStorefrontEmergency=document.querySelector('#digitalStorefrontEmergency');if(digitalStorefrontEmergency)digitalStorefrontEmergency.onclick=async()=>{const next=!digitalStorefrontManualPaused,verb=next?'ปิด':'เปิด';if(!confirm(`${verb}หน้าร้านไฟล์ดิจิทัลแบบ Manual ตอนนี้หรือไม่? สถานะ Auto จากโควตา D1 จะไม่ถูกแก้ไข`))return;digitalStorefrontEmergency.disabled=true;try{const response=await fetch('/api/admin/basket-visibility',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({paused:next})}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'บันทึกไม่สำเร็จ');digitalStorefrontPaused=data.storefront_paused===true;digitalStorefrontManualPaused=data.storefront_manual_paused===true;await loadDigitalStorefrontEmergency();alert(data.message)}catch(error){alert(error.message);digitalStorefrontEmergency.disabled=false}};
...
function productCardMarkup(p) {
    ...
    const vision5Rights=p.slug==="course-selling-rights";
    return `<article class="product-admin-card${checked?' bulk-selected':''}">${vision5Rights?'<span class="product-select-box" aria-hidden="true"></span>':`<label class="product-select-box" title="เลือกตะกร้านี้เพื่อย้ายหมวดหรือลบ"><input type="checkbox" data-select-product="${p.id}" ${checked?'checked':''} aria-label="เลือก ${esc(p.title)} เพื่อย้ายหมวดหรือลบ"></label>`}<img src="${esc(p.cover_url || "/assets/product-placeholder.svg")}" alt=""><div><h3>${esc(p.title)}</h3><p>เลข ${p.id} · ${esc(p.slug)} · ${esc(p.category || "ไม่ระบุหมวด")}</p><div class="product-meta"><span>${money(p.price)}</span><span>${p.status === "published" ? "เปิดขาย" : "แบบร่าง"}</span><span>${vision5Rights?"จัดการโดย Vision 5":`${Number(p.file_count) || 0} ไฟล์`}</span></div>${vision5Rights?'<div class="product-card-downloads"><span>รวมในยอดหน้าบ้านเพื่อให้จำนวนตรงกัน</span></div>':`<div class="product-card-downloads"><a class="product-download-action" href="/api/admin/product-previews.zip?product_id=${p.id}">ดาวน์โหลดรูปปก + รูปตัวอย่าง</a>${fileAction}</div>`}</div>${vision5Rights?'<a class="vds-btn vds-btn--secondary" href="/product.html?slug=course-selling-rights">ดูหน้าสินค้า</a>':`<button type="button" data-edit-product="${p.id}">แก้ไข</button>`}</article>`;
}
