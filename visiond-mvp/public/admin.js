import("/facebook-chat.js?v=014196");
const money = (n) =>
...
function productCardMarkup(p) {
    ...
    return `<article class="product-admin-card${checked?' bulk-selected':''}">${vision5Rights?'<span class="product-select-box" aria-hidden="true"></span>':`<label class="product-select-box" title="เลือกตะกร้านี้เพื่อลบ"><input type="checkbox" data-select-product="${p.id}" ${checked?'checked':''} aria-label="เลือก ${esc(p.title)}"></label>`}<img src="${esc(p.cover_url || "/assets/product-placeholder.svg")}" alt=""><div><h3>${esc(p.title)}</h3><p>เลข ${p.id} · ${esc(p.slug)} · ${esc(p.category || "ไม่ระบุหมวด")}</p><div class="product-meta"><span>${money(p.price)}</span><span>${p.status === "published" ? "เปิดขาย" : "แบบร่าง"}</span><span>${vision5Rights?"จัดการโดย Vision 5":`${Number(p.file_count) || 0} ไฟล์`}</span></div>${vision5Rights?'<div class="product-card-downloads"><span>รวมในยอดหน้าบ้านเพื่อให้จำนวนตรงกัน</span></div>':`<div class="product-card-downloads"><a class="product-download-action" href="/api/admin/product-previews.zip?product_id=${p.id}">ดาวน์โหลดรูปปก + รูปตัวอย่าง</a>${fileAction}</div>`}</div>${vision5Rights?'<a class="vds-btn vds-btn--secondary" href="/product.html?slug=course-selling-rights">ดูหน้าสินค้า</a>':`<button type="button" data-edit-product="${p.id}">แก้ไข</button>`}</article>`;
}
...
showAdminNotice();
init();
import('/mouse-ui.js?v=014196');
import('/i18n.js?v=014196');
