...
async function loadUsers() {
  ...
  usersTable.innerHTML = `<div class="user-table"><div class="user-row user-head"><b>สมาชิก</b><b>ไอดี</b><b>เบอร์โทร</b><b>ระดับ/สถานะ</b><b>จัดการ</b></div>${users.map((u) => `<div class="user-row"><div><b>${esc(u.name)}</b><small>${esc(u.email)}</small></div><span>${esc(u.username || "-")}</span><a href="tel:${esc(u.phone || "")}">${esc(u.phone || "ไม่ได้ระบุ")}</a><div class="user-status-stack"><span class="role-badge ${esc(u.role)}">${roleText[u.role] || esc(u.role)}</span>${u.is_course_owner ? '<span class="course-owner-badge">เจ้าของคอร์ส</span>' : ''}</div><div>${viewer.role === "boss" && u.role !== "boss" ? `<select data-role-id="${u.id}"><option value="user" ${["user", "customer"].includes(u.role) ? "selected" : ""}>User</option><option value="admin" ${u.role === "admin" ? "selected" : ""}>Admin</option></select><button data-save-role="${u.id}">บันทึก</button>` : '<span class="muted">ดูอย่างเดียว</span>'}</div></div>`).join("")}</div>`;
  document
