...
function addCurrentProductToCart(){const cart=getCart();if(cart.some(item=>item.slug===currentProduct.slug)){syncProductCartButton();return}if(cart.length>=30){alert('เลือกสินค้าได้สูงสุด 30 ตะกร้า');return}cart.push({slug:currentProduct.slug,title:currentProduct.title,price:Number(currentProduct.price)||0,cover_url:currentProduct.cover_url||'/assets/product-placeholder.svg'});localStorage.setItem('vd_cart',JSON.stringify(cart));window.visiondPixel?.track('AddToCart',{content_ids:[String(currentProduct.id||currentProduct.slug)],content_name:currentProduct.title,content_type:'product',value:Number(currentProduct.price||0)/100,currency:'THB'});updateCartCount();syncProductCartButton()}
