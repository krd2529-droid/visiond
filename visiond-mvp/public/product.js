...
function renderProduct(product){
  ...
  if(product.category==='resale-rights'){
    ...
    detailSection?.firstElementChild?.insertAdjacentHTML('afterend','<article class="course-rights-read-first"><h2>โปรดอ่านก่อนซื้อ</h2><p><b>ผู้ซื้อจำเป็นต้องอ่านเงื่อนไขสิทธิ์ลงขายคอร์สและการตรวจสลิปให้ครบถ้วนก่อนสั่งซื้อ</b></p><a class="secondary-button center" href="/Vision5-course-selling-rights-read-before-buy.txt" target="_blank" rel="noopener">เปิดเอกสารแนบ Vision 5</a><ul><li>ระบบใช้ EasySlip API Access Token ของผู้ซื้อสิทธิ์เอง</li><li>เมื่อลูกค้าซื้อคอร์ส ระบบตรวจสลิปอัตโนมัติและปลดล็อกคอร์สทันทีเมื่อข้อมูลถูกต้อง</li><li>หาก API อ่านไม่ได้หรือข้อมูลผิดปกติ เจ้าของคอร์สต้องตรวจสลิปและอนุมัติหรือปฏิเสธเอง</li><li>ก่อนสั่งซื้อ ระบบจะให้ยืนยันว่าอ่านเอกสารแนบครบถ้วนแล้ว</li></ul></article><article class="course-cover-policy"><h2>กติกาภาพปกคอร์ส</h2><p><b>ใช้ชื่อแพลตฟอร์มเป็นข้อความธรรมดาได้ แต่ห้ามนำโลโก้หรือสื่อของแบรนด์อื่นมาใช้เป็นภาพปกโดยไม่มีหลักฐานอนุญาต</b></p><ul><li>ห้ามใช้โลโก้ เครื่องหมายการค้า ภาพหน้าจอ หรือสื่อประชาสัมพันธ์ของบุคคลและแบรนด์อื่นเป็นภาพปก</li><li>หากจำเป็นต้องใช้ ต้องมีหนังสืออนุญาตและส่งหลักฐานให้ VisionD ตรวจสอบก่อน</li><li>พิมพ์ชื่อแพลตฟอร์มเป็นข้อความธรรมดาได้ เช่น “สอนเปิดร้านบน Shopee” หรือ “เทคนิคขายสินค้าบน Lazada”</li><li>ห้ามใช้สี ตัวอักษร รูปแบบ หรือองค์ประกอบที่ทำให้เข้าใจว่าเป็นคอร์สทางการหรือได้รับการรับรองจากแบรนด์</li><li>VisionD มีสิทธิ์ขอให้เปลี่ยนหรือระงับภาพปกที่เสี่ยงละเมิดสิทธิ์</li></ul></article>');
...
async function beginPurchase(){
  ...
  try{
    ...
    const response=await fetch('/api/orders',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({productSlugs:[currentProduct.slug],quantities:{[currentProduct.slug]:quantity},rights_terms_acknowledged:buyingRights})});
