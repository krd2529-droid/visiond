import {getSession, clearSession} from '/auth-session.js?v=02057';
...
export async function initAccountNav(){
  ...
  const syncNavGroups=()=>{
    let primary=nav.querySelector(':scope > .nav-primary-group'),utility=nav.querySelector(':scope > .nav-utility-group');
    if(!primary){primary=document.createElement('span');primary.className='nav-primary-group';nav.prepend(primary)}
    if(!utility){utility=document.createElement('span');utility.className='nav-utility-group';nav.append(utility)}
    [...nav.children].filter(item=>item!==primary&&item!==utility).forEach(item=>(item.matches('.nav-cart-action,.nav-admin-link,.nav-member-account,.course-owner-badge,.nav-logout,.vd-language-switcher')?utility:primary).append(item));
    [...primary.children].filter(item=>item.matches('.nav-cart-action,.nav-admin-link,.nav-member-account,.course-owner-badge,.nav-logout,.vd-language-switcher')).forEach(item=>utility.append(item));
  };
  syncNavGroups();window.VisionDSyncNavGroups=syncNavGroups;
  ...
  try{
    ...
    logout.onclick=async()=>{logout.disabled=true;clearSession();await fetch('/api/auth/logout',{method:'POST'});location.href='/'};
