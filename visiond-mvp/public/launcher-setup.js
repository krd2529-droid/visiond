(()=>{
 const expectedHash='da730c64607f39adde96fde1caa4de1de0df83ff5beab47fcb6b870cece59909';
 ...
 Promise.race([(async()=>{
  const r=await fetch('/downloads/visiond-helper/0.20.77/release.json',{cache:'no-store',signal:controller.signal});
  if(!r.ok)throw Error();const m=await r.json();
  if(m.version!=='0.20.77'||m.signature_status!=='NotSigned'||m.executable?.file!=='VisionD-Helper-Setup.exe'||m.executable.sha256!==expectedHash)throw Error();return m;
  ...
  const link=document.getElementById('download');link.href='/downloads/visiond-helper/0.20.77/VisionD-Helper-Setup.exe';link.removeAttribute('aria-disabled');
