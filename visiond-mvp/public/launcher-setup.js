(()=>{
 ...
 Promise.race([(async()=>{
  const r=await fetch('/downloads/visiond-helper/0.20.75/release.json',{cache:'no-store',signal:controller.signal});
  if(!r.ok)throw Error();const m=await r.json();
  if(m.version!=='0.20.75'||m.signature_status!=='NotSigned'||m.executable?.file!=='VisionD-Helper-Setup.exe'||!/^[a-f0-9]{64}$/.test(m.executable.sha256))throw Error();return m;
  ...
  const link=document.getElementById('download');link.href='/downloads/visiond-helper/0.20.75/VisionD-Helper-Setup.exe';link.removeAttribute('aria-disabled');
