(()=>{
 const expectedHash='bb1ca2900ad3ca16d5caae27b49e999e12b2bc42c31b25f17e202c4f507216d7';
 ...
 Promise.race([(async()=>{
  const r=await fetch('/downloads/visiond-helper/0.20.76/release.json',{cache:'no-store',signal:controller.signal});
  if(!r.ok)throw Error();const m=await r.json();
  if(m.version!=='0.20.76'||m.signature_status!=='NotSigned'||m.executable?.file!=='VisionD-Helper-Setup.exe'||m.executable.sha256!==expectedHash)throw Error();return m;
  ...
  const link=document.getElementById('download');link.href='/downloads/visiond-helper/0.20.76/VisionD-Helper-Setup.exe';link.removeAttribute('aria-disabled');
