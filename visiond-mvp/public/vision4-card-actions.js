(()=>{
  ...
  const decorate=()=>root.querySelectorAll('.v3-review-draft').forEach(card=>{
    ...
    const edit=document.createElement('a');edit.className='v4-edit-draft';edit.href=`/vision4-edit.html?id=${encodeURIComponent(id)}`;edit.textContent='แก้ไข';
    const remove=document.createElement('button');remove.type='button';remove.className='v4-delete-draft';remove.textContent='ลบการ์ด';
