...
function fillCourseForm(course){if(!course)return;for(const [name,value] of Object.entries({title:course.title,teacher_name:course.teacher_name,short_description:course.short_description||'',contact_info:course.contact_info||'',description:course.description||'',price_baht:(Number(course.price)||0)/100,content_minutes:Number(course.total_minutes)||1,expected_episodes:Number(course.expected_episodes)||1}))if(courseForm.elements[name])courseForm.elements[name].value=value;let tags=[];try{tags=JSON.parse(course.platform_tags||'[]')}catch{}courseForm.querySelectorAll('[name="platform_tags"]').forEach(input=>input.checked=tags.includes(input.value));const known=new Set([...courseForm.querySelectorAll('[name="platform_tags"]')].map(input=>input.value)),other=tags.find(tag=>!known.has(tag));courseForm.elements.platform_other.value=other||'';courseForm.elements.payment_account.value=String(course.payment_account_number||'').replace(/\D/g,'')==='4441181181'?'boss_krungsri':'visiond';}
async function openLessons(id) { currentCourseId = id; const course = courses.find((c) => Number(c.id) === Number(id)); fillCourseForm(course);lessonManager.hidden = false; closeLessons.hidden=false;coursePaymentForm.hidden=false; lessonCourseTitle.textContent = course?.title || ""; lessonForm.elements.course_id.value = id; courseMessage.textContent=course?.status==='published'?'คอร์สนี้เผยแพร่แล้ว · บันทึกเพื่อแก้ข้อมูลล่าสุด':'เปิดฉบับร่างแล้ว · แก้ข้อมูลและกดบันทึกร่างได้';lessonManager.scrollIntoView({ behavior: "smooth" }); await loadLessons(id); }
async function loadLessons(id) {
  ...
  lessonList.innerHTML = items.length ? items.map((l, i) => `<div class="lesson-admin-item"><span><b>EP ${i + 1} · ${esc(l.title)}</b><small> ${l.has_video ? "คลิป " : ""}${l.has_pdf ? (l.pdf_mime === "image/jpeg" ? "เอกสาร JPEG" : "เอกสาร PDF") : ""}</small></span><button data-delete="${l.id}">ลบ</button></div>`).join("") : "<p>ยังไม่มี EP</p>";
...
saveCourseDraft.onclick=async()=>{if(!courseForm.reportValidity())return;const original=saveCourseDraft.textContent;saveCourseDraft.disabled=true;saveCourseDraft.textContent='กำลังบันทึก…';courseMessage.textContent='กำลังบันทึกร่าง…';try{if(!currentCourseId)await createDraft();else{const r=await fetch(`/api/admin/courses/${currentCourseId}`,{method:'POST',body:new FormData(courseForm)}),d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||'บันทึกร่างไม่สำเร็จ');courseMessage.textContent='บันทึกร่างล่าสุดแล้ว';await loadCourses();await loadLessons(currentCourseId)}}catch(error){courseMessage.textContent=error.message}finally{saveCourseDraft.disabled=false;saveCourseDraft.textContent=original}};
