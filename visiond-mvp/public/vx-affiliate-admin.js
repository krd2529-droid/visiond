...
function render() {
  $('#commissions').innerHTML = `<h2>รายการค่าคอม</h2><p>จ่ายทุกวันที่ 1 · รอบนี้รวมยอดชำระสำเร็จก่อนวันที่ 1 ตามเวลาไทย ยอดตั้งแต่วันที่ 1 เข้ารอบถัดไป</p>${data.items.map(item => `<article class="admin-row"><label><input type="checkbox" data-pay-id="${esc(item.id)}" ${item.status === 'payable' ? '' : 'disabled'}> ${esc(item.referrer)}</label><b>${money(item.amount)}</b><span>${esc(item.status)} · ยอดเต็ม ${money(item.base_amount)} × ${(Number(item.rate_bps)||0)/100}%</span><div>${actions(item)}</div></article>`).join('') || '<p>ไม่มีรายการ</p>'}`;
  ...
  $('#payouts').innerHTML = `<h2>รอบจ่ายและเอกสารค่าคอม</h2>${data.payouts.map(payout => `<article class="admin-row"><b>${esc(payout.payout_no)} · ${money(payout.amount)}</b><span>${esc(payout.referrer)} · ${esc(payout.status)}</span><a class="statement-link" href="/vx-commission-statement.html?id=${encodeURIComponent(payout.id)}">ดูเอกสาร</a>${payout.status === 'processing' ? `<button data-paid="${esc(payout.id)}">บันทึกว่าจ่ายแล้ว</button>` : ''}</article>`).join('') || '<p>ยังไม่มีรอบจ่าย</p>'}`;
}
