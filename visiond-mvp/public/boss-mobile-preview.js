import {getSession, subscribeAuth} from '/auth-session.js?v=02057';
let cleanupPreview = () => {};
const bossPreviewDesktop = !matchMedia("(max-width: 760px), (pointer: coarse)").matches;
if (window.self === window.top && !location.pathname.startsWith("/admin") && bossPreviewDesktop) {
  subscribeAuth(user => {
    cleanupPreview();
    cleanupPreview = () => {};
    if (user?.role === "boss") initBossMobilePreview();
  });
  getSession();
}
...
async function initBossMobilePreview() {
  try {
    ...
    const onKey = event => event.key === "Escape" && !overlay.hidden && close();
    document.addEventListener("keydown", onKey);
    cleanupPreview = () => {
      if (!overlay.hidden) close();
      launch.remove();overlay.remove();
      document.querySelector("#boss-mobile-preview-style")?.remove();
