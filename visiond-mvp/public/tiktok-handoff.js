(() => {
  ...
  const match=new RegExp(`^id=(${uuid})&slot_id=(${uuid})&profile_kind=(slot|channel)&ticket=([0-9a-f]{64})$`).exec(fragment);
  ...
  fetch('/api/tiktok/handoff-redeem',{method:'POST',credentials:'same-origin',cache:'no-store',referrerPolicy:'no-referrer',headers:{'content-type':'application/json'},body:JSON.stringify({id:match[1],slot_id:match[2],profile_kind:match[3],ticket:match[4]})}).then(async response=>{
