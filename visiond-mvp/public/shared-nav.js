...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  if(nav){
    ...
    const links=[['nav-home-link','/','หน้าแรก'],['','/digital-products.html','สินค้าดิจิทัล'],['nav-course-basket','/course-center','ศูนย์จัดการคอร์ส'],['','/courses.html','ระบบ V-Learning'],['','/vtools','Vtools'],['nav-guides-link','/guides.html','คู่มือการใช้งาน'],['','/blog.html','บทความ'],['','/about.html','เกี่ยวกับเรา']];
    nav.innerHTML=links.map(([className,href,label])=>{const target=href.replace(/\.html$/,'');const current=path===target;return `<a${className?` class="${className}"`:''} href="${href}"${current?' aria-current="page"':''}>${label}</a>`}).join('')+'<a id="navLogin" class="login-link" href="/login.html">เข้าสู่ระบบ</a><a id="navRegister" class="signup-link" href="/register.html">สมัครสมาชิก</a><a class="cart-nav" href="/cart.html"><span aria-hidden="true">🛒</span> รถเข็น <b data-cart-count>0</b></a>';
