...
if(!document.querySelector('link[href^="/promotion.css"]'))document.head.insertAdjacentHTML('beforeend','<link rel="stylesheet" href="/promotion.css?v=014407">');
...
function render() {
  ...
  cartItems.innerHTML = items.length
    ? items
        .map(
          (p, i) =>
            `<article class="cart-product${p.category==='bundle-deals'||p.category==='resale-rights'||p.slug==='course-selling-rights'||p.product_kind==='course'||p.product_kind==='vision7-key'?' no-bundle-discount':''}"><img src="${esc(p.cover_url || "/assets/product-placeholder.svg")}" alt="รูป ${esc(p.title)}"><div><small>${esc(p.category_label || p.category || "ไฟล์ดิจิทัล")}</small><h2>${esc(p.title)}</h2><p>${p.product_kind==='vx-access'?'สิทธิ์ใช้งาน VX 30 วัน · เริ่มเมื่อยืนยันชำระเงิน · ไม่ใช่ไฟล์ดาวน์โหลด':p.product_kind==='vision7-key'?'โปรแกรม VBot พร้อมคีย์ตามอายุที่เลือก':p.category==='resale-rights'||p.slug==='course-selling-rights'?'1 ชิ้น = 1 เครดิตสำหรับเปิดตะกร้าคอร์ส 1 ตะกร้า':p.product_kind==='course'?`คอร์ส V-Learning • ${esc(p.pages || '-')} EP`:'ไฟล์ดิจิทัลพร้อมดาวน์โหลด • '+esc(p.pages || '-')+' แผ่น'}</p>${vbotControls(p,i)}${p.category==='bundle-deals'?'<strong class="cart-no-promo-note">รับราคาโปรโมชั่นสินค้าแล้ว · ไม่ร่วมส่วนลดโปรรถเข็น</strong>':p.category==='resale-rights'||p.slug==='course-selling-rights'?'<strong class="cart-no-promo-note">ไม่ร่วมโปรส่วนลด · ไม่คืนเงิน เว้นแต่ระบบยังใช้งานไม่ได้ภายใน 7 วันและ VisionD ตรวจสอบว่าเกิดจากระบบจริง</strong>':p.product_kind==='course'?'<strong class="cart-no-promo-note">คอร์สไม่ร่วมโปรส่วนลดหลายตะกร้า</strong>':''}<a href="${p.product_kind==='vx-access'?'/vtools':p.product_kind==='vision7-key'?`/vtools?app=${encodeURIComponent(p.vbot_app_code||'')}`:p.product_kind==='course'?`/course.html?id=${encodeURIComponent(p.course_id||'')}`:`/product.html?slug=${encodeURIComponent(p.slug)}`}">ดูรายละเอียด</a></div><div class="cart-product-price">${Number(p.promotion_percent)>0?`<span class="vd-promo-price"><del>${money(p.original_price)}</del><strong>${money(p.price)}</strong></span>`:`<b>${money(p.price)}</b>`}${p.category==='resale-rights'||p.slug==='course-selling-rights'?`<label class="rights-quantity">จำนวนสิทธิ์<input data-rights-quantity="${i}" type="number" min="1" max="30" value="${Number(p.quantity)||1}"></label><small>ได้รับ ${Number(p.quantity)||1} เครดิต · รวม ${money(Number(p.price)*(Number(p.quantity)||1))}</small>`:''}<button data-remove="${i}" type="button">ลบออกจากตะกร้า</button></div></article>`,
        )
        .join("")
...
slipForm.onsubmit = async (e) => {
  ...
  try {
    ...
    const learningOrder = isVLearningOrder(order),vxOrder=order.items?.some(item=>item.product_kind==='vx-access');
    ...
    if(vxOrder&&d.auto_approved)slipMessage.textContent='ยืนยันการชำระเงินแล้ว กำลังเปิดหน้าสิทธิ์ VX';
    const destination = vxOrder&&d.auto_approved?'/vtools':learningOrder
...
// Feature: COMMERCE-ORDER-001 — ตะกร้า ออเดอร์ และการชำระเงิน
