...
assert.match(client, /data-open-channel-settings/);
assert.match(client, /setWorkspaceView\("input"\)/);
assert.match(client, /ไปตั้งค่าช่อง/);
assert.doesNotMatch(client.match(/function renderShowcasePermission[\s\S]*?\n\}/)?.[0] || "", /href="\/api\/tiktok-shop\/connect/);
assert.match(css, /\.workspace-output #tiktokConnection/);
assert.match(css, /\.showcase-permission button\{/);
