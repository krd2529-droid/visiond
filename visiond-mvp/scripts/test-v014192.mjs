...
assert.match(version.trim(),/^v0\.14\.(192|193)$/);assert.match(index,/WEB v0\.14\.(192|193)/);assert.match(admin,/ADMIN v0\.14\.(192|193)/);assert.match(admin,/href="\/v12-connect\.html"[\s\S]*V12[\s\S]*V Connect/);
...
assert.match(api,/SELECT c\.id,c\.shop_id,c\.display_name,c\.platform,c\.updated_at,s\.name shop_name/);assert.match(api,/return json\(\{conversation,messages\}/);assert.match(api,/return json\(\{conversations:rows,channels:/);
