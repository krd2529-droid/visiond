...
const [products,cart,catalog,status,schema,migration,migration88,migration90,migration91,indexHtml,adminHtml,digitalHtml,cartHtml,version]=await Promise.all([
  ...
  read('migrations/0087_storefront_catalog_indexes.sql'),read('migrations/0088_tiktok_analyzer_pagination.sql'),read('migrations/0090_catalog_metadata_and_retention.sql'),read('migrations/0091_catalog_metadata_backfill.sql'),read('public/index.html'),
...
assert.match(migration90,/trg_storefront_catalog_products_update/,'product mutations must rotate the public catalog cache revision');
assert.match(migration88,/trg_storefront_catalog_categories_update/,'category mutations must rotate the public catalog cache revision');
...
for(const html of [indexHtml,adminHtml])assert.match(html,/v0\.20\.62/);
for(const html of [indexHtml,digitalHtml])assert.match(html,/catalog-sync\.js\?v=02060/);
assert.match(cartHtml,/cart\.js\?v=02059/);
assert.equal(version.trim(),'v0.20.62');
...
sqlite.exec(migration88.slice(migration88.indexOf('INSERT OR IGNORE INTO settings')));
...
sqlite.exec(migration90);sqlite.exec(migration91);
...
try{
  ...
  const cacheRequest=request('/api/products?q=cache-revision-target'),beforeResponse=await listStorefront({env,request:cacheRequest}),before=await beforeResponse.json();assert.equal(before.pagination.total,1);assert.equal(cacheEntries.size,2,'first response must cache canonical counts separately from the cursor page');
  ...
  const afterResponse=await listStorefront({env,request:cacheRequest}),after=await afterResponse.json();assert.equal(after.pagination.total,0,'same URL must not return stale cached totals after mutation');assert.equal(cacheEntries.size,4,'revision must select new targeted count and page cache keys');
