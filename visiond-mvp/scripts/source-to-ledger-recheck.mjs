...
const captureFiles=fs.readdirSync('source-captures').filter(file=>file.endsWith('.json')).sort();
const captures=captureFiles.map(file=>JSON.parse(fs.readFileSync(`source-captures/${file}`,'utf8')));
...
if(capture.intake_complete!==true)errors.push(`Source capture ยังไม่ยืนยัน intake_complete: ${capture.capture_id||'unknown'}`);
