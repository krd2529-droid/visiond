...
assert.match(provider,/posting_plan ต้องเป็นแผนภายใน 1 วัน/);assert.match(provider,/ห้ามใส่สินค้า F ในแผน/);assert.match(provider,/ครบ \$\{clipsPerDay\} คลิป/);assert.match(provider,/max_output_tokens:(?:7000|10000)/);assert.match(provider,/maxOutputTokens:(?:7000|10000)/);
