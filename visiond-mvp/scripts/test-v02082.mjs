...
const classes=new Map();const context={COMMISSION_WORKSPACE_ENABLED:false,pageAuthorized:false,pageViewerId:'',pageViewerRole:'',sessionStorage:{getItem:()=>null,setItem(){}},document:{body:{classList:{toggle:(k,v)=>classes.set(k,v)}}},$:()=>null};
vm.runInNewContext(source.slice(source.indexOf('const CHANNEL_ACTION_SESSION_KEY'),source.indexOf('$("#channelActionSwitch")?.addEventListener'))+';setChannelView("commission");',context);
