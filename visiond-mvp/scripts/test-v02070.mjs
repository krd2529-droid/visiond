  ...
  vm.runInContext(`
    ...
    class FixtureNode {
      ...
      querySelectorAll(selector){
        if(selector==='[data-field="summary"]')return [nodes.summary];
    ...
    function isBossPartnerCommissionView(){return false}
    function resetBossPartnerCommissionController(){}
...
assert.ok(!source.includes('data-load-more-runs'),'retired history controls cannot issue late requests');
...
assert.equal(version.trim(), 'v0.20.114');
assert.match(html, /<b>v0\.20\.114<\/b>/);
assert.match(html, /tiktok-analyzer\.js\?v=02163/);
