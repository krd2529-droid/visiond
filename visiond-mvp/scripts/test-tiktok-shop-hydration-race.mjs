...
assert.match(client, /if \(!channelOwnership\.current\(context\)\) throw new Error\("เปลี่ยนช่องแล้ว กรุณากดค้นหาอีกครั้ง"\)/, "Marketplace must reject a channel switch during search");
assert.match(client, /channel_id: context\.channelId/, "Marketplace payload must use the captured channel id");
