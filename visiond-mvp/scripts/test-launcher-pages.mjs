...
const s={URLSearchParams,document:{getElementById:node},location:{hash:'#id='+id,pathname:'/launcher-pair.html'},history:{replaceState(){}},localStorage:{setItem(){throw Error('storage unavailable')}},window:{close(){closed++}},setTimeout:fn=>timers.push(fn),fetch:async(url,opts)=>{calls.push(url);return{ok:true,json:async()=>url.endsWith('pair-prepare')?{pair_code:'12ABCDEF',confirm_nonce:'n',replace_id:''}:{helper_id:id}}}};
...
assert.deepEqual(calls,['/api/launcher/pair-prepare','/api/launcher/pair-confirm']);
