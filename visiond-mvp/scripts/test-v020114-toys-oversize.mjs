...
assert.match(adminSource,/toys-center-image\.js\?v=020114/);assert.match(adminSource,/ปรับรูป VisionD.*MB →.*MB/);assert.match(adminHtml,/toys-center-admin\.js\?v=020114/);assert.match(adminHtml,/toys-center\.css\?v=020114/);assert.match(adminHtml,/v0\.20\.116/);assert.equal(fs.readFileSync(new URL('../VERSION.txt',import.meta.url),'utf8').trim(),'v0.20.116');

console.log('PASS v0.20.116 bounded VisionD optimizer identity, MIME/magic, aspect/no-upscale, cache, order, stale queue, resource guards and server bypass');
