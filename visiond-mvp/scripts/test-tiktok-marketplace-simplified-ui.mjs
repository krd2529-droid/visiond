...
assert.match(client, /รหัส ['"] \+ escapeHtml\(category\.id\)/);
assert.match(client, /JSON\.stringify\(\{ connection_id: connection\.id, categories_only: true \}\)/);
assert.doesNotMatch(client, /categories_only: true, result_limit: 100/);
assert.match(helper, /category_id: category\.id, category_name: category\.name/);
assert.match(helper, /product\.category_chains \|\| product\.category/);
