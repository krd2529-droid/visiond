...
const shopRequests = [];
const shopFetch = async (_url, options) => { shopRequests.push(JSON.parse(options.body)); return Response.json({ code: 0, data: { products: [{ id: "moodeng-1", title: "MooDeng Baby ชุดเด็ก", shop: { name: "MooDeng Baby" } }], next_page_token: "shop-page-2", total_count: 21 } }); };
const shopSearched = await searchTikTokShopOpenCollaborationProducts(env, connection, { shopKeyword: "moodeng", resultLimit: 100 }, shopFetch);
assert.deepEqual(shopRequests[0].title_keywords, ["moodeng"], "shop keyword must be sent to TikTok instead of filtering the global top-products feed");
assert.equal(shopSearched.products[0].shop_name, "MooDeng Baby");
assert.equal(shopRequests.length, 1, "shop search must make only one TikTok page request per browser request");
assert.equal(shopSearched.next_page_token, "shop-page-2", "shop search must preserve TikTok pagination for the existing next-page button");
...
const helper = fs.readFileSync("functions/_tiktok_shop_api.js", "utf8"), endpoint = fs.readFileSync("functions/api/admin/tiktok-connections/marketplace.js", "utf8"), client = fs.readFileSync("public/tiktok-analyzer.js", "utf8"), schema = fs.readFileSync("functions/_tiktok_analyzer.js", "utf8"), migration = fs.readFileSync("migrations/0078_tiktok_shop_marketplace_snapshots.sql", "utf8");
...
assert.match(client, /shop_keyword:[\s\S]{0,240}result_limit: 20/, "shop search UI must request one TikTok page at a time");
