...
assert.match(source,/export const onRequestHead=ctx=>readMedia\(ctx,\{head:true\}\)/);assert.match(source,/ctx\.env\.FILES\.head\(key\)/);assert.equal(version.trim(),'v0.20.65');assert.match(indexHtml,/WEB v0\.20\.65/);assert.match(adminHtml,/ADMIN v0\.20\.65/);
