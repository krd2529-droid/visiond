...
assert.match(source,/export const onRequestHead=ctx=>readMedia\(ctx,\{head:true\}\)/);assert.match(source,/ctx\.env\.FILES\.head\(key\)/);assert.equal(version.trim(),'v0.20.61');assert.match(indexHtml,/WEB v0\.20\.61/);assert.match(adminHtml,/ADMIN v0\.20\.61/);
console.log('PASS v0.20.61 explicit media HEAD: dispatch, R2 metadata-only body, privacy and GET regression');
