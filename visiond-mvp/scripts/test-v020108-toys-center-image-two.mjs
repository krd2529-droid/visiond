...
const item={id:7,meta_id:'TOY-0007',slug:'toy-seven',title:'หุ่นสะสมรุ่น 7',description:'รายละเอียดจริง',availability:'in stock',condition:'used',price:1850,brand:'Vision Toy',quantity:2,image_1_url:'/api/toys-center/images/7/1',image_2_url:'/api/toys-center/images/7/2',status:'published'};
...
const server=http.createServer((request,response)=>{
  ...
  if(/^\/api\/toys-center\/images\/7\/[12]$/.test(url.pathname)){
    imageHits.set(url.pathname,(imageHits.get(url.pathname)||0)+1);response.setHeader('content-type','image/svg+xml');response.setHeader('cache-control','no-store');response.end(image);return;
  }
...
try{
  ...
  assert.equal(new URL(await list.locator('.store-product img').getAttribute('src'),base).pathname,item.image_2_url,'list image is source slot 2');
  assert.equal(await list.locator('.store-product img').getAttribute('alt'),`${item.title} — รูปสินค้า`,'list alt does not expose a slot number');
  ...
  assert.equal(imageHits.get(item.image_1_url)||0,0,'list must not request slot 1');assert.equal(imageHits.get(item.image_2_url)||0,1,'list requests slot 2 once');

  const detail=await browser.newPage({viewport:{width:1280,height:900}});await detail.goto(`${base}/toyscenter?product=toy-seven`);await detail.waitForFunction(()=>document.querySelector('.store-detail img')?.naturalWidth>0);
  assert.equal(await detail.locator('.store-detail img').count(),1,'detail renders exactly one image');
  assert.equal(new URL(await detail.locator('.store-detail img').getAttribute('src'),base).pathname,item.image_2_url,'detail image is source slot 2');
  assert.equal(await detail.locator('.store-detail figcaption').textContent(),'รูปสินค้า','detail caption is truthful without a slot number');
  ...
  assert.ok(desktop.galleryWidth>400,'single desktop image uses the intended detail width');assert.ok(Math.abs(desktop.galleryWidth-desktop.stageWidth)<2,'single image fills the gallery column');assert.equal(desktop.fits,true);assert.equal(desktop.objectFit,'scale-down');assert.equal(desktop.scrollWidth,desktop.width);
  assert.equal(imageHits.get(item.image_1_url)||0,0,'detail must not request slot 1');assert.equal(imageHits.get(item.image_2_url)||0,2,'fresh list and detail pages each request slot 2');

  const mobile=await browser.newPage({viewport:{width:390,height:844}});await mobile.goto(`${base}/toyscenter?product=toy-seven`);await mobile.waitForFunction(()=>document.querySelector('.store-detail img')?.naturalWidth>0);
  const compact=await mobile.evaluate(()=>{const stage=document.querySelector('.store-image-stage'),img=stage.querySelector('img'),s=stage.getBoundingClientRect(),i=img.getBoundingClientRect();return{imageCount:document.querySelectorAll('.store-detail img').length,fits:i.left>=s.left&&i.right<=s.right&&i.top>=s.top&&i.bottom<=s.bottom,objectFit:getComputedStyle(img).objectFit,scrollWidth:document.documentElement.scrollWidth,width:innerWidth,pagerHidden:document.querySelector('.pager').hidden}});
  assert.deepEqual(compact,{imageCount:1,fits:true,objectFit:'scale-down',scrollWidth:390,width:390,pagerHidden:true});assert.equal(imageHits.get(item.image_1_url)||0,0,'390px detail must not request slot 1');
  ...
  assert.match(html,/toys-center\.css\?v=020108/);assert.match(html,/toyscenter\.js\?v=020108/);assert.match(feed,/mediaUrl\(origin,r\.id,2\)/,'Meta remains on image 2');
  assert.match(adminHtml,/name="image_1"/);assert.match(adminHtml,/name="image_2"/);assert.match(adminHtml,/toys-center\.css\?v=020108/);assert.match(adminHtml,/toys-center-admin\.js\?v=020107/);assert.equal((adminHtml.match(/\.heic,\.heif/g)||[]).length,2,'both admin HEIC slots remain');assert.match(adminJs,/form\.image_1/);assert.match(adminJs,/form\.image_2/);
