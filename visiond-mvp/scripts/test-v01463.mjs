...
check('version',()=>assert.equal(read('VERSION.txt').trim(),'v0.14.63'));
...
check('key center secret readiness',()=>{const api=read('functions/api/admin/vision7/licenses.js'),ui=read('public/vision7-admin.js');assert.match(api,/encryption_ready/);assert.match(api,/VISION7_LICENSE_ENCRYPTION_NOT_CONFIGURED/);assert.match(ui,/newKey\.disabled = !encryptionReady/)});
...
check('new requirements captured atomically',()=>{const source=JSON.parse(read('source-captures/vision7-veasy-key-center-v1.json')),ledger=JSON.parse(read('requirements-ledger.json'));assert.equal(source.items.length,7);for(const item of source.items)assert.ok(ledger.requirements.some(x=>x.id===item.requirement_id));assert.equal(ledger.requirements.find(x=>x.id==='VE-KEYCENTER-001').status,'DONE-VERIFIED');assert.equal(ledger.requirements.find(x=>x.id==='VE-LICENSE-001').status,'PENDING')});
check('migration sequence',()=>{const files=fs.readdirSync('migrations').filter(x=>/^\d{4}_.*\.sql$/.test(x)).sort();assert.equal(files.at(-1).slice(0,4),'0025')});
