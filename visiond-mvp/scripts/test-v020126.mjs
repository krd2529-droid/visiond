...
assert.equal((await text('VERSION.txt')).trim(),'v0.20.126');
assert.match(await text('public/index.html'),/WEB v0\.20\.126/);
assert.match(await text('public/admin.html'),/ADMIN v0\.20\.126/);
assert.match(await text('public/live-center.html'),/live-center\.js\?v=020126/);
assert.match(await text('FEATURE-MAP.md'),/Gemini structured JSON/);
...
console.log('v0.20.126 Live Center AI provider/transport fix and vSport byte-identity checks passed');
