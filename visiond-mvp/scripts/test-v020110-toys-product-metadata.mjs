...
const puts=[];const env={DB:{prepare:sql=>new Bound(sql)},FILES:{put:async(...args)=>puts.push(args),delete:async()=>{}}};
...
assert.equal((await read('VERSION.txt')).trim(),'v0.20.110');assert.match(html,/v0\.20\.110/);assert.match(html,/toys-center-admin\.js\?v=020110/);assert.match(await read('public/toyscenter.html'),/toyscenter\.js\?v=020110/);
sqlite.close();
