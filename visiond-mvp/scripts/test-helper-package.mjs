...
const root='public/downloads/visiond-helper/0.20.73/',manifest=JSON.parse(readFileSync(root+'release.json')),sha=p=>createHash('sha256').update(readFileSync(p)).digest('hex');
...
const setup=readFileSync('tools/browser-launcher/Setup.cs','utf8');assert.match(setup,/LocalHelper\.Pair\(\);status.Text=LocalHelper.PairStatus/);assert.doesNotMatch(setup,/Start\("--pair"\)/);
