...
const rich={id:7,meta_id:'TOY-META-7',slug:'with-metadata',title:'หุ่นสะสมรุ่น 7',description:'รายละเอียดจริง',availability:'in stock',condition:'used',price:1250,brand:'Vision Toy',product_line:'Transformers Generations',series:'Studio Series',quantity:3,image_1_url:'/images/7/1',image_2_url:'/images/7/2',status:'published'};
...
try{
  ...
  assert.deepEqual(await page.locator('.store-detail-facts dt').allTextContents(),['สภาพสินค้า','แบรนด์','ไลน์สินค้า','ซีรีส์','จำนวน']);assert.match(await page.locator('.store-detail-facts').textContent(),/Transformers Generations/);assert.match(await page.locator('.store-detail-facts').textContent(),/Studio Series/);assert.equal(await page.locator('.store-detail-gallery img').count(),1);assert.equal((await page.locator('.store-detail-gallery img').getAttribute('src')),'/images/7/2');assert.equal(hits.filter(hit=>hit.includes('/api/toys-center/products')).length,1);
