...
assert.match(adminSource,/productCategoryOptions\(p\.category \|\| ""\)/);assert.match(adminSource,/categoryCountsGeneration/);assert.doesNotMatch(elonSource,/Math\.random\(\)<0\.02/);assert.match(trashSource,/product_bundle_items b/);assert.equal(version.trim(),'v0.20.69');
