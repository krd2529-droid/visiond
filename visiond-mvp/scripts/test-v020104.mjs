...
const controls=controlContext.controls();for(const label of ['ช่องที่เลือก','รวมทุกช่อง','รายเดือน','วันเดียว','ช่วงวัน','แสดงค่าคอม'])assert.ok(controls.includes(label),label);assert.equal((controls.match(/แสดงค่าคอม/g)||[]).length,1);assert.match(controls,/value="selected" checked/);assert.match(controls,/value="month" checked/);assert.match(controls,/type="month"[^>]+max="2026-08"/);
...
assert.equal(read('VERSION.txt').trim(),'v0.20.104');assert.match(html,/v0\.20\.104/);assert.match(html,/tiktok-analyzer\.js\?v=02162/);assert.match(html,/tiktok-analyzer\.css\?v=02102/);
