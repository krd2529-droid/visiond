...
assert.ok(['v0.20.132', 'v0.20.133', 'v0.20.134', 'v0.20.135'].includes(releasedVersion));
assert.match(await text('public/index.html'), /WEB v0\.20\.(?:132|133|134|135)/);
assert.match(await text('public/admin.html'), /ADMIN v0\.20\.(?:132|133|134|135)/);
assert.match(await text('public/live-center.html'), /live-center\.css\?v=020(?:132|133|134|135)/);
assert.match(await text('public/live-center.html'), /live-center\.js\?v=020(?:132|133|134|135)/);
...
assert.match(openerHtml, /live-center\.css\?v=020(?:132|133|134|135)/);
assert.match(openerHtml, /live-package-open\.js\?v=020(?:132|133|134|135)/);
...
else assert.equal(await sha256('functions/_live_center.js'), '2FD2240E4F413B3BDC57A325D5CAAE8412E1D057EA3DB2A51A5FEED3F047A9D0', 'private host server and D1/provider boundary remain byte-identical');
