...
try{
  ...
  await page.waitForFunction(()=>document.querySelector('#preview1 img')?.naturalWidth>0);
  assert.deepEqual(await page.$eval('#preview1 img',img=>({width:img.naturalWidth,height:img.naturalHeight})),{width:1280,height:854});
  ...
  await page.waitForFunction(()=>document.querySelector('#formStatus').textContent.includes('แปลงรูป VisionD')&&document.querySelector('#formStatus').textContent.includes('ไม่สำเร็จ'));
  await page.click('#aiFillProduct');await page.waitForTimeout(250);assert.equal(requests.length,successfulCount);assert.match(await page.textContent('#formStatus'),/แปลงรูป VisionD.*ไม่สำเร็จ/);
  await page.fill('[name=meta_id]','TOY-BROKEN');await page.fill('[name=title]','สินค้า');await page.fill('[name=description]','รายละเอียด');await page.fill('[name=price]','100');await page.fill('[name=brand]','VisionD');await page.click('#saveProduct');await page.waitForTimeout(250);assert.equal(requests.length,successfulCount);assert.match(await page.textContent('#formStatus'),/แปลงรูป VisionD.*ไม่สำเร็จ/);
