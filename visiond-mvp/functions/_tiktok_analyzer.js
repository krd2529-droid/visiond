...
export async function ensureTikTokAnalyzerSchema(env){
  ...
  for(const sql of["ALTER TABLE tiktok_channel_products ADD COLUMN product_type TEXT NOT NULL DEFAULT 'B'","ALTER TABLE tiktok_channel_products ADD COLUMN source_kind TEXT NOT NULL DEFAULT 'winner'","ALTER TABLE tiktok_channel_products ADD COLUMN customer_gender TEXT NOT NULL DEFAULT ''","ALTER TABLE tiktok_channel_products ADD COLUMN customer_age_range TEXT NOT NULL DEFAULT ''","ALTER TABLE tiktok_channel_products ADD COLUMN inventory_status TEXT NOT NULL DEFAULT 'analyzed'","ALTER TABLE tiktok_channel_products ADD COLUMN decided_at TEXT","ALTER TABLE tiktok_channel_products ADD COLUMN review_started_at TEXT","ALTER TABLE tiktok_channel_products ADD COLUMN next_review_at TEXT","ALTER TABLE tiktok_channel_products ADD COLUMN review_cycle_days INTEGER NOT NULL DEFAULT 0","ALTER TABLE tiktok_channel_products ADD COLUMN review_status TEXT NOT NULL DEFAULT 'not_scheduled'","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN creator_username TEXT NOT NULL DEFAULT ''","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN creator_avatar_url TEXT NOT NULL DEFAULT ''","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN selection_region TEXT NOT NULL DEFAULT ''","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN profile_json TEXT NOT NULL DEFAULT '{}'","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN last_synced_at TEXT","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN last_sync_error TEXT NOT NULL DEFAULT ''","ALTER TABLE tiktok_shop_showcase_products ADD COLUMN product_grade TEXT NOT NULL DEFAULT 'B'"]){try{await env.DB.prepare(sql).run()}catch(error){if(!String(error).toLowerCase().includes('duplicate column'))throw error}}
}
...
const weeklySalesGradePrompt=`เกณฑ์ยอดขาย 7 วันล่าสุดสำหรับเกรดผลงาน A/B/C/F: A=ขายได้อย่างน้อย 7 ชิ้นต่อ 7 วัน ซึ่งเท่ากับเฉลี่ยอย่างน้อยวันละ 1 ชิ้น, B=ขายได้ 4-6 ชิ้นต่อ 7 วัน, C=ขายได้ 1-3 ชิ้นต่อ 7 วัน, F=ขายได้ 0 ชิ้นต่อ 7 วัน ให้ใช้จำนวนออเดอร์หรือจำนวนชิ้นที่มีหลักฐานจริงในช่วง 7 วันล่าสุดเท่านั้น หากช่วงข้อมูลไม่ครบ 7 วันหรืออ่านยอดไม่ได้ ห้ามคาดคะเนเป็นยอดต่อสัปดาห์และให้ระบุ data_gaps เกณฑ์ยอดขายรายสัปดาห์นี้เป็นเกณฑ์ล่าสุดและให้ใช้แทนกฎ C แบบ 72 ชั่วโมงก่อนหน้า เกณฑ์นี้ใช้กับเกรดผลงาน A/B/C/F ส่วน D ยังเป็นสินค้ากระแส ฤดูกาล หรือโปรโมชั่น และ E ยังเป็นสินค้า AI แนะนำซึ่งเป็นเพียงข้อเสนอ`;
...
const reviewSchedulePrompt=`กฎรอบติดตามล่าสุด: ให้แยกรอบตรวจสินค้าเทสออกจากรอบจัดเกรดยอดขายปกติ สินค้า C เป็นสินค้าเทส ให้ตรวจหลักฐานใหม่เมื่อครบ 3 วันนับจากวันที่แนบรูปหรือเริ่มวิเคราะห์สินค้านั้น ส่วนสินค้า A และ B ให้ประเมินผลตามยอดขายย้อนหลัง 7 วันและนัดตรวจใหม่ทุก 7 วัน ในผลสรุปและ homework ให้ระบุว่าสินค้าใดควรตรวจรอบ 3 วันหรือ 7 วัน ห้ามใช้ข้อมูลก่อนถึงวันตรวจมาตัดสินว่าเป็นผลครบช่วง และห้ามเดาวันเริ่มต้นเมื่อไม่มีวันที่แนบหรือวันที่วิเคราะห์`;
