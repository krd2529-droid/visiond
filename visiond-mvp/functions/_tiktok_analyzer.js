...
export async function ensureTikTokAnalyzerSchema(env){
  ...
  await env.DB.batch([env.DB.prepare("UPDATE tiktok_channel_products SET product_type='F' WHERE product_type='E'"),env.DB.prepare("UPDATE tiktok_shop_showcase_products SET product_grade='F' WHERE product_grade='E'")]);
}
...
const dailyProductListPrompt=`กฎรายการสินค้า: daily_product_list เป็นรายชื่อสินค้าพร้อม Ranking สำหรับเตรียมสร้างคลิป ไม่ใช่แผนคลิป ให้สร้างสูงสุด 40 รายการที่ไม่ซ้ำ เรียง ranking_score จากมากไปน้อย และใส่ rank ต่อเนื่องตั้งแต่ 1 แต่ละรายการมี rank, product ชื่อสินค้าเต็ม, product_type เกรด A-D และ F, ranking_score 0-100 และ ranking_reason โดย A=ขายดี วิวดีและทราฟฟิกดี, B=สินค้ารองที่วิวดีและยังขายได้บ้าง, C=สินค้าใหม่หรือสินค้าเทสที่ต้องเฝ้าสังเกต, D=สินค้ากระแส ตามฤดูกาล หรือจัดโปรโมชั่น, F=วิวไม่ดีและขายไม่ได้ควรคัดออก เป้าหมายคือประเมินซ้ำจากข้อมูลแต่ละรอบและคัดสินค้าไปเรื่อย ๆ เพื่อเพิ่มจำนวนสินค้าเกรด A ให้มากที่สุด เลื่อน C เป็น A หรือ B เมื่อมีหลักฐานเพียงพอ ใช้ D ตามจังหวะ และแทนที่ F ด้วยสินค้าใหม่สำหรับทดสอบรอบถัดไป ห้ามเปลี่ยนเกรดเพียงเพื่อเพิ่มจำนวน A โดยไม่มีหลักฐาน ให้เกรด F อยู่ท้ายลิสต์และต้องมีหลักฐานชัด ห้ามใช้คลิปวิวน้อยเพียงคลิปเดียวตัดสิน หากหลักฐานไม่พอให้ ranking_score เป็น null และระบุใน ranking_reason ห้ามแต่งคะแนน ห้ามกำหนดเวลา Hook มุมขาย รูปแบบ CTA หรือเงื่อนไขผ่าน`;
