...
export async function ensureTikTokAnalyzerSchema(env){
  ...
  await env.DB.batch([env.DB.prepare("UPDATE tiktok_channel_products SET product_type='E' WHERE product_type='F'"),env.DB.prepare("UPDATE tiktok_shop_showcase_products SET product_grade='E' WHERE product_grade='F'")]);
}
