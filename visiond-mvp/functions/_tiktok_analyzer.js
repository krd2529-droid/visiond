...
export async function ensureTikTokAnalyzerSchema(env){
  await env.DB.batch([
    ...
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS tiktok_channel_products(
      id TEXT PRIMARY KEY,channel_id TEXT NOT NULL,name TEXT NOT NULL COLLATE NOCASE,name_key TEXT NOT NULL DEFAULT '',product_url TEXT NOT NULL DEFAULT '',
      ...
      inventory_status TEXT NOT NULL DEFAULT 'analyzed',decided_at TEXT,
    ...
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS tiktok_commission_center_snapshots(
      connection_id TEXT NOT NULL,commission_day TEXT NOT NULL,currency TEXT NOT NULL DEFAULT 'THB',amount REAL NOT NULL,status TEXT NOT NULL DEFAULT 'actual',captured_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY(connection_id,commission_day,currency),FOREIGN KEY(connection_id) REFERENCES tiktok_shop_creator_connections(id) ON DELETE CASCADE)`),
    ...
    env.DB.prepare('CREATE INDEX IF NOT EXISTS idx_tiktok_commission_center_day ON tiktok_commission_center_snapshots(connection_id,commission_day DESC)'),
  ...
  for(const sql of["ALTER TABLE tiktok_channel_products ADD COLUMN product_type TEXT NOT NULL DEFAULT 'B'","ALTER TABLE tiktok_channel_products ADD COLUMN source_kind TEXT NOT NULL DEFAULT 'winner'","ALTER TABLE tiktok_channel_products ADD COLUMN customer_gender TEXT NOT NULL DEFAULT ''","ALTER TABLE tiktok_channel_products ADD COLUMN customer_age_range TEXT NOT NULL DEFAULT ''","ALTER TABLE tiktok_channel_products ADD COLUMN inventory_status TEXT NOT NULL DEFAULT 'analyzed'","ALTER TABLE tiktok_channel_products ADD COLUMN decided_at TEXT","ALTER TABLE tiktok_channel_products ADD COLUMN review_started_at TEXT","ALTER TABLE tiktok_channel_products ADD COLUMN next_review_at TEXT","ALTER TABLE tiktok_channel_products ADD COLUMN review_cycle_days INTEGER NOT NULL DEFAULT 0","ALTER TABLE tiktok_channel_products ADD COLUMN review_status TEXT NOT NULL DEFAULT 'not_scheduled'","ALTER TABLE tiktok_channel_products ADD COLUMN name_key TEXT NOT NULL DEFAULT ''","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN creator_username TEXT NOT NULL DEFAULT ''","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN creator_avatar_url TEXT NOT NULL DEFAULT ''","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN selection_region TEXT NOT NULL DEFAULT ''","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN profile_json TEXT NOT NULL DEFAULT '{}'","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN last_synced_at TEXT","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN last_sync_error TEXT NOT NULL DEFAULT ''","ALTER TABLE tiktok_shop_showcase_products ADD COLUMN product_grade TEXT NOT NULL DEFAULT 'B'"]){try{await env.DB.prepare(sql).run()}catch(error){if(!String(error).toLowerCase().includes('duplicate column'))throw error}}
  await env.DB.prepare('CREATE INDEX IF NOT EXISTS idx_tiktok_products_channel_name_key ON tiktok_channel_products(channel_id,name_key)').run();
  await env.DB.prepare('CREATE INDEX IF NOT EXISTS idx_tiktok_products_empty_name_key ON tiktok_channel_products(name_key) WHERE name_key=\'\'').run();
  const missingKeys=(await env.DB.prepare("SELECT id,name FROM tiktok_channel_products WHERE name_key='' LIMIT 24").all()).results||[];
  if(missingKeys.length)await env.DB.batch(missingKeys.map(row=>env.DB.prepare('UPDATE tiktok_channel_products SET name_key=? WHERE id=?').bind(productKey(row.name),row.id)));
  for(const sql of["ALTER TABLE tiktok_shop_creator_connections ADD COLUMN collector_session_secret_name TEXT NOT NULL DEFAULT ''","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN collector_status TEXT NOT NULL DEFAULT 'not_connected'","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN collector_last_attempt_at TEXT","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN collector_last_success_at TEXT","ALTER TABLE tiktok_shop_creator_connections ADD COLUMN collector_last_error TEXT NOT NULL DEFAULT ''"]){try{await env.DB.prepare(sql).run()}catch(error){if(!String(error).toLowerCase().includes('duplicate column'))throw error}}
}
...
const weeklySalesGradePrompt=`ห้ามใช้เกณฑ์ยอดขาย 3 วันหรือ 7 วันจัด A/B/C ให้ใช้ยอดขายจริงย้อนหลัง 30 วันเท่านั้น: A ตั้งแต่ 30 ชิ้น, B 16-29 ชิ้น, C 1-15 ชิ้น, และยอด 0 หรือข้อมูลไม่ครบเป็นไม่มีเกรด`;
const monthlyAttachmentGradePrompt=`สำหรับข้อมูลยอดขายย้อนหลัง 30 วัน ให้จัด A เมื่อขายได้อย่างน้อย 30 ชิ้น, B เมื่อขายได้ 16-29 ชิ้น, C เมื่อขายได้ 1-15 ชิ้น และยอด 0 เป็นไม่มีเกรด สินค้าที่ไม่ปรากฏในข้อมูลห้ามสรุปว่าเป็นยอด 0 หรือ F เพราะรายการอาจแสดงเฉพาะสินค้าที่ขายได้ F ต้องเกิดจากผู้ใช้คัดออกเท่านั้น`;
const finalGradeMeaningPrompt=`กฎความหมายเกรดล่าสุดซึ่งใช้แทนข้อความที่ขัดกันทั้งหมด: A/B/C คำนวณจากยอดจริง 30 วันเท่านั้น; D คือสินค้าที่ผู้ใช้คัดเข้าลิสต์เพื่อทดสอบ; E คือสินค้ากระแสหรือสินค้าแนะนำจาก AI; F คือสินค้าที่ผู้ใช้คัดออกหรือกดไม่ผ่านเท่านั้น ห้าม AI กำหนด F จากยอด 0 วิวน้อย คะแนนต่ำ หรือข้อมูลไม่พอ และห้ามคำนวณ A/B/C จากยอด 3 วันหรือ 7 วัน`;
const unifiedMonthlyGradePrompt=`กฎเกรดฉบับล่าสุด ให้ใช้แทนกฎรายสัปดาห์และข้อความที่ขัดกันทั้งหมด: ใช้ยอดขายจริงย้อนหลัง 30 วันเท่านั้น A=อย่างน้อย 30 ชิ้นต่อเดือน เป็นสินค้าหลัก, B=16-29 ชิ้นต่อเดือน เป็นสินค้ารอง, C=1-15 ชิ้นต่อเดือน เป็นสินค้าที่ขายได้เล็กน้อย, ยอด 0 หรือไม่มีข้อมูล=ไม่มีเกรด, D=สินค้าที่ผู้ใช้คัดเข้าลิสต์เพื่อทดสอบ, E=สินค้ากระแสหรือสินค้าแนะนำที่ AI วิเคราะห์เสนอและยังไม่มียอดจริงยืนยัน, F=สินค้าที่ผู้ใช้คัดออกเท่านั้น ห้าม AI กำหนด F เอง`;
const selectionRatioPrompt=`สัดส่วนเป้าหมายของ daily_product_list คือ 30 สินค้าหลัก + 10 สินค้าแนะนำเกรด E โดย 30 สินค้าหลักให้ใช้เฉพาะ A/B/C: ใส่ A ได้สูงสุด 20 รายการ, กัน C ไว้ 5 รายการ, และใช้ B เติมจำนวนที่เหลือให้ครบ 30 ดังนั้นเมื่อ A มีตั้งแต่ 20 ไล่ลงถึง 1 จำนวน B จะเป็น 5 ไล่ขึ้นถึง 24 และ C เท่ากับ 5 เสมอ หากยังไม่มีทั้งสินค้า A และ B ให้ใช้ C ทั้ง 30 รายการ จากนั้นวาง E 10 รายการเป็นอันดับ 31-40 สินค้า C สำหรับทดลองอาจคัดมาจากสินค้า C เดิม, สินค้า D ที่ถึงจังหวะกระแส ฤดูกาล หรือโปรโมชั่น, หรือสินค้า E ที่ผู้ใช้เลือกนำมาทดลอง เมื่อดึง D หรือ E เข้ากลุ่มทดลอง ต้องเปลี่ยน product_type ของรายการนั้นเป็น C และห้ามให้สินค้าเดียวกันซ้ำอยู่ใน D หรือ E; หากดึง E มาเป็น C ต้องหาสินค้า E แนะนำอื่นที่ไม่ซ้ำเติมให้กลุ่ม E ยังคงครบ 10 รายการ ห้ามนำ D หรือ F เข้า 30 สินค้าหลักโดยยังคงเกรดเดิม และห้ามสร้างชื่อสินค้าซ้ำหรือชื่อปลอมเพื่อให้ครบสัดส่วน หากสินค้าในแต่ละเกรดมีไม่พอ ให้ใช้สินค้าจริงที่เหมาะสำหรับทดลองเป็น C เติมก่อนและระบุจำนวนที่ขาดใน data_gaps`;
const reviewSchedulePrompt=`กฎรอบติดตามล่าสุด: D คือสินค้าทดสอบ ให้ตรวจรอบทดลองเมื่อครบ 3 วัน ส่วน A/B/C ต้องประเมินจากยอดขายย้อนหลังครบ 30 วันและนัดประเมินใหม่ทุก 30 วัน ห้ามใช้ข้อมูล 3 วันหรือ 7 วันคำนวณ A/B/C`;
const reviewSourcePrompt=`แหล่งข้อมูลสำหรับรอบตรวจ: เมื่อมีข้อมูล TikTok หรือ TikTok Shop API ให้ใช้ข้อมูล API เป็นหลักและไม่บังคับผู้ใช้แนบรูปใหม่ รูปแนบเป็นทางสำรองก่อน API ได้รับอนุญาต หรือเป็นหลักฐานเสริมเมื่อ API ขาดฟิลด์ที่จำเป็น หากใช้ API ให้ระบุว่าเป็นการตรวจอัตโนมัติจาก API; หาก API ยังไม่พร้อมจึงค่อยแจ้งให้แนบรูปสำหรับรอบตรวจ`;
...
export async function analyzeTikTok(provider,{channel,notes,candidates,strategy,dateRange,lookbackDays,attachmentPeriodDays=30,clipsPerDay=40,images},fetchImpl=fetch){
  ...
  try{
    ...
    result.attachment_period_days=Number(attachmentPeriodDays)||30;
    if(images.length){
      const period=Number(attachmentPeriodDays)||30,key=value=>clean(value).toLocaleLowerCase().replace(/\s+/g,' '),number=value=>{const parsed=Number(String(value??'').replace(/,/g,''));return Number.isFinite(parsed)?parsed:null},metrics=new Map((Array.isArray(result.extracted_metrics)?result.extracted_metrics:[]).map(item=>[key(item?.product),number(item?.orders)]).filter(([,orders])=>orders!==null)),grade=orders=>period===3?(orders>0?'C':''):period===7?(orders>=7?'A':orders>=4?'B':orders>=1?'C':''):(orders>=30?'A':orders>=16?'B':orders>=1?'C':''),monthlyOrders=item=>{const direct=metrics.get(key(item?.name||item?.product));if(direct!==undefined)return direct;const evidence=[item?.ranking_reason,item?.evidence,item?.decision,Array.isArray(item?.reasons)?item.reasons.join(' '):''].filter(Boolean).join(' '),after=evidence.match(/(?:ยอดขาย|ขาย(?:ได้|ดี)?)\s*(\d[\d,]*)\s*ชิ้น(?=[^\n]{0,30}30\s*วัน)/i),before=evidence.match(/(?:ยอดขาย|ขาย(?:ได้|ดี)?)[^\n]{0,20}30\s*วัน[^\d\n]{0,20}(\d[\d,]*)\s*ชิ้น/i),loose=evidence.match(/(?:ยอดขาย|ขาย(?:ได้|ดี)?)\s*(\d[\d,]*)\s*ชิ้น/i);return number(after?.[1]??before?.[1]??loose?.[1])},apply=item=>{const orders=monthlyOrders(item);if(orders===null||orders===undefined||['D','E'].includes(String(item?.product_type||'').toUpperCase()))return;item.product_type=grade(orders);if('ranking_reason'in item)item.ranking_reason=orders>0?`ยอดขาย ${period} วัน ${orders} ชิ้น · เกณฑ์ ${item.product_type}`:`ยอดขาย ${period} วัน 0 ชิ้น · ไม่มีเกรด`};
      for(const list of[result.winner_products,result.next_product_candidates,result.avoid_products,result.daily_product_list])if(Array.isArray(list))list.forEach(apply);
      const correctTextGrade=item=>{if(!item||['D','E'].includes(String(item.product_type||'').toUpperCase()))return;const evidence=[item.ranking_reason,item.evidence,item.decision,Array.isArray(item.reasons)?item.reasons.join(' '):''].filter(Boolean).join(' '),after=evidence.match(/(?:ยอดขาย|ขาย(?:ได้|ดี)?)[ \t]*([0-9][0-9,]*)[ \t]*ชิ้น(?=[^\r\n]{0,30}30[ \t]*วัน)/i),before=evidence.match(/(?:ยอดขาย|ขาย(?:ได้|ดี)?)[^\r\n]{0,20}30[ \t]*วัน[^0-9\r\n]{0,20}([0-9][0-9,]*)[ \t]*ชิ้น/i),loose=evidence.match(/(?:ยอดขาย|ขาย(?:ได้|ดี)?)[ \t]*([0-9][0-9,]*)[ \t]*ชิ้น/i),orders=number(after?.[1]??before?.[1]??loose?.[1]);if(orders===null)return;item.product_type=grade(orders);if('ranking_reason'in item)item.ranking_reason=orders>0?`ยอดขาย ${period} วัน ${orders} ชิ้น · เกณฑ์ ${item.product_type}`:`ยอดขาย ${period} วัน 0 ชิ้น · ไม่มีเกรด`};for(const list of[result.winner_products,result.next_product_candidates,result.avoid_products,result.daily_product_list])if(Array.isArray(list))list.forEach(correctTextGrade);
      if(period!==30){
        for(const list of[result.winner_products,result.next_product_candidates,result.avoid_products,result.daily_product_list])if(Array.isArray(list))for(const item of list){const type=String(item?.product_type||'').toUpperCase();if(['A','B','C'].includes(type))item.product_type=''}
