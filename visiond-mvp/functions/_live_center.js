...
function deleteShowPayload(body){
  exactKeys(body,new Set(['owner_id','title','expected_revision']),'show_delete');
  const ownerId=positiveInt(body.owner_id),expectedRevision=positiveInt(body.expected_revision),title=cleanLine(body.title,160);
  if(!ownerId)throw new LiveInputError('owner_id ไม่ถูกต้อง');
  if(!expectedRevision)throw new LiveInputError('expected_revision ไม่ถูกต้อง');
  return{owner_id:ownerId,title,expected_revision:expectedRevision};
}
...
function sceneStatements(env,showId,scenes,products,{gateToken='',gateRevision=0}={}){
  return scenes.map((scene,position)=>{
    ...
    return env.DB.prepare('INSERT INTO live_show_scenes(id,show_id,position,product_id,product_meta_id_snapshot,product_title_snapshot,product_price_snapshot,product_currency_snapshot,product_stock_snapshot,product_cover_image_id_snapshot,product_cover_position_snapshot,product_cover_key_snapshot,product_cover_mime_snapshot,product_cover_size_snapshot,product_cover_etag_snapshot,script,cue_label,cue_duration_seconds,cue_transition) SELECT ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? WHERE EXISTS(SELECT 1 FROM live_shows WHERE id=? AND revision=? AND last_mutation_key=? AND deleted_at IS NULL)').bind(...values,showId,gateRevision,gateToken);
  });
}
async function detail(env,id){
  const show=await env.DB.prepare('SELECT id,title,description,avatar_preset,output_profile,scene_count,revision,created_by,updated_by,created_at,updated_at FROM live_shows WHERE id=? AND deleted_at IS NULL').bind(id).first();if(!show)return null;
...
export async function listLiveShows(ctx){
  const auth=await liveAdmin(ctx);if(auth.error)return auth.error;
  try{const url=new URL(ctx.request.url),limit=parseLimit(url.searchParams);if(!limit)return liveJson({error:'limit ไม่ถูกต้อง'},400);const raw=url.searchParams.get('cursor'),cursor=showCursor(raw);if(raw&&!cursor)return liveJson({error:'cursor ไม่ถูกต้อง'},400);const anchor=cursor?[cursor.at,cursor.id]:['9999-12-31T23:59:59.999Z','~'],args=[anchor[0],anchor[1],limit+1],rows=(await ctx.env.DB.prepare('SELECT id,title,description,avatar_preset,output_profile,scene_count,revision,created_by owner_id,created_at,updated_at FROM live_shows INDEXED BY idx_live_shows_updated WHERE deleted_at IS NULL AND (updated_at,id)<(?,?) ORDER BY updated_at DESC,id DESC LIMIT ?').bind(...args).all()).results||[],hasMore=rows.length>limit,page=rows.slice(0,limit),last=page.at(-1);return liveJson({viewer_id:auth.user.id,items:page.map(row=>({...row,owner_id:Number(row.owner_id),scene_count:Number(row.scene_count),revision:Number(row.revision)})),pagination:{limit,has_more:hasMore,next_cursor:hasMore&&last?encodeLiveCursor([last.updated_at,last.id]):null}})}catch(error){return inputResponse(error)||serverFailure(error)}
}

export async function createLiveShow(ctx){
  const auth=await liveAdmin(ctx);if(auth.error)return auth.error;
  try{
    const key=idempotencyKey(ctx.request),payload=showPayload(await bodyJson(ctx.request)),requestHash=await liveSha256(canonicalLiveJson(payload)),existing=await ctx.env.DB.prepare('SELECT id,create_request_hash,deleted_at FROM live_shows WHERE created_by=? AND create_idempotency_key=?').bind(auth.user.id,key).first();
    if(existing){if(existing.create_request_hash!==requestHash)return liveJson({error:'Idempotency-Key นี้ถูกใช้กับข้อมูลอื่นแล้ว',code:'LIVE_IDEMPOTENCY_CONFLICT'},409);if(existing.deleted_at)return liveJson({error:'รายการจากคำขอนี้ถูกลบแล้ว กรุณาสร้างรายการใหม่',code:'LIVE_SHOW_DELETED'},409);return liveJson({viewer_id:auth.user.id,ok:true,replayed:true,item:await detail(ctx.env,existing.id)},200)}
    ...
    try{await ctx.env.DB.batch(statements)}catch(error){if(!uniqueError(error))throw error;const raced=await ctx.env.DB.prepare('SELECT id,create_request_hash,deleted_at FROM live_shows WHERE created_by=? AND create_idempotency_key=?').bind(auth.user.id,key).first();if(!raced||raced.create_request_hash!==requestHash)return liveJson({error:'Idempotency-Key นี้ถูกใช้กับข้อมูลอื่นแล้ว',code:'LIVE_IDEMPOTENCY_CONFLICT'},409);if(raced.deleted_at)return liveJson({error:'รายการจากคำขอนี้ถูกลบแล้ว กรุณาสร้างรายการใหม่',code:'LIVE_SHOW_DELETED'},409);return liveJson({viewer_id:auth.user.id,ok:true,replayed:true,item:await detail(ctx.env,raced.id)},200)}
...
export async function updateLiveShow(ctx){
  const auth=await liveAdmin(ctx);if(auth.error)return auth.error;
  try{
    const id=routeId(ctx.params.id,SHOW_ID,'show id'),payload=showPayload(await bodyJson(ctx.request),{updating:true}),current=await ctx.env.DB.prepare('SELECT revision FROM live_shows WHERE id=? AND deleted_at IS NULL').bind(id).first();if(!current)return liveJson({error:'ไม่พบรายการไลฟ์'},404);if(Number(current.revision)!==payload.expected_revision)return liveJson({error:'รายการนี้ถูกแก้จากอีกหน้าต่าง กรุณาโหลดใหม่',code:'LIVE_STALE_REVISION',current_revision:Number(current.revision)},409);const products=await snapshotSceneProducts(ctx,payload.scenes),token=`mut_${crypto.randomUUID().replaceAll('-','')}`,nextRevision=payload.expected_revision+1,now=new Date().toISOString(),statements=[ctx.env.DB.prepare('UPDATE live_shows SET title=?,description=?,avatar_preset=?,output_profile=?,scene_count=?,revision=revision+1,last_mutation_key=?,updated_by=?,updated_at=? WHERE id=? AND revision=? AND deleted_at IS NULL').bind(payload.title,payload.description,payload.avatar_preset,payload.output_profile,payload.scenes.length,token,auth.user.id,now,id,payload.expected_revision),ctx.env.DB.prepare('DELETE FROM live_show_scenes WHERE show_id=? AND EXISTS(SELECT 1 FROM live_shows WHERE id=? AND revision=? AND last_mutation_key=? AND deleted_at IS NULL)').bind(id,id,nextRevision,token),...sceneStatements(ctx.env,id,payload.scenes,products,{gateToken:token,gateRevision:nextRevision})],results=await ctx.env.DB.batch(statements);
    if(!Number(results[0]?.meta?.changes)){const current=await ctx.env.DB.prepare('SELECT revision FROM live_shows WHERE id=? AND deleted_at IS NULL').bind(id).first();return current?liveJson({error:'รายการนี้ถูกแก้จากอีกหน้าต่าง กรุณาโหลดใหม่',code:'LIVE_STALE_REVISION',current_revision:Number(current.revision)},409):liveJson({error:'ไม่พบรายการไลฟ์'},404)}
    return liveJson({viewer_id:auth.user.id,ok:true,item:await detail(ctx.env,id)});
  }catch(error){return inputResponse(error)||serverFailure(error)}
}

const liveShowCleanupRetryAt=attempts=>new Date(Date.now()+Math.min(60,2**Math.min(6,attempts))*60_000).toISOString();
async function cleanupDeletedShowPresenters(ctx,{showId,ownerId,batches=4,limit=24}){
  if(!ctx.env.FILES||typeof ctx.env.FILES.delete!=='function'||typeof ctx.env.FILES.head!=='function')return;
  for(let batch=0;batch<batches;batch++){
    const now=new Date().toISOString(),rows=(await ctx.env.DB.prepare(`SELECT id,owner_id,show_id,object_key,reason,status,attempts
      FROM live_portrait_object_cleanup_jobs INDEXED BY idx_live_portrait_cleanup_due
      WHERE owner_id=? AND show_id=? AND status IN ('reserved','pending','error')
        AND (next_attempt_at IS NULL OR next_attempt_at<=?)
      ORDER BY next_attempt_at ASC,id ASC LIMIT ?`).bind(ownerId,showId,now,Math.min(24,Math.max(1,limit))).all()).results||[];
    if(!rows.length)return;
    await Promise.all(rows.map(async row=>{
      try{
        if(row.reason==='orphan_guard'&&!await ctx.env.FILES.head(row.object_key)){
          const attempts=Math.min(8,Number(row.attempts||0)+1),pendingAt=new Date().toISOString();
          await ctx.env.DB.prepare(`UPDATE live_portrait_object_cleanup_jobs
            SET status='pending',attempts=?,next_attempt_at=?,last_error_code='R2_OBJECT_NOT_READY',updated_at=?
            WHERE id=? AND owner_id=? AND show_id=? AND object_key=? AND reason='orphan_guard' AND status IN ('reserved','pending','error')`).bind(attempts,liveShowCleanupRetryAt(attempts),pendingAt,row.id,ownerId,showId,row.object_key).run();
          return;
        }
        await ctx.env.FILES.delete(row.object_key);
        if(await ctx.env.FILES.head(row.object_key))throw new Error('R2_OBJECT_STILL_PRESENT');
        const completedAt=new Date().toISOString();
        await ctx.env.DB.prepare(`UPDATE live_portrait_object_cleanup_jobs
          SET status='done',attempts=MIN(attempts+1,8),next_attempt_at=NULL,last_error_code='',updated_at=?,completed_at=?
          WHERE id=? AND owner_id=? AND show_id=? AND object_key=?`).bind(completedAt,completedAt,row.id,ownerId,showId,row.object_key).run();
      }catch{
        const attempts=Math.min(8,Number(row.attempts||0)+1),failedAt=new Date().toISOString();
        await ctx.env.DB.prepare(`UPDATE live_portrait_object_cleanup_jobs
          SET status='error',attempts=?,next_attempt_at=?,last_error_code='R2_DELETE_FAILED',updated_at=?
          WHERE id=? AND owner_id=? AND show_id=? AND object_key=?`).bind(attempts,liveShowCleanupRetryAt(attempts),failedAt,row.id,ownerId,showId,row.object_key).run().catch(()=>undefined);
      }
    }));
    if(rows.length<limit)return;
  }
}
async function deletedShowResponse(ctx,auth,target,{replayed=false}={}){
  const pending=await ctx.env.DB.prepare(`SELECT id FROM live_portrait_object_cleanup_jobs INDEXED BY idx_live_portrait_cleanup_due
    WHERE owner_id=? AND show_id=? AND status IN ('reserved','pending','error') LIMIT 1`).bind(Number(target.created_by),target.id).first();
  if(pending&&typeof ctx.waitUntil==='function')ctx.waitUntil(cleanupDeletedShowPresenters(ctx,{showId:target.id,ownerId:Number(target.created_by)}).catch(error=>console.error('LIVE_SHOW_PRESENTER_CLEANUP_FAILED',target.id,String(error?.message||error).slice(0,160))));
  return liveJson({viewer_id:auth.user.id,ok:true,deleted:true,replayed,deleted_id:target.id,cleanup_pending:Boolean(pending)});
}
export async function deleteLiveShow(ctx){
  const auth=await liveAdmin(ctx);if(auth.error)return auth.error;
  try{
    const id=routeId(ctx.params.id,SHOW_ID,'show id'),key=idempotencyKey(ctx.request),payload=deleteShowPayload(await bodyJson(ctx.request,4000)),requestHash=await liveSha256(canonicalLiveJson({show_id:id,...payload}));
    const keyReplay=await ctx.env.DB.prepare('SELECT id,created_by,delete_request_hash,deleted_at FROM live_shows WHERE deleted_by=? AND delete_idempotency_key=?').bind(auth.user.id,key).first();
    if(keyReplay&&(keyReplay.id!==id||keyReplay.delete_request_hash!==requestHash))return liveJson({error:'Idempotency-Key นี้ถูกใช้กับรายการอื่นแล้ว',code:'LIVE_IDEMPOTENCY_CONFLICT'},409);
    if(keyReplay?.deleted_at)return deletedShowResponse(ctx,auth,keyReplay,{replayed:true});
    const target=await ctx.env.DB.prepare('SELECT id,title,revision,created_by,deleted_at,delete_idempotency_key,delete_request_hash FROM live_shows INDEXED BY idx_live_shows_owner_id WHERE created_by=? AND id=?').bind(payload.owner_id,id).first();
    if(!target)return liveJson({error:'ไม่พบรายการไลฟ์'},404);
    if(target.deleted_at){
      if(target.delete_request_hash===requestHash)return deletedShowResponse(ctx,auth,target,{replayed:true});
      return liveJson({error:'รายการนี้ถูกลบด้วยคำขออื่นแล้ว',code:'LIVE_SHOW_ALREADY_DELETED'},409);
    }
    if(target.title!==payload.title||Number(target.revision)!==payload.expected_revision)return liveJson({error:'ชื่อหรือ revision ของรายการเปลี่ยนแล้ว กรุณาโหลดใหม่ก่อนลบ',code:'LIVE_STALE_DELETE',current_revision:Number(target.revision)},409);
    const now=new Date().toISOString(),token=`delete_${crypto.randomUUID().replaceAll('-','')}`,guard='EXISTS(SELECT 1 FROM live_shows WHERE id=? AND created_by=? AND deleted_at=? AND delete_idempotency_key=? AND delete_request_hash=?)',guardArgs=[id,payload.owner_id,now,key,requestHash];
    const statements=[
      ctx.env.DB.prepare(`UPDATE live_shows SET scene_count=0,revision=revision+1,last_mutation_key=?,updated_by=?,updated_at=?,deleted_at=?,deleted_by=?,delete_idempotency_key=?,delete_request_hash=?
        WHERE id=? AND created_by=? AND deleted_at IS NULL AND revision=? AND title=?`).bind(token,auth.user.id,now,now,auth.user.id,key,requestHash,id,payload.owner_id,payload.expected_revision,payload.title),
      ctx.env.DB.prepare(`UPDATE live_portrait_object_cleanup_jobs SET reason=CASE WHEN reason='orphan_guard' THEN reason ELSE 'deleted' END,status=CASE WHEN status='done' THEN 'done' ELSE 'pending' END,
        attempts=CASE WHEN status='done' THEN attempts WHEN reason='orphan_guard' AND status='reserved' AND next_attempt_at>? THEN attempts ELSE 0 END,
        next_attempt_at=CASE WHEN status='done' THEN NULL WHEN reason='orphan_guard' AND status='reserved' AND next_attempt_at>? THEN next_attempt_at ELSE ? END,
        last_error_code=CASE WHEN status='done' THEN last_error_code ELSE '' END,updated_at=?
        WHERE owner_id=? AND show_id=? AND ${guard}`).bind(now,now,now,now,payload.owner_id,id,...guardArgs),
      ctx.env.DB.prepare(`INSERT OR IGNORE INTO live_portrait_object_cleanup_jobs(id,owner_id,show_id,presenter_asset_id,object_key,reason,status,idempotency_key,attempts,next_attempt_at,last_error_code,created_at,updated_at)
        SELECT 'liveoc_'||lower(hex(randomblob(16))),p.owner_id,p.show_id,p.id,p.object_key,'deleted','pending','delete_show_'||p.id,0,?,'',?,?
        FROM live_presenter_assets p WHERE p.owner_id=? AND p.show_id=? AND ${guard}`).bind(now,now,now,payload.owner_id,id,...guardArgs),
      ctx.env.DB.prepare(`DELETE FROM live_audience_events WHERE owner_id=? AND show_id=? AND ${guard}`).bind(payload.owner_id,id,...guardArgs),
      ctx.env.DB.prepare(`UPDATE live_runtime_sessions SET status='stopped',session_epoch=session_epoch+1,updated_at=?,stopped_at=COALESCE(stopped_at,?)
        WHERE owner_id=? AND show_id=? AND status IN ('starting','active') AND ${guard}`).bind(now,now,payload.owner_id,id,...guardArgs),
      ctx.env.DB.prepare(`UPDATE live_provider_resources SET status='delete_pending',cleanup_idempotency_key=COALESCE(cleanup_idempotency_key,'delete_show_'||id),next_cleanup_at=?,updated_at=?
        WHERE owner_id=? AND show_id=? AND status IN ('creating','active','error') AND ${guard}`).bind(now,now,payload.owner_id,id,...guardArgs),
      ctx.env.DB.prepare(`DELETE FROM live_runtime_sessions WHERE owner_id=? AND show_id=? AND mode='local_test' AND ${guard}`).bind(payload.owner_id,id,...guardArgs),
      ctx.env.DB.prepare(`UPDATE live_presenter_bindings SET presenter_asset_id=NULL,binding_revision=binding_revision+1,updated_by=?,updated_at=?
        WHERE owner_id=? AND show_id=? AND ${guard}`).bind(auth.user.id,now,payload.owner_id,id,...guardArgs),
      ctx.env.DB.prepare(`UPDATE live_presenter_assets SET status='deleted',deleted_at=COALESCE(deleted_at,?),deleted_by=COALESCE(deleted_by,?),
        delete_idempotency_key=COALESCE(delete_idempotency_key,?),delete_request_hash=COALESCE(delete_request_hash,?),updated_at=?
        WHERE owner_id=? AND show_id=? AND status<>'deleted' AND ${guard}`).bind(now,auth.user.id,key,requestHash,now,payload.owner_id,id,...guardArgs),
      ctx.env.DB.prepare(`UPDATE live_portrait_upload_claims SET status='error',lease_token=NULL,lease_expires_at=NULL,last_error_code='LIVE_SHOW_DELETED',updated_at=?
        WHERE owner_id=? AND show_id=? AND status='processing' AND ${guard}`).bind(now,payload.owner_id,id,...guardArgs),
      ctx.env.DB.prepare(`DELETE FROM live_show_scenes WHERE show_id=? AND ${guard}`).bind(id,...guardArgs),
    ];
    let results;
    try{results=await ctx.env.DB.batch(statements)}catch(error){
      const committed=await ctx.env.DB.prepare('SELECT id,created_by,deleted_at,delete_idempotency_key,delete_request_hash FROM live_shows INDEXED BY idx_live_shows_owner_id WHERE created_by=? AND id=?').bind(payload.owner_id,id).first().catch(()=>null);
      if(committed?.deleted_at&&committed.delete_request_hash===requestHash)return deletedShowResponse(ctx,auth,committed,{replayed:true});
      throw error;
    }
    if(!Number(results[0]?.meta?.changes)){
      const current=await ctx.env.DB.prepare('SELECT id,title,revision,created_by,deleted_at,delete_idempotency_key,delete_request_hash FROM live_shows INDEXED BY idx_live_shows_owner_id WHERE created_by=? AND id=?').bind(payload.owner_id,id).first();
      if(current?.deleted_at&&current.delete_request_hash===requestHash)return deletedShowResponse(ctx,auth,current,{replayed:true});
      return current?liveJson({error:'รายการนี้ถูกแก้จากอีกหน้าต่าง กรุณาโหลดใหม่',code:'LIVE_STALE_DELETE',current_revision:Number(current.revision)},409):liveJson({error:'ไม่พบรายการไลฟ์'},404);
    }
    return deletedShowResponse(ctx,auth,{...target,deleted_at:now,delete_idempotency_key:key,delete_request_hash:requestHash});
  }catch(error){return inputResponse(error)||serverFailure(error)}
}
...
async function versionGateState(env,showId){
  return env.DB.prepare(`SELECT s.revision,s.scene_count,(SELECT COUNT(*) FROM live_show_scenes sc JOIN toys_center_products p ON p.id=sc.product_id WHERE sc.show_id=s.id AND p.status='published' AND p.availability='in stock' AND p.quantity>0) eligible_count,(SELECT COUNT(*) FROM live_show_scenes sc JOIN toys_center_products p ON p.id=sc.product_id WHERE sc.show_id=s.id AND ${LIVE_SCENE_SNAPSHOT_MATCH}) snapshot_match_count FROM live_shows s WHERE s.id=? AND s.deleted_at IS NULL`).bind(showId).first();
}
...
export async function createLiveVersion(ctx){
  ...
  try{
    const showId=routeId(ctx.params.id,SHOW_ID,'show id'),key=idempotencyKey(ctx.request),payload=versionPayload(await bodyJson(ctx.request,20000)),requestHash=await liveSha256(canonicalLiveJson({show_id:showId,...payload})),show=await ctx.env.DB.prepare('SELECT id,title,description,avatar_preset,output_profile,scene_count,revision FROM live_shows WHERE id=? AND deleted_at IS NULL').bind(showId).first();
    if(!show)return liveJson({error:'ไม่พบรายการไลฟ์'},404);const replay=await existingVersion(ctx.env,showId,key);
    ...
    if(Number(show.revision)!==payload.expected_revision)return liveJson({error:'รายการถูกแก้หลังหน้าจอนี้ กรุณาโหลดใหม่',code:'LIVE_STALE_REVISION',current_revision:Number(show.revision)},409);
    ...
    for(let attempt=0;attempt<4;attempt++){
      ...
      try{
        const statements=[ctx.env.DB.prepare(`INSERT INTO live_show_versions(id,show_id,version_number,show_revision,schema_version,idempotency_key,request_hash,package_object_key,package_sha256,manifest_sha256,package_size,asset_count,created_by,created_at) SELECT ?,?,?,?,?,?,?,?,?,?,?,?,?,? FROM live_shows s WHERE s.id=? AND s.revision=? AND s.scene_count=? AND s.deleted_at IS NULL AND (SELECT COUNT(*) FROM live_show_scenes sc JOIN toys_center_products p ON p.id=sc.product_id WHERE sc.show_id=s.id AND ${LIVE_SCENE_SNAPSHOT_MATCH})=?`).bind(versionId,showId,versionNumber,payload.expected_revision,LIVE_PACKAGE_SCHEMA_VERSION,key,requestHash,pendingKey,built.packageSha256,built.manifestSha256,built.bytes.byteLength,pendingAssets.length,auth.user.id,createdAt,showId,payload.expected_revision,pendingAssets.length,pendingAssets.length),...pendingAssets.map(asset=>ctx.env.DB.prepare('INSERT INTO live_show_version_assets(id,show_id,version_id,scene_position,product_id_snapshot,source_image_id,source_position,object_key,mime_type,file_size,sha256,etag,created_at) SELECT ?,?,?,?,?,?,?,?,?,?,?,?,? WHERE EXISTS(SELECT 1 FROM live_shows WHERE id=? AND deleted_at IS NULL)').bind(asset.id,showId,versionId,asset.scenePosition,asset.scene.product_id,asset.source.id,asset.source.position,asset.object_key,asset.mime_type,asset.file_size,asset.sha256,asset.etag,createdAt,showId))],results=await ctx.env.DB.batch(statements);
