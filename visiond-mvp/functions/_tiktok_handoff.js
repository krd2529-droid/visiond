...
export async function redeemHandoff(ctx){
 ...
 const row=await ctx.env.DB.prepare(`SELECT f.* FROM tiktok_oauth_handoffs f WHERE f.id=? AND f.ticket_hash=? AND f.slot_id=? AND f.profile_kind=? AND f.status='issued' AND f.expires_at>CURRENT_TIMESTAMP AND ${handoffLiveSql}`).bind(body.id,await sha256(body.ticket),body.slot_id,body.profile_kind||'slot').first();if(!row)return fail(409);
...
export async function prepareHandoffContinuation(env,auth,channelId){
 const flow=auth.handoff;if(!flow||flow.provider!=='tiktok'||flow.continuation!=='shop')return null;
 const state='h1'+random(),nonce=random(),config=tikTokShopOAuthConfig(env);
 if(!config.configured)throw new Error('SHOP_NOT_CONFIGURED');
 const statements=[env.DB.prepare("UPDATE tiktok_oauth_handoffs SET provider='shop',continuation='',status='redeemed',state_hash=?,nonce_hash=?,channel_id=? WHERE id=? AND provider='tiktok' AND continuation='shop' AND status='complete'").bind(await sha256(state),await sha256(nonce),channelId,flow.id),
 env.DB.prepare(`INSERT INTO tiktok_oauth_handoff_guard(id) VALUES((SELECT f.id FROM tiktok_oauth_handoffs f WHERE f.id=? AND f.provider='shop' AND f.status='redeemed' AND f.expires_at>CURRENT_TIMESTAMP AND ${handoffLiveSql}))`).bind(flow.id),
 env.DB.prepare('DELETE FROM tiktok_oauth_handoff_guard WHERE id=?').bind(flow.id)];
 const response=new Response(null,{status:302,headers:{...privateHeaders,location:tikTokShopCreatorAuthorizeUrl(config,state),'set-cookie':`${nonceName(flow.id)}=${nonce}; Path=/; Secure; HttpOnly; SameSite=Lax; Max-Age=600`}});
