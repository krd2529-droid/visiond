...
export async function ensurePartnerCatalogSchema(env){
  ...
  pending=(async()=>{
    await env.DB.exec(`CREATE TABLE IF NOT EXISTS partner_product_changes (id INTEGER PRIMARY KEY AUTOINCREMENT,product_id INTEGER NOT NULL,action TEXT NOT NULL CHECK(action IN ('upsert','delete')),changed_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);CREATE INDEX IF NOT EXISTS idx_partner_product_changes_cursor ON partner_product_changes(id);CREATE INDEX IF NOT EXISTS idx_partner_product_changes_product ON partner_product_changes(product_id);`);
