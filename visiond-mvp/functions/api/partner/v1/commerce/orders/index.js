...
export async function onRequestPost(ctx){
  ...
  const normalized=[],seen=new Set();for(let index=0;index<items.length;index++){const productId=Number(items[index]?.product_id),quantity=Number(items[index]?.quantity);if(!Number.isSafeInteger(productId)||productId<1||!Number.isInteger(quantity)||quantity<1||quantity>100||seen.has(productId))return json({error:'COMMERCE_ORDER_ITEM_INVALID',line:index,request_id:auth.requestId},400,headers);seen.add(productId);normalized.push({productId,quantity})}
  ...
  const placeholders=normalized.map(()=>'?').join(','),products=(await ctx.env.DB.prepare(`SELECT id,title,price FROM products WHERE id IN (${placeholders}) AND status='published' AND deleted_at IS NULL AND COALESCE(product_kind,'product') IN ('product','vision7-key') AND category<>'resale-rights'`).bind(...normalized.map(item=>item.productId)).all()).results||[];
