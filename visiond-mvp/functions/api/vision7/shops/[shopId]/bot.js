...
async function snapshot(env,shop){
  const channels=(await env.DB.prepare('SELECT platform,status,external_id,updated_at FROM veasy_channels WHERE shop_id=? ORDER BY platform').bind(shop.id).all()).results||[],state=await env.DB.prepare("SELECT state,handoff_platform,started_at,stopped_at,last_error,updated_at FROM veasy_bot_state WHERE shop_id=?").bind(shop.id).first(),line=await env.DB.prepare("SELECT e.status,e.adapter_state,c.verified_at,c.external_account_id FROM veasy_webhook_endpoints e LEFT JOIN veasy_channel_credentials c ON c.shop_id=e.shop_id AND c.provider='line' WHERE e.shop_id=? AND e.provider='line' AND e.status!='revoked'").bind(shop.id).first(),ai=veasyAiProviderStatus(env),lineReady=Boolean(line&&line.status==='active'&&line.adapter_state==='ready'&&line.verified_at);
  if(lineReady&&!channels.some(x=>x.platform==='line'&&x.status==='connected')){await env.DB.prepare("INSERT INTO veasy_channels(shop_id,platform,status,external_id) VALUES(?,'line','connected',?) ON CONFLICT(shop_id,platform) DO UPDATE SET status='connected',external_id=excluded.external_id,updated_at=CURRENT_TIMESTAMP").bind(shop.id,String(line.external_account_id||'')).run();const old=channels.find(x=>x.platform==='line');if(old)Object.assign(old,{status:'connected',external_id:String(line.external_account_id||''),updated_at:new Date().toISOString()});else channels.push({platform:'line',status:'connected',external_id:String(line.external_account_id||''),updated_at:new Date().toISOString()})}
  const connected=channels.filter(x=>x.status==='connected'),blockers=[];if(shop.status!=='active')blockers.push('SHOP_INACTIVE');if(!connected.length)blockers.push('NO_CONNECTED_CHANNEL');if(!ai.configured)blockers.push('AI_NOT_CONFIGURED');if(connected.some(x=>x.platform==='line')&&!lineReady)blockers.push('LINE_WEBHOOK_NOT_READY');
  return {shop_id:shop.id,ready:blockers.length===0,blockers,channels,ai,line:line||null,state:state||{state:'stopped',handoff_platform:'',started_at:null,stopped_at:null,last_error:'',updated_at:null}};
}
