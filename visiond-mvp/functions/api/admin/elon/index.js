...
export async function onRequestGet(ctx){
  ...
  try{
    const stats=await ctx.env.DB.prepare(`SELECT COUNT(*) total,SUM(CASE WHEN status='active' THEN 1 ELSE 0 END) active,
      ...
      FROM elon_conversations c WHERE datetime(COALESCE((SELECT MAX(last_activity.created_at) FROM elon_messages last_activity WHERE last_activity.conversation_id=c.id),c.created_at))>=datetime('now','-60 days')`).first();
    ...
    const binds=[],where=["datetime(COALESCE((SELECT MAX(last_activity.created_at) FROM elon_messages last_activity WHERE last_activity.conversation_id=c.id),c.created_at))>=datetime('now','-60 days')"];
    ...
    const sql=`SELECT c.id,c.title,c.status,c.created_at,COALESCE((SELECT MAX(activity.created_at) FROM elon_messages activity WHERE activity.conversation_id=c.id),c.created_at) updated_at,u.name member_name,u.username,u.email,
      ...
      FROM elon_conversations c JOIN users u ON u.id=c.user_id ${where.length?'WHERE '+where.join(' AND '):''} ORDER BY COALESCE((SELECT MAX(last_activity.created_at) FROM elon_messages last_activity WHERE last_activity.conversation_id=c.id),c.created_at) DESC LIMIT ? OFFSET ?`;
