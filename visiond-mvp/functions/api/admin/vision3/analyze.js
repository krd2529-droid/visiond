...
const fallbackType=name=>/(เขียน|คัด|ลากเส้น|เติม|ระบาย|trace|writing|worksheet|alphabet|handwriting|phonics|math|number)/i.test(name)?'worksheet':'game';
...
export async function onRequestPost(ctx){
  ...
  let kind=fallbackType(fileName),specific=cleanSpecific(fileName.replace(/\.(zip|pdf)$/i,'')),aiUsed=false,reason='จำแนกสำรองจากชื่อไฟล์';
  ...
  const category=kind==='worksheet'?'worksheet':'development-game',prefix=kind==='worksheet'?'แบบฝึกหัด':'เกม',base=`${prefix}${specific}`;
