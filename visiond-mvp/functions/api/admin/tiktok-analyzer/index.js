...
export async function onRequestPost(ctx){
  ...
  for(const entry of classified){const item=entry.item,productName=text(item?.name,240);if(!productName)continue;const evidence=text(item?.evidence||(Array.isArray(item?.reasons)?item.reasons.join(' · '):'')||(Array.isArray(item?.risks)?item.risks.join(' · '):''),2000);statements.push(ctx.env.DB.prepare(`INSERT INTO tiktok_channel_products(id,channel_id,name,product_url,product_type,source_kind,score,evidence,source_run_id) VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(channel_id,name) DO UPDATE SET product_url=CASE WHEN excluded.product_url<>'' THEN excluded.product_url ELSE tiktok_channel_products.product_url END,product_type=excluded.product_type,source_kind=excluded.source_kind,score=excluded.score,evidence=excluded.evidence,source_run_id=excluded.source_run_id,last_seen_at=CURRENT_TIMESTAMP`).bind(crypto.randomUUID(),channelId,productName,safeUrl(item?.product_url),productType(item?.product_type,entry.fallback),entry.kind,Math.max(0,Math.min(100,Number(entry.score)||0)),evidence,runId))}
