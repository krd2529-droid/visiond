...
export async function onRequestGet(ctx){
  ...
  if(channelId){const channel=await ctx.env.DB.prepare('SELECT * FROM tiktok_channels WHERE id=?').bind(channelId).first();if(!channel)return json({error:'ไม่พบช่อง'},404,headers);const runs=(await ctx.env.DB.prepare("SELECT id,title,date_range,strategy,status,result_json,provider,model,created_at FROM tiktok_analysis_runs WHERE channel_id=? ORDER BY created_at DESC").bind(channelId).all()).results||[],products=(await ctx.env.DB.prepare('SELECT id,name,product_url,score,evidence,source_run_id,first_seen_at,last_seen_at FROM tiktok_channel_products WHERE channel_id=? ORDER BY score DESC,last_seen_at DESC').bind(channelId).all()).results||[];return json({channel,products,runs:runs.map(x=>({...x,result:parse(x.result_json)}))},200,headers)}
...
export async function onRequestPost(ctx){
  ...
  for(const winner of Array.isArray(result.winner_products)?result.winner_products.slice(0,50):[]){const productName=text(winner?.name,240);if(!productName)continue;statements.push(ctx.env.DB.prepare(`INSERT INTO tiktok_channel_products(id,channel_id,name,product_url,score,evidence,source_run_id) VALUES(?,?,?,?,?,?,?) ON CONFLICT(channel_id,name) DO UPDATE SET product_url=CASE WHEN excluded.product_url<>'' THEN excluded.product_url ELSE tiktok_channel_products.product_url END,score=MAX(tiktok_channel_products.score,excluded.score),evidence=excluded.evidence,source_run_id=excluded.source_run_id,last_seen_at=CURRENT_TIMESTAMP`).bind(crypto.randomUUID(),channelId,productName,safeUrl(winner?.product_url),Math.max(0,Math.min(100,Number(winner?.score)||0)),text(winner?.evidence,2000),runId))}
