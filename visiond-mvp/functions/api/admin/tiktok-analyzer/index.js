...
export async function onRequestGet(ctx){
  ...
  if(channelId){const channel=await ctx.env.DB.prepare('SELECT * FROM tiktok_channels WHERE id=?').bind(channelId).first();if(!channel)return json({error:'ไม่พบช่อง'},404,headers);const runs=(await ctx.env.DB.prepare("SELECT id,title,date_range,strategy,status,result_json,provider,model,created_at FROM tiktok_analysis_runs WHERE channel_id=? ORDER BY created_at DESC").bind(channelId).all()).results||[],products=(await ctx.env.DB.prepare("SELECT id,name,product_url,product_type,source_kind,score,evidence,source_run_id,first_seen_at,last_seen_at FROM tiktok_channel_products WHERE channel_id=? ORDER BY instr('ABCDEF',product_type),score DESC,last_seen_at DESC").bind(channelId).all()).results||[];return json({channel,products,runs:runs.map(x=>({...x,result:parse(x.result_json)}))},200,headers)}
...
export async function onRequestPost(ctx){
  ...
  const lookbackDays=[7,14,30,60,90].includes(Number(form.get('lookback_days')))?Number(form.get('lookback_days')):30;
  let result;try{result=await analyzeTikTok(provider,{channel:{name:channelName,channel_url:channelUrl||channel?.channel_url||''},notes,candidates:text(form.get('candidate_products'),3000),strategy:text(form.get('strategy'),100),dateRange:text(form.get('date_range'),100),lookbackDays,images})}catch(error){return json({error:'วิเคราะห์ภาพไม่สำเร็จ กรุณาลองใหม่',code:String(error?.message||error)},502,headers)}
  ...
  const classified=[...(Array.isArray(result.winner_products)?result.winner_products:[]).map(item=>({item,fallback:'A',kind:'winner',score:item?.score})),...(Array.isArray(result.next_product_candidates)?result.next_product_candidates:[]).map(item=>({item,fallback:'B',kind:'candidate',score:item?.fit_score})),...(Array.isArray(result.avoid_products)?result.avoid_products:[]).map(item=>({item,fallback:'F',kind:'avoid',score:item?.score}))].slice(0,100);
  for(const entry of classified){const item=entry.item,productName=text(item?.name,240);if(!productName)continue;const evidence=text(item?.evidence||(Array.isArray(item?.reasons)?item.reasons.join(' · '):'')||(Array.isArray(item?.risks)?item.risks.join(' · '):''),2000),hasScore=entry.score!==null&&entry.score!==undefined&&entry.score!==''&&Number.isFinite(Number(entry.score)),score=Math.max(0,Math.min(100,Number(entry.score)||0)),type=hasScore&&score<40?'F':productType(item?.product_type,entry.fallback);statements.push(ctx.env.DB.prepare(`INSERT INTO tiktok_channel_products(id,channel_id,name,product_url,product_type,source_kind,score,evidence,source_run_id) VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(channel_id,name) DO UPDATE SET product_url=CASE WHEN excluded.product_url<>'' THEN excluded.product_url ELSE tiktok_channel_products.product_url END,product_type=excluded.product_type,source_kind=excluded.source_kind,score=excluded.score,evidence=excluded.evidence,source_run_id=excluded.source_run_id,last_seen_at=CURRENT_TIMESTAMP`).bind(crypto.randomUUID(),channelId,productName,safeUrl(item?.product_url),type,entry.kind,score,evidence,runId))}
