...
export async function onRequestGet(ctx){
  ...
  const {results}=await ctx.env.DB.prepare(`SELECT u.id,u.email,u.username,u.name,u.phone,u.role,u.created_at,EXISTS(SELECT 1 FROM entitlements e JOIN products p ON p.id=e.product_id LEFT JOIN courses c ON c.product_id=p.id WHERE e.user_id=u.id AND e.active=1 AND (p.category='resale-rights' OR c.course_type='resale_rights')) is_course_owner FROM users u ORDER BY CASE u.role WHEN 'boss' THEN 1 WHEN 'admin' THEN 2 ELSE 3 END,u.id DESC`).all();
