...
export async function onRequestPost(ctx){
  ...
  const statements=[];for(const product of newProducts){statements.push(ctx.env.DB.prepare('INSERT INTO order_items(order_id,product_id,price) VALUES(?,?,?)').bind(order.id,product.id,product.price));statements.push(ctx.env.DB.prepare('INSERT INTO entitlements(user_id,product_id,order_id,active) VALUES(?,?,?,1)').bind(user.id,product.id,order.id))}await ctx.env.DB.batch(statements);
