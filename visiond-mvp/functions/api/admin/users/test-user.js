...
export async function onRequestPost(ctx){
  ...
  const hash=await hashPassword(password),row=await ctx.env.DB.prepare("INSERT INTO users(email,username,name,phone,password_hash,role,is_test_user,vision5_test_account) VALUES(?,?,?,?,?,'user',1,0) RETURNING id,email,username,name,phone,role,is_test_user").bind(email,username,TEST_USER_NAME,phone,hash).first();
