...
const salesPrompt=products=>`คุณคือผู้ช่วยฝ่ายขายภายในของ VisionD วิเคราะห์บทสนทนาเพื่อช่วยเจ้าหน้าที่ ห้ามส่งข้อความเอง
...
export async function onRequestPost(ctx){
  ...
  const provider=selectElonProvider(ctx.env);if(!provider)return json({error:'ยังไม่ได้ตั้งค่า AI Provider สำหรับ ELON'},409,headers);
  if(!await enforceElonGlobalBudget(ctx.env))return json({error:'โควตา AI วันนี้เต็มแล้ว'},429,headers);
  ...
  try{
    const result=await requestElonProvider(provider,{systemPrompt:salesPrompt(catalog),history,message:'วิเคราะห์ประวัติข้างต้นและสร้างร่างข้อความขายที่เหมาะสม'});
    const insight=parseSalesInsight(extractProviderText(provider.name,result.payload));
    if(!insight)return json({error:'AI ตอบรูปแบบไม่ถูกต้อง กรุณาลองใหม่'},502,headers);
    ...
    return json({insight:{...insight,source_message_count:messages.length}},200,headers);
  }catch(error){console.error('V12_SALES_ASSISTANT_FAILED',{code:clean(error?.message,80)});return json({error:'วิเคราะห์บทสนทนาไม่สำเร็จ',code:clean(error?.message,80)},502,headers)}
}
