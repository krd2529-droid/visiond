...
export const buildFallbackInsight=(messages=[])=>{const last=[...messages].reverse().find(item=>item.role!=='assistant')?.content||messages.at(-1)?.content||'';return {stage:'exploring',intent:clean(last,300)||'ยังระบุความสนใจไม่ได้',summary:last?`ลูกค้าส่งข้อความล่าสุดว่า “${clean(last,360)}”`:'ยังมีข้อมูลไม่พอ',objections:'ยังไม่พบข้อกังวลที่ยืนยันได้',next_action:'สอบถามประเภทงาน ช่วงวัย หรือรูปแบบสินค้าที่ลูกค้าต้องการก่อนเสนอสินค้า',suggested_reply:'ขอบคุณที่สนใจค่ะ เพื่อแนะนำสินค้าให้ตรงที่สุด ขอทราบว่าต้องการงานประเภทไหน และใช้สำหรับช่วงวัยใดคะ'}};
...
export async function onRequestPost(ctx){
  ...
  const provider=selectElonProvider(ctx.env),budgetAvailable=provider?await enforceElonGlobalBudget(ctx.env):false;
  ...
  try{
    ...
    return json({insight:{...insight,source_message_count:messages.length},analysis_source:analysisSource,warning},200,headers);
