...
export async function onRequestPost(ctx){
  ...
  if((order.course_owner_user_id||order.seller_course_id)&&!testSeller)return json({error:'ออเดอร์คอร์สผู้ขายต้องให้เจ้าของคอร์สตรวจใน Vision 5 เท่านั้น'},403);
  ...
  if((order.has_resale_rights||testSeller)&&(order.slip_verification_status!=='manual'||b.confirmed!==true))return json({error:'Boss ต้องเปิดดูสลิปและยืนยันการอนุมัติรายการแมนนวลก่อน'},409);
  if(order.status!=='pending_review'||!order.slip_key)return json({error:'ลูกค้ายังไม่ได้ส่งสลิป จึงยังอนุมัติไม่ได้'},409);
  const count=await grantOrder(ctx.env,order,{...a.user,note:b.note||'',method:testSeller?'boss_test_seller_slip_approval':order.has_resale_rights?'boss_rights_slip_approval':'slip_approval'});
  if(!count)return json({error:'ออเดอร์นี้ถูกดำเนินการแล้ว จึงไม่เพิ่มเครดิตซ้ำหรือปลดล็อกซ้ำ'},409);
  return json({ok:true,count,message:testSeller?'Boss อนุมัติบัญชีทดสอบและปลดล็อกคอร์สแล้ว':order.has_resale_rights?`Boss อนุมัติแล้ว เพิ่ม ${count} เครดิตเรียบร้อย`:'อนุมัติและปลดล็อกเรียบร้อย'});
}
