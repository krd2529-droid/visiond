...
export async function onRequestPost(ctx){
  ...
  const body=await ctx.request.json().catch(()=>({})),request=String(body.request||'').trim().slice(0,2000),maxItems=Math.min(30,Math.max(1,Number(body.max_items)||30));
  ...
  const response=await fetch(`https://generativelanguage.googleapis.com/v1/models/${MODEL}:generateContent`,{method:'POST',headers:{'content-type':'application/json','x-goog-api-key':apiKey},body:JSON.stringify({contents:[{parts:[{text:instruction}]}],generationConfig:{temperature:.7,maxOutputTokens:2000}})});
