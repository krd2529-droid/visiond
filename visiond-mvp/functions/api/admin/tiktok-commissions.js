...
export async function onRequestGet(ctx){
  const delegate=await denyActiveVxWorkspaceDelegate(ctx);if(delegate.error)return delegate.error;await ensureDatabase(ctx.env);await ensureTikTokAnalyzerSchema(ctx.env);const auth=await requireVxUser(ctx);if(auth.error)return auth.error;
