...
export async function onRequestPut(ctx) {
  ...
  if (!/^product-\d+-[0-9a-f-]+\.(pdf|zip)$/i.test(key) || !uploadId || !Number.isInteger(partNumber) || partNumber < 1 || partNumber > 10000 || !ctx.request.body) return json({ error: "ข้อมูลชิ้นส่วนไม่ถูกต้อง" }, 400);
