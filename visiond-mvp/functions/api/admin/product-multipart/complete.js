...
export async function onRequestPost(ctx) {
  ...
  if (!/^(?:product-\d+|vision4-pending)-[0-9a-f-]+\.(pdf|zip)$/i.test(key) || (pending?!key.startsWith('vision4-pending-'):!key.startsWith(`product-${productId}-`)) || !uploadId || !parts.length || size < 1 || size > 1024 * 1024 * 1024) return json({ error: "ข้อมูลการอัปโหลดไม่ถูกต้อง" }, 400);
