...
export async function onRequestPost(ctx){
  ...
  if(action==='mark_test_account'||action==='unmark_test_account'){const value=action==='mark_test_account'?1:0;const changed=await ctx.env.DB.prepare('UPDATE users SET vision5_test_account=? WHERE id=?').bind(value,id).run();return changed.meta.changes?json({ok:true,message:value?'ตั้งเป็นบัญชีทดสอบ Vision 5 แล้ว · สลิปต้องให้ Boss อนุมัติ':'ยกเลิกสถานะบัญชีทดสอบแล้ว'}):json({error:'ไม่พบยูสเซอร์'},404)}
