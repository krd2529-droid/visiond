...
export async function onRequestPost(ctx) {
  ...
  for (const p of Array.isArray(b.plans) ? b.plans : []) {
    const pc = validPlanCode(p.plan_code);
    if (!pc) continue;
    await ctx.env.DB.prepare(
      ...
      .bind(
        id,
        Number(p.product_id) || null,
        pc,
        text(p.name, 80) || pc,
        Math.max(0, Math.round(Number(p.price) || 0)),
        pc === "lifetime" ? null : pc === "monthly" ? 30 : 365,
        p.active === false ? 0 : 1,
      )
      .run();
  }
