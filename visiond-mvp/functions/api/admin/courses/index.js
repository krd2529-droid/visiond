...
export async function onRequestPost(ctx){
  ...
  const course=await ctx.env.DB.prepare('INSERT INTO courses(product_id,subtitle,teacher_name,active,course_type,license_edit_days) VALUES(?,?,?,?,?,?)').bind(product.meta.last_row_id,String(form.get('subtitle')||''),String(form.get('teacher_name')||''),form.get('active')==='1'?1:0,courseType,licenseDays).run();
