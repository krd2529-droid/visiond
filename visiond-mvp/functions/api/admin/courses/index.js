...
export async function onRequestPost(ctx){
  ...
  if(!draftOnly&&!teacher)return json({error:'กรุณาใส่ชื่อผู้สอน'},400);if(!draftOnly&&(!Number.isInteger(rawEpisodes)||rawEpisodes<1||rawEpisodes>200))return json({error:'จำนวนบทเรียนต้องอยู่ระหว่าง 1–200 EP'},400);if(!['boss_krungsri','visiond'].includes(paymentMode))return json({error:'บัญชีรับเงินไม่ถูกต้อง'},400);
