...
export async function onRequestPost(ctx){
  await ensureDatabase(ctx.env);const auth=await requireAdmin(ctx);if(auth.error)return auth.error;
  const form=await ctx.request.formData(),title=String(form.get('title')||'').trim(),price=Math.round(Number(form.get('price_baht'))*100);
  ...
  const product=await ctx.env.DB.prepare(`INSERT INTO products(slug,title,short_description,description,price,cover_url,preview_urls,category,file_type,pages,status,source,product_kind)
    VALUES(?,?,?,?,?,?,?,?,'วิดีโอ + PDF',0,?,'course-admin','course')`).bind(slug,title,String(form.get('short_description')||''),String(form.get('description')||''),price,cover,JSON.stringify([cover]),'online-course',form.get('active')==='1'?'published':'draft').run();
  const course=await ctx.env.DB.prepare('INSERT INTO courses(product_id,subtitle,teacher_name,active) VALUES(?,?,?,?)').bind(product.meta.last_row_id,String(form.get('subtitle')||''),String(form.get('teacher_name')||''),form.get('active')==='1'?1:0).run();
