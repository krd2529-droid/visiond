...
export async function onRequestPost(ctx){
  ...
  const product=await ctx.env.DB.prepare(`INSERT INTO products(slug,title,short_description,description,price,cover_url,preview_urls,category,file_type,pages,status,source,product_kind)
    VALUES(?,?,?,?,?,?,?,?,?,0,'published','course-admin',?)`).bind(slug,title,String(form.get('short_description')||''),String(form.get('description')||''),price,cover,JSON.stringify([cover]),courseType==='resale_rights'?'resale-rights':'online-course',courseType==='resale_rights'?'สิทธิ์ใช้งาน':'วิดีโอ + เอกสาร',courseType==='resale_rights'?'product':'course').run();
  const course=await ctx.env.DB.prepare("INSERT INTO courses(product_id,subtitle,teacher_name,active,course_type,license_edit_days,course_origin,review_status,approved_at,approved_by) VALUES(?,?,?,1,?,?,'company','approved',CURRENT_TIMESTAMP,?)").bind(product.meta.last_row_id,String(form.get('subtitle')||''),String(form.get('teacher_name')||''),courseType,licenseDays,auth.user.id).run();
  return json({ok:true,id:course.meta.last_row_id,slug,status:'published',review_status:'approved'},201);
}
