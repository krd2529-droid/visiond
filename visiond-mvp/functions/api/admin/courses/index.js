...
export async function onRequestPost(ctx){
  await ensureDatabase(ctx.env);const auth=await requireAdmin(ctx);if(auth.error)return auth.error;
  const form=await ctx.request.formData(),draftOnly=String(form.get('draft_only')||'')==='1',title=String(form.get('title')||'').trim(),teacher=String(form.get('teacher_name')||'').trim(),rawPrice=Number(form.get('price_baht')),price=Number.isFinite(rawPrice)?Math.max(0,Math.round(rawPrice*100)):0,courseType=form.get('course_type')==='resale_rights'?'resale_rights':'online_course',licenseDays=30,rawEpisodes=Number(form.get('expected_episodes')),episodes=Number.isInteger(rawEpisodes)&&rawEpisodes>=1&&rawEpisodes<=200?rawEpisodes:1,rawMinutes=Number(form.get('content_minutes')),contentMinutes=Number.isFinite(rawMinutes)&&rawMinutes>=1&&rawMinutes<=100000?rawMinutes:0,paymentMode=String(form.get('payment_account')||'boss_krungsri');
  if(!draftOnly&&!title)return json({error:'กรุณาใส่ชื่อคอร์ส'},400);if(!draftOnly&&(!Number.isFinite(rawPrice)||rawPrice<0))return json({error:'ราคาไม่ถูกต้อง'},400);
  if(!draftOnly&&!teacher)return json({error:'กรุณาใส่ชื่อผู้สอน'},400);if(!draftOnly&&(!Number.isInteger(rawEpisodes)||rawEpisodes<1||rawEpisodes>200))return json({error:'จำนวนบทเรียนต้องอยู่ระหว่าง 1–200 EP'},400);if(!draftOnly&&(!Number.isFinite(rawMinutes)||rawMinutes<1||rawMinutes>100000))return json({error:'เวลาเนื้อหาต้องมากกว่า 0 นาที'},400);if(!['boss_krungsri','visiond'].includes(paymentMode))return json({error:'บัญชีรับเงินไม่ถูกต้อง'},400);
  ...
  let slug=slugify(form.get('slug')||title||`course-draft-${Date.now()}`),n=1;while(await ctx.env.DB.prepare('SELECT id FROM products WHERE slug=?').bind(slug).first())slug=`${slugify(form.get('slug')||title||'course-draft')}-${++n}`;
