...
export async function onRequestGet(ctx){await ensureDatabase(ctx.env);const auth=await requireAdmin(ctx);if(auth.error)return auth.error;const course=await companyCourse(ctx);if(!course)return json({error:'ไม่พบคอร์สบริษัท หรือคอร์สนี้ต้องจัดการผ่าน Vision 5'},403);const {results}=await ctx.env.DB.prepare(`SELECT id,title,description,sort_order,duration_seconds,pdf_mime,document_name,CASE WHEN video_key IS NULL THEN 0 ELSE 1 END has_video,CASE WHEN pdf_key IS NULL THEN 0 ELSE 1 END has_pdf FROM course_lessons WHERE course_id=? ORDER BY sort_order,id`).bind(course.id).all();return json({items:results});}
export async function onRequestPost(ctx){
  ...
  const form=await ctx.request.formData(),title=String(form.get('title')||'').trim(),rawDuration=Number(form.get('duration_minutes')),durationSeconds=Number.isFinite(rawDuration)&&rawDuration>=0&&rawDuration<=100000?Math.round(rawDuration*60):0;if(!title)return json({error:'กรุณาใส่ชื่อบทเรียน'},400);if(!Number.isFinite(rawDuration)||rawDuration<0||rawDuration>100000)return json({error:'ความยาว EP ไม่ถูกต้อง'},400);
  const video=form.get('video'),pdf=form.get('pdf'),draftOnly=String(form.get('draft_only')||'')==='1';if(!video?.size&&!pdf?.size&&!draftOnly)return json({error:'แนบคลิป เอกสาร PDF, JPEG หรือ PNG อย่างน้อยหนึ่งไฟล์'},400);
  ...
  try{
    const result=await ctx.env.DB.prepare(`INSERT INTO course_lessons(course_id,title,description,sort_order,video_key,pdf_key,video_mime,pdf_mime,document_name,duration_seconds) VALUES(?,?,?,?,?,?,?,?,?,?)`).bind(course.id,title,String(form.get('description')||''),Number(max.n)+10,videoKey,pdfKey,video?.type||null,pdfKey?pdf.type:null,pdfKey?String(pdf.name||'document').slice(0,240):'',durationSeconds).run();
