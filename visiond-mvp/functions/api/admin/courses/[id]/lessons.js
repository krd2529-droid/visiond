...
import { ensureCourseLessonSchema, MAX_COURSE_VIDEO_BYTES } from '../../../../_course_rules.js';
...
export async function onRequestGet(ctx){await ensureDatabase(ctx.env);const auth=await requireAdmin(ctx);if(auth.error)return auth.error;await ensureCourseLessonSchema(ctx.env);const course=await companyCourse(ctx);if(!course)return json({error:'ไม่พบคอร์สบริษัท หรือคอร์สนี้ต้องจัดการผ่าน Vision 5'},403);const {results}=await ctx.env.DB.prepare(`SELECT id,title,description,episode_label,sort_order,duration_seconds,video_mime,pdf_mime,document_name,CASE WHEN video_key IS NULL THEN 0 ELSE 1 END has_video,CASE WHEN pdf_key IS NULL THEN 0 ELSE 1 END has_pdf FROM course_lessons WHERE course_id=? ORDER BY sort_order,id`).bind(course.id).all();return json({items:results});}
export async function onRequestPost(ctx){
  await ensureDatabase(ctx.env);const auth=await requireAdmin(ctx);if(auth.error)return auth.error;await ensureCourseLessonSchema(ctx.env);const exists=await ctx.env.DB.prepare('SELECT id FROM courses WHERE id=?').bind(ctx.params.id).first();if(!exists)return json({error:'ไม่พบคอร์ส'},404);const course=await companyCourse(ctx);if(!course)return json({error:'คอร์ส Vision 5 ต้องจัดการบทเรียนผ่านหน้าของเจ้าของคอร์สเท่านั้น'},403);
