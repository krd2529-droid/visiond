...
export async function onRequestPost(ctx){
  ...
  await ctx.env.DB.batch([
    ctx.env.DB.prepare('UPDATE courses SET subtitle=?,teacher_name=?,active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(String(body.subtitle||''),String(body.teacher_name||''),body.active?1:0,course.id),
