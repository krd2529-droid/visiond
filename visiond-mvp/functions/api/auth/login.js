...
export async function onRequestPost(ctx){
  ...
  const u=isBossLogin
    ?await ctx.env.DB.prepare("SELECT * FROM users WHERE lower(username)=? OR role='boss' ORDER BY CASE WHEN lower(username)=? THEN 0 ELSE 1 END,id LIMIT 1").bind(login,login).first()
