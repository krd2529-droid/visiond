...
export async function onRequestGet(ctx){
  ...
  object=await ctx.env.FILES.get(key);const headers=new Headers();object.writeHttpMetadata(headers);headers.set('content-length',String(head.size));headers.set('accept-ranges','bytes');headers.set('cache-control','private, no-store');headers.set('content-disposition',isVideo?'inline':`${['application/pdf','image/jpeg','image/png'].includes(lesson.pdf_mime)?'inline':'attachment'}; filename="${String(lesson.document_name||(lesson.pdf_mime==='image/jpeg'?'lesson.jpg':lesson.pdf_mime==='image/png'?'lesson.png':'lesson.pdf')).replace(/["\r\n]/g,'')}"`);headers.set('x-content-type-options','nosniff');return new Response(object.body,{status,headers});
}
