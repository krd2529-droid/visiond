...
export async function onRequestGet(ctx){
  ...
  const file=await ctx.env.DB.prepare(`SELECT pf.* FROM product_files pf JOIN products p ON p.id=pf.product_id WHERE pf.id=? AND (EXISTS(SELECT 1 FROM entitlements e WHERE e.product_id=pf.product_id AND e.user_id=? AND e.active=1) OR EXISTS(SELECT 1 FROM product_bundle_items b JOIN entitlements e ON e.product_id=b.bundle_product_id WHERE b.source_product_id=pf.product_id AND e.user_id=? AND e.active=1) OR EXISTS(SELECT 1 FROM category_memberships cm WHERE cm.user_id=? AND cm.category_slug=p.category AND cm.active=1 AND cm.expires_at>CURRENT_TIMESTAMP))`).bind(ctx.params.id,auth.user.id,auth.user.id,auth.user.id).first();
