import {requireVxUser,vxChannelInsert,vxChannelRestore} from '../../_vx_access.js';
import {vxWorkspaceOwnerId,vxWorkspaceAccessStillCurrent,activeVxWorkspaceDelegation} from '../../_vx_workspace.js';
...
import {mirrorTikTokAvatar} from '../../_tiktok_avatar.js';
...
async function channelPlanForProfile(ctx,auth,requestedChannelId,profile,limit){
  const env=ctx.env,ownerId=vxWorkspaceOwnerId(auth),userId=ownerId;
  ...
  if(linked?.channel_id){
    if(!await vxWorkspaceAccessStillCurrent(ctx,auth))throw new Error('VX_ACCESS_EXPIRED');
...
export async function onRequestGet(ctx){
  ...
  const auth=handoff?.auth||await requireVxUser(ctx);if(auth.error)return back('login_required');if(!handoff&&auth.user.role==='user'&&await activeVxWorkspaceDelegation(ctx.env,auth.user.id))return back('access_expired');const ownerId=vxWorkspaceOwnerId(auth);
  ...
  const stateRow=handoff?.stateRow||await consumeTikTokState(ctx.env,state,ownerId);if(!stateRow)return back('invalid_state');
  ...
  try{
    ...
    if(!await vxWorkspaceAccessStillCurrent(ctx,auth))return done('access_expired','',stateRow.channel_id);
    ...
    if(!await vxWorkspaceAccessStillCurrent(ctx,auth))return done('access_expired','',channelId);
    const prepared=await prepareTikTokConnection(ctx.env,ownerId,channelId,token,profile),statements=[];
    const existingBinding=await ctx.env.DB.prepare('SELECT slot_id,user_id,channel_id,provider_open_id FROM tiktok_browser_profile_bindings WHERE channel_id=? OR (user_id=? AND provider_open_id=?) LIMIT 1').bind(channelId,ownerId,prepared.openId).first();
    if(!profileSlotId&&existingBinding&&(Number(existingBinding.user_id)!==Number(ownerId)||existingBinding.channel_id!==channelId||existingBinding.provider_open_id!==prepared.openId))return done('profile_conflict','',channelId);
    if(!profileSlotId&&plan.kind==='existing'&&!await ctx.env.DB.prepare('SELECT id FROM tiktok_channels WHERE id=? AND created_by=? AND archived_at IS NULL').bind(channelId,ownerId).first())return done('channel_unavailable','',channelId);
    if(!await vxWorkspaceAccessStillCurrent(ctx,auth))return done('access_expired','',channelId);
    if(plan.statement)statements.push(plan.statement);
    statements.push(prepared.statement);
    if(profileSlotId)statements.push(tikTokProfileBindingStatement(ctx.env,{slotId:profileSlotId,userId:ownerId,channelId,openId:prepared.openId,profileKind:auth.handoff?.profile_kind||'slot'}));
    ...
    const connection=await ctx.env.DB.prepare("SELECT * FROM tiktok_connections WHERE id=? AND user_id=? AND channel_id=? AND open_id=? AND status='active'").bind(prepared.id,ownerId,channelId,prepared.openId).first();if(!connection)return done('channel_unavailable','',channelId);
    const stillAuthorized=()=>vxWorkspaceAccessStillCurrent(ctx,auth);
    if(handoff){await mirrorTikTokAvatar(ctx.env,connection,profile.avatar_url,fetch,{stillAuthorized});return continuation?.response||done('connected','',channelId)}
    const blocked=await requireD1DataFetchAvailable(ctx,'tiktok_oauth_post_connect_sync');if(blocked)return done('connected','sync_deferred_d1_quota',channelId);await syncTikTokConnection(ctx.env,connection,fetch,{stillAuthorized,profile});return done('connected','',channelId);
  }catch(error){
