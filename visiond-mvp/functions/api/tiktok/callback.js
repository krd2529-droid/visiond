...
export async function onRequestGet(ctx){
  ...
  try{const config=tikTokOAuthConfig(ctx.env);if(!config.configured)return back('not_configured','',channelId);const token=await exchangeTikTokCode(config,code),profile=await fetchTikTokProfile(token.access_token);if(!profile.open_id)return back('profile_failed','',channelId);channelId=await channelForProfile(ctx.env,auth.user.id,channelId,profile,auth.vx.account_limit);if(!channelId)return back('channel_unavailable','',stateRow.channel_id);const connectionId=await saveTikTokConnection(ctx.env,auth.user.id,channelId,token,profile),connection=await ctx.env.DB.prepare('SELECT * FROM tiktok_connections WHERE id=?').bind(connectionId).first();await syncTikTokConnection(ctx.env,connection);return back('connected','',channelId)}catch(error){if(error?.message==='VX_ACCOUNT_LIMIT')return back('account_limit','บัญชีครบตามแพ็กเกจแล้ว กรุณาลบช่องเดิมหรือเลือกแพ็กเกจใหม่ที่ Vtools',channelId);if(error?.code==='TIKTOK_ACCOUNT_ALREADY_LINKED')return back('account_already_linked',error.channelName,channelId);if(error?.code==='TIKTOK_CHANNEL_ALREADY_LINKED')return back('channel_already_linked',error.accountName,channelId);console.error('TIKTOK_OAUTH_CALLBACK_FAILED',{message:String(error?.message||error).slice(0,180)});return back('failed','',channelId)}
}
