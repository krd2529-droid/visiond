...
export async function onRequestGet(ctx){
  ...
  if(!channelId)return json({error:'กรุณาเลือกช่องก่อนเชื่อม TikTok'},400);
  if(!await ctx.env.DB.prepare("SELECT id FROM tiktok_channels WHERE id=? AND created_by=? AND archived_at IS NULL").bind(channelId,auth.user.id).first())return json({error:'ไม่พบช่องที่ต้องการเชื่อม'},404);
