...
export async function onRequestPost(ctx){
  ...
  const order=await ctx.env.DB.prepare("SELECT id,slip_key FROM orders WHERE id=? AND course_owner_user_id=? AND seller_course_id IS NOT NULL AND status='pending_review'").bind(ctx.params.id,auth.user.id).first();
