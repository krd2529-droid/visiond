...
export async function onRequestPost(ctx){
  ...
  const key=qr instanceof File&&qr.size?`seller-payment-qr-${auth.user.id}-${crypto.randomUUID()}.${ext(qr)}`:'';
  if(key)await ctx.env.FILES.put(key,await qr.arrayBuffer(),{httpMetadata:{contentType:qr.type}});
  ...
  if(!result.meta.changes){if(key)await ctx.env.FILES.delete(key);return json({error:'ข้อมูลบัญชีถูกบันทึกไปแล้ว'},409)}
