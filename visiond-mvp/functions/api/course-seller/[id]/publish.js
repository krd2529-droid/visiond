...
export async function onRequestPost(ctx) {
  ...
  try {
    const courseUpdate=publishCredit
      ?ctx.env.DB.prepare("UPDATE courses SET license_entitlement_id=?,edit_expires_at=COALESCE(edit_expires_at,?),license_edit_days=30,contact_info=?,payment_bank_name=?,payment_account_name=?,payment_account_number=?,payment_qr_url=?,review_status='pending',review_note='',submitted_at=CURRENT_TIMESTAMP,active=0,updated_at=CURRENT_TIMESTAMP WHERE id=? AND owner_user_id=? AND course_plan='rights' AND license_entitlement_id IS NULL AND basket_binding_locked=1 AND review_status IN ('draft','changes_requested') AND EXISTS(SELECT 1 FROM course_right_credits credit WHERE credit.id=? AND credit.user_id=? AND credit.active=1 AND credit.used_course_id IS NULL)").bind(
        bindingId,expires,contact,owner.seller_bank_name,owner.seller_account_name,owner.seller_account_number,paymentQrUrl,course.id,auth.user.id,publishCredit.id,auth.user.id)
      :ctx.env.DB.prepare(
