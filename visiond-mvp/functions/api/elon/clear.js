...
const conversationId=value=>/^[a-f0-9-]{20,64}$/i.test(String(value||''))?String(value):'';
...
export async function onRequestPost(ctx){
  ...
  const result=await ctx.env.DB.prepare(`UPDATE elon_conversations SET status='ended',ended_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP
    WHERE id=? AND user_id=? AND status='active'`).bind(id,auth.user.id).run();
