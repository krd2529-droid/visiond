...
export async function onRequestGet(ctx) {
  ...
  const [code, summary, clicks, signups, adjustment, payouts] = await Promise.all([
    ...
    ctx.env.DB.prepare('SELECT id,payout_no,amount,currency,status,created_at,paid_at FROM vx_referral_payouts WHERE referrer_user_id=? ORDER BY created_at DESC LIMIT 50').bind(auth.user.id).all()
  ]);
