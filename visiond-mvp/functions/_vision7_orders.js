...
export async function fulfillVision7Order(env,order,actor={}){
  ...
  const items=(await env.DB.prepare(`SELECT oi.id order_item_id,oi.vision7_renew_license_id,q.id plan_id,q.program_id,q.plan_code,q.duration_days,p.max_devices FROM order_items oi JOIN vision7_plans q ON q.product_id=oi.product_id JOIN vision7_programs p ON p.id=q.program_id WHERE oi.order_id=? ORDER BY oi.id`).bind(order.id).all()).results||[];
  const output=[];
