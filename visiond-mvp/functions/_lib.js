...
export async function currentUser(ctx){const sid=cookie(ctx.request,'vd_session');if(!sid)return null;return await ctx.env.DB.prepare(`SELECT u.id,u.email,u.name,u.role FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.id=? AND s.expires_at>datetime('now')`).bind(sid).first()}
...
export async function requireAdmin(ctx){const x=await requireUser(ctx);if(x.error)return x;if(x.user.role!=='admin')return {error:json({error:'บัญชีนี้ไม่มีสิทธิ์แอดมิน'},403)};return x}
