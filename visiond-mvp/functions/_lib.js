...
export async function currentUser(ctx){const sid=cookie(ctx.request,'vd_session');if(!sid)return null;return await ctx.env.DB.prepare(`SELECT u.id,u.email,u.username,u.name,u.role,u.created_at FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.id=? AND s.expires_at>datetime('now')`).bind(sid).first()}
