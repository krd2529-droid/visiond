...
export async function grantOrder(env, order, actor = {}) {
  ...
  const items = (await env.DB.prepare("SELECT oi.id order_item_id,oi.product_id,COALESCE(oi.product_title,p.title,'สินค้าเดิม') title,p.product_kind,p.category,p.member_category,p.member_duration_months FROM order_items oi LEFT JOIN products p ON p.id=oi.product_id WHERE oi.order_id=? ORDER BY oi.id").bind(order.id).all()).results||[];
