...
export async function grantOrder(env, order, actor = {}) {
  ...
  const items = (await env.DB.prepare('SELECT p.id product_id,p.title,p.product_kind,p.member_category,p.member_duration_months FROM order_items oi JOIN products p ON p.id=oi.product_id WHERE oi.order_id=?').bind(order.id).all()).results;
