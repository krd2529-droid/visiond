export async function grantOrder(env, order, actor = {}) {
  ...
  const items = (await env.DB.prepare('SELECT p.id product_id,p.title FROM order_items oi JOIN products p ON p.id=oi.product_id WHERE oi.order_id=?').bind(order.id).all()).results;
  ...
  for (const item of items) {
    statements.push(env.DB.prepare('INSERT OR IGNORE INTO entitlements(user_id,product_id,order_id) VALUES(?,?,?)').bind(order.user_id,item.product_id,order.id));
