...
- Cache/version: HTML/CSS/module entry และ static ESM dependency ทุก edge ของ editor/opener ใช้ release query key เดียวกัน (`?v=020134`) รวม parser, AI host, player, presenter, photo-avatar และ shared Thai speech ตลอด direct/nested graph เพื่อไม่ให้ returning browser ผสม entry ใหม่กับ dependency รุ่นเก่าที่ cache ไว้
- รหัส UI: `main[data-feature="LIVE-CENTER-001"]` และเมนูหลังบ้านใช้ `data-feature="LIVE-CENTER-001"`; ปุ่มใช้ canonical `.vds-btn`
- การทดสอบ: `scripts/test-v020134-live-photo-foundation.mjs` ครอบ service/config/privacy/idempotency/cache/exact-audio contracts, `scripts/test-v020134-live-audience-foundation.mjs` ครอบ capacity/grounding/redaction/retention/EXPLAIN และ real Chrome desktop/390 ที่ `scripts/test-v020134-live-photo-foundation-browser.mjs` ครอบ consent, blackhole timeout+stable retry identity, labelled simulator, offline zero-network, mutual exclusion และ disconnected actual-video OBS; `scripts/test-v020134.mjs` เป็น release umbrella และ `npm run test:v020134` ต่อ complete connected chain v0.20.133 → v0.20.132 → v0.20.131 → v0.20.130 → v0.20.129 → v0.20.128 → v0.20.123 → v0.20.122 โดย compatibility อนุญาตเพียง exact same-origin GET ของ Facebook config และยังห้าม connector mutation/external platform host
