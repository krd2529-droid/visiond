...
- OBS local surface: ปุ่มโหมด OBS เปิดพื้นผิวสะอาดเต็มจอ/เต็มหน้าต่างที่มีเฉพาะรูป ชื่อสินค้า ราคากับ stock snapshot จากแพ็กเกจ และตำแหน่งฉาก; ไม่แสดงบทพูดหรือตัวควบคุม ไม่อ้างว่าเชื่อม/เริ่ม livestream และออกอย่างปลอดภัยเมื่อกด Escape, fullscreen เปลี่ยน, เปิดไฟล์ใหม่ หรือ fullscreen promise เก่าจบช้า; Cut แสดงทันทีและ Fade มี visual transition จริง
...
- หน้า/ไฟล์: `public/live-center.{html,css,js}`, `public/live-center-package.js`, `public/live-package-player.js`, `public/live-package-open.{html,js}`, `functions/_live_center.js`
- รหัส UI: `main[data-feature="LIVE-CENTER-001"]` และเมนูหลังบ้านใช้ `data-feature="LIVE-CENTER-001"`; ปุ่มใช้ canonical `.vds-btn`
- การทดสอบ: `scripts/test-v020123.mjs` เรียก focused route/parser/store/SQLite/R2 suite และ real-Chrome desktop/390 ใน `scripts/test-v020123-live-center.mjs`; AI script ใช้ `scripts/test-v020125-live-center-ai.mjs` ครอบ auth/provider/atomic throttle/grounding/strict Gemini response/transport fallback/duration eligibility/race, exact Production ids 4/3 + representative shape, structural canonicalization, การตัด opening สำเร็จรูป และ Chrome desktop/390; local player ใช้ real three-scene schema-v1 fixture ใน `scripts/test-v020129-live-player.mjs` กับ injected clock ตรวจ deadline/throttle/final/replay/TTS/manual sync และ `scripts/test-v020129-live-player-browser.mjs` ตรวจ Chrome desktop/390, Cut/Fade, no-voice/fullscreen fallback, clean OBS, object-URL/speech/timer cleanup และ deferred fullscreen request/exit + reopen race; `npm run test:v020129` ต่อ release chain v0.20.128 → v0.20.123 → v0.20.122 และยืนยัน parser/vSport bytes ที่ต้องไม่เปลี่ยน
