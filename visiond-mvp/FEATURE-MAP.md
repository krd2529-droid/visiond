...
- หน้าใช้งาน: `/guides.html` และเมนู `คู่มือการใช้งาน` ระหว่าง Vtools กับบทความ
- คู่มือพร้อมใช้: VX สำหรับลูกค้า และ V-Learning สำหรับพาร์ตเนอร์ 50/50 กับลูกค้าซื้อคอร์ส
- คู่มือที่วางลำดับถัดไป: การสมัครสมาชิก และการซื้อสินค้าดิจิทัล โดยแสดงสถานะกำลังจัดทำและยังไม่สร้างปุ่มดาวน์โหลด
- รองรับเดสก์ท็อปและมือถือผ่าน `guides.css`; ไฟล์คู่มือให้บริการจาก `public/manuals/`
...
- Data/API: `admin_work_notes`, `admin_work_note_attachments`; CRUD ที่ `functions/api/admin/work-notes/index.js`; อัปโหลดรูปที่ `functions/api/admin/work-notes/attachments.js`; อ่าน/ลบรูปที่ `functions/api/admin/work-notes/[attachmentId].js`; AI เติมเนื้อหาก่อนทำสไลด์ที่ `functions/api/admin/work-notes/expand.js`; AI outline ที่ `functions/api/admin/work-notes/presentation.js`; ทุก endpoint ใช้ `requireAdmin`
...
- Show: บันทึก metadata, ลำดับสินค้า, snapshot ชื่อ/ราคา/สกุลเงิน/สต็อก และ baseline ของ normalized cover (row/key/position/MIME/size/ETag), บทและ cue ในตาราง normalized; save ใช้ revision แบบ compare-and-set ป้องกัน stale overwrite และปฏิเสธฟิลด์ลับ/ฟิลด์นอก allowlist การ์ดรายการที่ใช้ร่วมกันระหว่าง Boss/Admin มีปุ่มเปิดและปุ่ม `ลบ` แยกกัน พร้อม confirm ที่ระบุชื่อรายการตรงตัว; การลบเป็น tombstone แบบ idempotent `(created_by,id)` ไม่ใช่ purge เพราะ version/package เป็น audit ที่ immutable โดยซ่อน tombstone จาก list/detail/update/version/package/presenter/audience ทั้งหมด ลบ/redact เฉพาะ mutable scene/test state หยุด runtime ทำ provider เป็น `delete_pending` และล้างเฉพาะ presenter object key ที่บันทึกไว้ผ่าน durable cleanup job หลัง commit ขณะที่ version/package rows และ package/copied-asset binaries ยังคงอยู่
...
- AI Script: แต่ละฉากมีปุ่ม `AI คิดบทพูด` ที่เรียก provider ฝั่งเซิร์ฟเวอร์เฉพาะเมื่อ Boss/Admin กด โดยอ่านสินค้า Toys Center ปัจจุบันด้วย primary-key lookup เดียว ใช้ facts ชื่อ/รายละเอียด/แบรนด์/ไลน์/ซีรีส์/ราคา/stock และเป้าหมาย 5–180 วินาที; throttle ต่อผู้ใช้เป็น atomic indexed UPSERT 30 ครั้ง/15 นาที Live Center ขอ Gemini structured JSON ด้วย schema ที่ enumerate ID ปัจจุบันและปิด thinking เฉพาะ text Flash 2.5 ที่รองรับ จากนั้น server รับเฉพาะ JSON สอง key ซึ่งต่างได้เพียง RFC whitespace ตรวจ finish reason บังคับให้ provider เลือกทั้งชื่อและราคา+stock และรับเฉพาะ ID ปัจจุบันที่ไม่ซ้ำ; server ไม่โฆษณา opening สำเร็จรูปและ render `fact.name` เป็นชื่อสินค้าปัจจุบันตรงตัวที่ต้นบท ก่อน canonicalize optional facts ตามลำดับที่เลือก → current price/stock → closing จึงไม่ปฏิเสธแผนที่ schema อนุญาตเพียงเพราะ provider วาง structural ID ต่างตำแหน่ง ก่อน render ประโยคสมบูรณ์ที่ผูกกับ facts (ราคา+stock เป็น segment เดียว และรายละเอียดเป็น exact quoted excerpt) พร้อมตรวจ secret/claim/number/ระยะเวลา ก่อนใส่ textarea แบบยังไม่บันทึก; stale result ไม่ข้ามฉากหรือรายการ และ non-JSON 5xx แสดงข้อความ retry ภาษาไทยโดยไม่ล้างบทเดิม
- AI พิธีกรสด: หลังเปิดแพ็กเกจที่ตรวจแล้ว Boss/Admin ต้องกดเริ่มโหมดออนไลน์เอง จึงเรียก `POST /api/admin/live-center/host-turn` ด้วยเฉพาะ product id, บริบทบทสดล่าสุดไม่เกิน 4 ช่วง และคิวสั้นที่ไม่บังคับ; route ตรวจสิทธิ์และ atomic per-user limit 120 ครั้ง/15 นาที ก่อน primary-key lookup สินค้า sellable ปัจจุบันหนึ่งครั้ง แล้วให้ provider เลือก structured safe segment plan ที่ต้องมีชื่อ ราคา/สกุลเงิน/stock จริง พร้อม lead/closing หลายแบบและปฏิเสธบทซ้ำตรงตัว บริบทกับคิวเป็น untrusted selection hint ไม่ใช่ facts และ response `private, no-store` คืนเฉพาะบทที่ render แล้วกับ authoritative product snapshot โดยไม่คืน key/prompt/raw response/usage และไม่แก้ show/version/package/catalog/order/platform state
- Local player: `/live-package-open.html` ตรวจ canonical schema v1, exact key tree, offsets, one-to-one scene/assets, MIME magic และ digest ก่อนสร้าง local in-memory playback; ไม่ autoplay และหลังโหลดไฟล์แล้วไม่เรียก D1/R2/API/แพลตฟอร์มต่อฉาก ผู้ใช้กดเริ่ม/หยุด/เริ่มใหม่/ก่อนหน้า/ถัดไปได้ มี monotonic countdown และ auto-advance ตาม duration, ยกเลิกเสียงเก่าเมื่อเปลี่ยนฉาก และใช้ Web Speech ในอุปกรณ์ผ่าน shared cancellable voice discovery ที่รอรายการเสียง async, เลือก `th-TH` ก่อน `th-*`, ไม่ใช้เสียง default/ภาษาอื่น และแสดงคำแนะนำภาษาไทยให้ติดตั้งหรือเปิดเสียงไทยแล้วลองใหม่เมื่อหาไม่พบ
- AI host runtime: state machine แยกจาก local player และไม่อ่าน saved script; explicit Start ทำ ready → generating → speaking แล้วมี active request สูงสุดหนึ่งกับ prefetched turn สูงสุดหนึ่ง บทสดขึ้น caption ก่อน shared Thai-only voice discovery และ Web Speech; presenter กับ prefetch เริ่มเฉพาะ `utterance.onstart` ของเสียงไทยจริงที่ผ่าน speech token/epoch/product guard (ระหว่างค้นหา/เข้าคิวยังคง thinking) ส่วนเครื่องที่ไม่มีเสียงไทยหยุดแบบ fail-closed พร้อมข้อความติดตั้ง/เปิดเสียงและปุ่ม retry; duplicate/late `voiceschanged/onstart`, `onend` ก่อนเริ่ม, Pause/Stop/Skip/reopen/auth loss/unload เพิ่ม epoch, abort request, ถอด listener/timeout, ทิ้ง prefetch และ cancel speech เพื่อกัน callback เก่า ขณะพูด `onend` ใช้ prefetch ครั้งเดียวหรือรอ request เดิม; Resume สร้างบทใหม่ สลับสินค้าขณะทำงานต่อได้ และสินค้าสุดท้ายพูดต่อโดยไม่วนจนผู้ใช้เปิด loop-products เอง ทุก failure หยุดโดยไม่มี timer retry หรือ fallback เป็น saved script/default voice
- OBS local surface + AI human presenter: โหมด local คงพื้นผิวสะอาดเดิมที่มีเฉพาะรูป ชื่อ ราคา/stock snapshot และตำแหน่งฉากโดยไม่แสดงบท/controls; เมื่อ AI mode ทำงานจึงเพิ่ม VisionD-branded half-body SVG presenter แบบ code-native/network-free ที่มีหน้า ปาก ลำตัว แขนสองข้างและ state จำกัด `idle/thinking/talk/present/open/cheer/paused/error` พร้อม live caption และ authoritative current product ทั้งใน operator preview กับ OBS เดียวกัน `visiond-default` แสดงตัวเต็มและ `presenter-placeholder` แสดงมนุษย์แบบย่อ; compatibility resolver เฉพาะ online AI map ค่า package เก่า `none` เป็น `visiond-default` ตั้งแต่ preview ก่อน Start โดยไม่แก้ metadata/package bytes หรือ semantics ของ presenter core; reduced-motion คงท่านิ่งโดยไม่เดิน gesture timer และ pause/stop/error/reopen/offline teardown ป้องกัน stale animation โดยยังไม่มี cue/editor controls/secrets/saved script ปุ่ม fullscreen รองรับ fallback เต็มหน้าต่าง, Escape และ reopen race; Cut แสดงทันทีและ Fade มี visual transitionจริง
- Photo Avatar foundation: Boss/Admin อัปโหลดได้เฉพาะภาพ JPEG/PNG ที่ browser ลดขนาดและส่งพร้อมการยืนยันสิทธิ์/ความยินยอม จากนั้น Pages ส่ง bytes แบบ bounded ไปยัง private `PORTRAIT_SANITIZER` Service binding; Worker ที่ไม่มี public route ใช้ Cloudflare Images decode/scale/re-encode แล้ว Pages ตรวจ progressive JPEG, ขนาด, dimensions, metadata/trailing bytes ซ้ำก่อนเก็บ private R2 ตาม owner/show และผูก portrait active ได้หนึ่งรายการต่อ show ทุก upload/delete ใช้ stable idempotency claim, rate limit, indexed retention และ retryable object cleanup; binding หายต้อง fail closed โดยไม่เขียน claim/rate/R2
- Real presenter/runtime boundary: โหมด Photo Avatar ใช้เฉพาะ `<video>` stream ที่มี audio track หนึ่งและ video track หนึ่งซึ่งยัง live; analyser ของเสียงจริงเท่านั้นที่เปลี่ยน speaking state และ stop/reopen/track-ended ยกเลิก stream/callback เก่า ทั้ง preview และ clean OBS แสดง `ยังไม่ได้เชื่อมต่อ` เมื่อไม่มี session และห้ามแทนด้วย SVG/static portrait ส่วน legacy presenter ยังใช้ได้เมื่อเลือกอย่างชัดเจน D-ID/Azure/Facebook health เป็น private/no-store และไม่มี provider secret ใน client/package; D-ID Agent/WebRTC provisioning ยัง fail closed จน authenticated schema/credentials ถูกพิสูจน์ จึงไม่อ้าง photoreal external E2E สำเร็จ
- Audience foundation: local simulator แยกและติดป้ายว่าเป็นข้อมูลทดสอบ เปิดใช้ใน Production อย่างชัดเจนด้วย `LIVE_CENTER_LOCAL_TEST_ENABLED="1"` สำหรับ Boss/Admin เท่านั้น; queue รับได้สูงสุด 24 ต่อ session, dedupe/rate/priority แบบ atomic, product id ต้องอยู่ใน `live_show_scenes`, claim/complete/stop/expiry เป็น indexed bounded batches และ redact viewer hash/label/question/answer/claim token ก่อน terminal/retention การตอบใช้ authoritative current show product facts และตอบข้อจำกัดภาษาไทยเมื่อไม่ทราบ; Facebook endpoint รอบนี้อ่านเฉพาะ configuration health และไม่เริ่ม/หยุด/เขียนแพลตฟอร์ม
- Data/API: ตารางเดิม `live_shows`, `live_show_scenes`, `live_show_versions`, `live_show_version_assets` จาก migration `0112`; migration `0113_live_photo_avatar_audience.sql` เพิ่ม private portraits/bindings/upload claims/object cleanup/provider resources/runtime sessions/audience events พร้อม owner/show/status/time indexes; migration `0114_live_show_tombstones.sql` เพิ่ม tombstone/idempotency fields, partial active keyset index, indexed owner+id lookup และ trigger ห้าม hard-delete/restore; API ใต้ `/api/admin/live-center/**` รวม private non-mutating `host-turn`, portrait/session/health/audience/local-test/Facebook-config routes และ `DELETE /api/admin/live-center/shows/:id` โดยทุก package/admin response เป็น `private, no-store`
- หน้า/ไฟล์: `public/live-center.{html,css,js}`, `public/live-portrait-image.js`, `public/live-audience-queue.js`, `public/live-photo-avatar.js`, `public/live-center-package.js`, `public/live-package-player.js`, `public/live-package-ai-host.js`, `public/live-package-thai-speech.js`, `public/live-package-presenter.js`, `public/live-package-open.{html,js}`, `functions/_live_{center,portrait_foundation,audience_foundation,avatar_provider,azure_speech,center_crypto}.js`, private routes ใต้ `functions/api/admin/live-center/**` และ private Worker `workers/live-portrait-sanitizer/**`
- Production binding/deploy boundary: Pages `wrangler.toml` ประกาศ `PORTRAIT_SANITIZER` เฉพาะ `[env.production]` และมีผลเมื่อ deploy production branch `main` (หรือระบุ `--branch main`) หลัง deploy Worker `visiond-live-portrait-sanitizer` ก่อน; local/default ไม่มี binding และต้อง fail closed โดยเจตนา ห้ามใช้ Pages `env.IMAGES`, public sanitizer URL หรือ pure-JS JPEG fallback ที่เกิน Workers Free CPU budget
- Cache/version: HTML/CSS/module entry และ static ESM dependency ทุก edge ของ editor/opener ใช้ release query key เดียวกัน (`?v=020135`) รวม parser, AI host, player, presenter, photo-avatar และ shared Thai speech ตลอด direct/nested graph เพื่อไม่ให้ returning browser ผสม entry ใหม่กับ dependency รุ่นเก่าที่ cache ไว้; delete invalidate เฉพาะ show/list/version tags ที่เกี่ยวข้องและ ticket/epoch กัน response เก่าคืน tombstone เข้า editor
- รหัส UI: `main[data-feature="LIVE-CENTER-001"]` และเมนูหลังบ้านใช้ `data-feature="LIVE-CENTER-001"`; ปุ่มใช้ canonical `.vds-btn`
- การทดสอบ: ชุด v0.20.134 เดิมยังครอบ service/config/privacy/idempotency/cache/exact-audio, audience capacity/grounding/redaction/retention/EXPLAIN และ real Chrome consent/local simulator/offline/provider boundary; `scripts/test-v020135-live-show-delete.mjs` เพิ่ม auth-before-id/SQL/R2, strict body/private headers, idempotent/concurrent/ambiguous tombstone, immutable audit retention, exact-key cleanup, delete-vs-final-upload CAS, keyset delete-between-pages และ partial-index EXPLAIN ส่วน `scripts/test-v020135-live-show-delete-browser.mjs` ครอบ DOM sibling/accessible name, exact confirm cancel/failure/per-card busy/success, stale-open suppression และ desktop/390; `scripts/test-v020135.mjs` เป็น release umbrella และ `npm run test:v020135` ต่อ complete connected chain ถึง v0.20.122
...
- Commission share card: สร้าง PNG 1080x1350 ธีม VX จากยอดจริงในแดชบอร์ด แบ่งหน้าละ 10 ช่องแบบสองคอลัมน์และเลขช่องต่อเนื่อง โดยทุกภาพแสดงยอดรวมทุกช่องและช่วงวันที่เริ่มต้น–สิ้นสุดเดียวกัน พร้อมแชร์ไฟล์ทั้งชุดหรือดาวน์โหลด; ไม่มีปุ่มและไม่สร้างภาพเมื่อไม่มีค่าคอม
- Commission image library: บันทึกชุด PNG แยกตามเจ้าของ ช่วงวันที่แบบตรงตัว สกุลเงิน ประเภทยอด และ fingerprint ข้อมูล; เรียกช่วงเดิมที่ข้อมูลเดิมจะใช้รูปเดิม ส่วนวันเดียวกับช่วงหลายวันเป็นคนละชุด และไฟล์ทุกไฟล์ตรวจ ownership ก่อนอ่าน
- Commission social workflow: ฟอร์มกำหนดชื่อและวันเดียว/ช่วงวันที่จะโหลดค่าคอมของช่วงนั้นก่อนสร้างรูป, บันทึกคลังสำเร็จแล้วจึงแสดงปุ่มแชร์, รองรับ caption/hashtags และใช้ VX referral link ของผู้ใช้จากระบบ `/api/vx/referrals` เดิมพร้อมปุ่มคัดลอก
- Commission image preview: หลังสร้างหรือดึงชุดเดิมจากคลัง หน้าค่าคอมแสดงภาพ PNG ที่ใส่ยอดจริงครบทุกหน้าก่อนปุ่มแชร์ โดยใช้ไฟล์ชุดเดียวกับที่ส่งไป Web Share
- Commission share privacy: รูปค่าคอมที่แชร์แสดงเฉพาะ `ช่องที่ N` และยอด ไม่วาดชื่อช่องจริงและไม่ใส่ชื่อช่องใน cache fingerprint; ตารางส่วนตัวภายในแดชบอร์ดยังคงชื่อช่องเพื่อการจัดการ
- VX referral payout cycle: ค่าคอมคำนวณจากยอดออเดอร์เต็มตาม rate ของผู้แนะนำ; รอบจ่ายวันที่ 1 ใช้เฉพาะออเดอร์ที่ยืนยันชำระก่อนต้นเดือนตามเวลาไทย ยอดตั้งแต่วันที่ 1 เข้ารอบถัดไป และ backend บังคับ cutoff ซ้ำตอนสร้าง payout
- VX commission payout statement: ทุกรอบจ่ายมีเอกสาร VisionD ที่ผู้รับเปิดดูและพิมพ์/PDF ได้ แสดงรายการขาย ยอดขายเต็ม เปอร์เซ็นต์ค่าคอม ยอดค่าคอมต่อรายการ และยอดรวมค่าคอมทั้งหมด โดย API ตรวจ ownership และปฏิเสธเอกสารที่ผลรวมไม่ตรง payout ledger
