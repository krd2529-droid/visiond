...
- การทดสอบ: `scripts/test-v020123.mjs` เรียก focused route/parser/store/SQLite/R2 suite และ real-Chrome desktop/390 ใน `scripts/test-v020123-live-center.mjs`; AI script ใช้ `scripts/test-v020125-live-center-ai.mjs` ครอบ auth/provider/atomic throttle/grounding/strict Gemini response/transport fallback/duration eligibility/race, exact product 2 + representative shape, structural canonicalization และ Chrome desktop/390; `scripts/test-v020127.mjs` เรียก suite นี้และยืนยัน SHA ของ vSport ที่ไม่ถูกแก้ ก่อน `npm run test:v020127` ต่อ regressions v0.20.123/v0.20.122 โดยตั้งใจไม่แก้ v0.20.124 focused bytes
