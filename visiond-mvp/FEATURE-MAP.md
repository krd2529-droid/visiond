...
- Digital commerce boundary: v0.14.407 ล็อก contract สำหรับ product detail, order create/status, payment state และ one-time entitlement claim; v0.14.408 เปิดเฉพาะ product detail runtime แล้ว ส่วน `/commerce/*` ยังไม่เปิด; `/orders/sync` เป็น reconciliation เท่านั้นและห้ามสร้างออเดอร์/อนุมัติชำระ/ออกสิทธิ์; VisionD คำนวณราคาและอนุมัติเอง, API 1 บาทไม่หักลูกค้า, Partner API ห้ามคืนไฟล์หรือ permanent download URL
