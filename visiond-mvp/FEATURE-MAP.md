...
- AI host runtime: state machine แยกจาก local player และไม่อ่าน saved script; explicit Start ทำ ready → generating → speaking แล้วมี active request สูงสุดหนึ่งกับ prefetched turn สูงสุดหนึ่ง ขณะพูด `onend` ใช้ prefetch ครั้งเดียวหรือรอ request เดิม Pause/Stop/Skip/reopen/auth loss/unload เพิ่ม epoch, abort request, ทิ้ง prefetch และ cancel speech เพื่อกัน response/callback เก่า; Resume สร้างบทใหม่ สลับสินค้าขณะทำงานต่อได้ และสินค้าสุดท้ายพูดต่อโดยไม่วนจนผู้ใช้เปิด loop-products เอง ทุก failure หยุดพร้อมปุ่ม retry โดยไม่มี timer retry หรือ fallback เป็น saved script
- OBS local surface + AI overlay: โหมด local คงพื้นผิวสะอาดเดิมที่มีเฉพาะรูป ชื่อ ราคา/stock snapshot และตำแหน่งฉากโดยไม่แสดงบท/controls; เมื่อ AI mode ทำงานจึงเพิ่มป้าย VisionD AI Host, idle/thinking/speaking/error visual, live caption และ authoritative current product โดยยังไม่มี cue/editor controls/secrets/saved script ปุ่ม fullscreen รองรับ fallback เต็มหน้าต่าง, Escape และ reopen race; Cut แสดงทันทีและ Fade มี visual transition จริง
