...
- Data/API: `live_shows`, `live_show_scenes`, `live_show_versions`, `live_show_version_assets`; migration `0112_visiond_live_center.sql`; API ใต้ `/api/admin/live-center/**` รวม private non-mutating `host-turn`; package/admin responses เป็น `private, no-store`
- หน้า/ไฟล์: `public/live-center.{html,css,js}`, `public/live-center-package.js`, `public/live-package-player.js`, `public/live-package-ai-host.js`, `public/live-package-thai-speech.js`, `public/live-package-presenter.js`, `public/live-package-open.{html,js}`, `functions/_live_center.js`, `functions/api/admin/live-center/host-turn.js`
- Cache/version: HTML module entry และ static ESM dependency ทุก edge ของ opener ใช้ release query key เดียวกัน (`?v=020133`) รวม parser, AI host, player, presenter และ shared Thai speech ตลอด direct/nested graph เพื่อไม่ให้ returning browser ผสม entry ใหม่กับ dependency รุ่นเก่าที่ cache ไว้
- รหัส UI: `main[data-feature="LIVE-CENTER-001"]` และเมนูหลังบ้านใช้ `data-feature="LIVE-CENTER-001"`; ปุ่มใช้ canonical `.vds-btn`
- การทดสอบ: `scripts/test-v020123.mjs` เรียก focused route/parser/store/SQLite/R2 suite และ real-Chrome desktop/390 ใน `scripts/test-v020123-live-center.mjs`; AI script ใช้ `scripts/test-v020125-live-center-ai.mjs` ครอบ auth/provider/atomic throttle/grounding/strict response; local player ใช้ `scripts/test-v020129-live-player*.mjs`; real-time host ใช้ `scripts/test-v020130-live-ai-host.mjs` ตรวจ auth/origin/input/context/secrets, 120/121 atomic race, one-PK lookup, grounded variation/repeat/provider/privacy/no-mutation และ deterministic abort/prefetch/TTS/pause/resume/skip/reopen/auth failure รวม queued-no-start, duplicate/late onstart และ error/end-before-start พร้อม `scripts/test-v020130-live-ai-host-browser.mjs` ตรวจ Chrome desktop/390, offline zero-network, caption-before-speech, mutual exclusion, clean AI OBS, failure/retry, Cut/Fade และ valid→valid/invalid reopen; human presenter ใช้ `scripts/test-v020131-human-presenter.mjs` กับ real-Chrome `scripts/test-v020131-human-presenter-browser.mjs` ตรวจ anatomy/eight states/presets/genuine speech boundary/stale teardown/reduced-motion/desktop/390/zero-extra-network; legacy-avatar/Thai-only voice ใช้ `scripts/test-v020132-legacy-thai-voice.mjs` และ real-Chrome `scripts/test-v020132-legacy-thai-voice-browser.mjs` ตรวจ exact `th-TH`, async discovery/timeout/teardown, English-only fail-closed, onstart-only prefetch, package `none` compatibility, clean OBS, desktop/390 และ zero-extra-network; returning-cache ใช้ installed-Chrome `scripts/test-v020133-live-esm-cache.mjs` seed exact v0.20.131 presenter ที่ unversioned URL แล้วพิสูจน์ว่า v0.20.133 graph โหลด dependency cache key ใหม่และเปิดแพ็กเกจได้; `npm run test:v020133` ต่อ release chain v0.20.132 → v0.20.131 → v0.20.130 → v0.20.129 → v0.20.128 → v0.20.123 → v0.20.122 และยืนยัน schema/parser/server/vSport boundaries
