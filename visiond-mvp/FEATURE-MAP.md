...
- Local player: `/live-package-open.html` ตรวจ canonical schema v1, exact key tree, offsets, one-to-one scene/assets, MIME magic และ digest ก่อนสร้าง local in-memory playback; ไม่ autoplay และหลังโหลดไฟล์แล้วไม่เรียก D1/R2/API/แพลตฟอร์มต่อฉาก ผู้ใช้กดเริ่ม/หยุด/เริ่มใหม่/ก่อนหน้า/ถัดไปได้ มี monotonic countdown และ auto-advance ตาม duration, ยกเลิกเสียงเก่าเมื่อเปลี่ยนฉาก และใช้ Web Speech ในอุปกรณ์โดยเลือกเสียงไทยก่อนพร้อม fallback แบบไม่มีเสียง
- AI host runtime: state machine แยกจาก local player และไม่อ่าน saved script; explicit Start ทำ ready → generating → speaking แล้วมี active request สูงสุดหนึ่งกับ prefetched turn สูงสุดหนึ่ง บทสดขึ้น caption ก่อนส่ง Web Speech แต่ presenter และ prefetch เริ่มเฉพาะ `utterance.onstart` ที่ผ่าน speech token/epoch/product guard จริงเท่านั้น (เสียงที่แค่เข้าคิวจึงยังคง thinking); duplicate/late `onstart`, `onend` ก่อนเริ่ม, Pause/Stop/Skip/reopen/auth loss/unload เพิ่ม epoch, abort request, ทิ้ง prefetch และ cancel speech เพื่อกัน callback เก่า ขณะพูด `onend` ใช้ prefetch ครั้งเดียวหรือรอ request เดิม; Resume สร้างบทใหม่ สลับสินค้าขณะทำงานต่อได้ และสินค้าสุดท้ายพูดต่อโดยไม่วนจนผู้ใช้เปิด loop-products เอง ทุก failure หยุดพร้อมปุ่ม retryโดยไม่มี timer retry หรือ fallback เป็น saved script
- OBS local surface + AI human presenter: โหมด local คงพื้นผิวสะอาดเดิมที่มีเฉพาะรูป ชื่อ ราคา/stock snapshot และตำแหน่งฉากโดยไม่แสดงบท/controls; เมื่อ AI mode ทำงานจึงเพิ่ม VisionD-branded half-body SVG presenter แบบ code-native/network-free ที่มีหน้า ปาก ลำตัว แขนสองข้างและ state จำกัด `idle/thinking/talk/present/open/cheer/paused/error` พร้อม live caption และ authoritative current product ทั้งใน operator preview กับ OBS เดียวกัน `visiond-default` แสดงตัวเต็ม, `presenter-placeholder` แสดงมนุษย์แบบย่อ และ `none` ซ่อนจริงพร้อมยุบ layout เป็นหนึ่งคอลัมน์; reduced-motion คงท่านิ่งโดยไม่เดิน gesture timer และ pause/stop/error/reopen/offline teardown ป้องกัน stale animation โดยยังไม่มี cue/editor controls/secrets/saved script ปุ่ม fullscreen รองรับ fallback เต็มหน้าต่าง, Escape และ reopen race; Cut แสดงทันทีและ Fade มี visual transitionจริง
