...
- AI Script: แต่ละฉากมีปุ่ม `AI คิดบทพูด` ที่เรียก provider ฝั่งเซิร์ฟเวอร์เฉพาะเมื่อ Boss/Admin กด โดยอ่านสินค้า Toys Center ปัจจุบันด้วย primary-key lookup เดียว ใช้ facts ชื่อ/รายละเอียด/แบรนด์/ไลน์/ซีรีส์/ราคา/stock และเป้าหมาย 5–180 วินาที; throttle ต่อผู้ใช้เป็น atomic indexed UPSERT 30 ครั้ง/15 นาที Provider เลือกได้เฉพาะ ID จาก canonical structured plan ส่วน server render ประโยคสมบูรณ์ที่ผูกกับ facts ปัจจุบัน (ราคา+stock เป็น segment เดียว และรายละเอียดเป็น exact quoted excerpt) พร้อมตรวจ secret/claim/number ก่อนใส่ textarea แบบยังไม่บันทึก และ stale result ไม่ข้ามฉากหรือรายการ
...
- การทดสอบ: `scripts/test-v020123.mjs` เรียก focused route/parser/store/SQLite/R2 suite และ real-Chrome desktop/390 ใน `scripts/test-v020123-live-center.mjs`; AI script ใช้ `scripts/test-v020125-live-center-ai.mjs` ครอบ auth/provider/atomic throttle/grounding/race และ Chrome desktop/390; `npm run test:v020125` ต่อ vSport v0.20.124 และ regressions เดิมทั้งหมด
