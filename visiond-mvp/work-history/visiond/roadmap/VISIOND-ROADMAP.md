...
## v0.14.299 — Course Basket Load Recovery (IMPLEMENTED-AWAITING-DEPLOY)
- ตะกร้าคอร์สและ EP ต้องไม่หายหรือค้างจากความผิดพลาดของแดชบอร์ด ยอดขาย หรือสลิป
- หน้าสร้างเปิดฟอร์มทันทีและแจ้งข้อผิดพลาดแบบไม่ทำลายงานที่กำลังกรอก

## v0.14.251 — V12 Sales Playbook + Facebook Caption Knowledge (IMPLEMENTED-AWAITING-DEPLOY)
- ฝึก V12 และ LINE ด้วยลำดับขายแบบถามคัดกรองทีละข้อ เสนอไม่เกินสองทางเลือก และไม่กดดันลูกค้า
- ดึงแคปชั่นล่าสุดจาก Facebook Page ที่เชื่อมและยืนยันแล้วผ่าน Meta API เพื่อใช้น้ำเสียงเท่านั้น แคตตาล็อกปัจจุบันมีสิทธิ์เหนือราคา/โปรเก่าเสมอ
- เคสจ่ายแล้ว คืนเงิน ยกเลิก เข้าไม่ได้ ร้องเรียน หรือกฎหมาย ต้องหยุดบอทเฉพาะบทสนทนาและส่งต่อเจ้าหน้าที่
...
- Backend SDK พร้อมสินค้า ลูกค้า ออเดอร์ Signed Event, Idempotency และ Sensitive Payload Guard โดยไม่มี Credential จริง
...
- ปุ่ม E2E ตรวจสินค้าและวงจรธุรกรรม Sandbox พร้อม Idempotency, Webhook, Retry/Dead และ Alert โดยไม่แตะข้อมูล Production
...
- ลำดับมาตรฐานที่ลูกค้าเห็นทุกหน้า: `1 ซื้อสิทธิ์`, `2 เริ่มขายฟรี`, `3 พาร์ตเนอร์ 50/50` ห้ามเรียงใหม่เอง
- แบบ 1 ใช้เครดิตและผู้สอนได้ 100%; แบบ 2 ฟรีสูงสุด 3 คอร์ส/5 EP; แบบ 3 ไม่จำกัด รับเงินผ่าน VisionD แบ่ง 50/50 และหักค่า API 1 บาทจากผู้สอน

# v0.14.280 — Course Plan Loader Isolation (IMPLEMENTED · BUG)
- หน้า `/course-center` โหลดเฉพาะรูปแบบ 1/2 และจำนวนเครดิตผ่าน API น้ำหนักเบา ห้ามรอรายการคอร์ส ยอดขาย สลิป หรือบัญชีรับเงิน
- การโหลดต้องมีเวลาสิ้นสุดและปุ่มลองใหม่ ห้ามค้างคำว่า `กำลังโหลดรูปแบบคอร์ส…` โดยไม่มีทางดำเนินการต่อ
- หน้าแบบ 1/2 ยังคงใช้ API รายละเอียดเต็มของตัวเองหลังผู้ใช้เลือกแบบแล้วเท่านั้น

# v0.14.281 — Course Type 1 Startup Crash Recovery (IMPLEMENTED · BUG)
- หลักฐานจาก Console production ระบุ `course-seller.js` ล้มก่อนเรียก `load()` เพราะผูก `onclick` กับปุ่ม `sendCourseReview` ที่ถูกถอดออกจาก HTML
- คืนปุ่มส่งตรวจและข้อความกำกับไว้ในขั้นตอน EP ตาม DOM contract เดิม พร้อมตรวจ null ทุกจุดที่อ้างปุ่มนี้
- หน้าแบบ 1 ต้องโหลดเครดิต ข้อมูลรับเงิน และแบบฟอร์มสร้างคอร์สต่อได้ แม้ส่วนควบคุมเสริมใน DOM ถูกเปลี่ยนภายหลัง
# v0.14.282 — ร่างคอร์สแบบ 1 ก่อนมีเครดิตและส่งตรวจจากขั้นที่ 6

- แบบ 1 สร้างและบันทึกร่างได้แม้ยังไม่มีเครดิต โดยไม่ซ่อนหรือลบข้อมูลร่าง
- ร่างที่ยังไม่ผูกเครดิตแก้ไขต่อได้ แต่ส่งตรวจไม่ได้จนกว่าจะมีเครดิต 1 เครดิต
- ปุ่ม `ส่งตรวจ` อยู่ในขั้นที่ 6 `ตรวจความพร้อมและส่งตรวจ` หลังข้อมูลคอร์ส EP บัญชีรับเงิน และวิธีตรวจสลิป
- เมื่อส่งสำเร็จ ระบบตั้งสถานะ `pending` และส่งรายการไปยังเมนู `ตรวจคอร์ส` ในหลังบ้าน
- ก่อนส่งตรวจ ระบบตรวจ External course ownership, เครดิต, EP, บัญชีรับเงิน และวิธีตรวจสลิปฝั่ง API ซ้ำเสมอ
# v0.14.283 — EP เป็นส่วนหนึ่งของตะกร้าคอร์ส

- หน้าสร้างแบบ 1 เรียงข้อมูลตะกร้าคอร์สแล้วต่อด้วยส่วนอัปโหลดงาน EP ในฟอร์มเดียวกัน
- ทุก EP ผูกกับตะกร้าคอร์สที่กำลังสร้าง พร้อมชื่อ คำอธิบาย คลิป และเอกสารประกอบ
- ปุ่มหลักบันทึกตะกร้าคอร์สและ EP เป็นร่างชุดเดียว ก่อนดำเนินขั้นรับเงิน ตรวจสลิป และส่งตรวจ

# v0.14.284 — คืนส่วน EP ที่หายจากหน้าสร้างตะกร้าคอร์ส

- สาเหตุจริงคือฟอร์มไม่มี `expected_episodes` ทำให้สคริปต์สร้างส่วน EP หยุดก่อน render ไม่ใช่เงื่อนไขเครดิต
- หน้าสร้างตะกร้าคอร์สต้องมีจำนวน EP ค่าเริ่มต้น 1 และแสดงช่องชื่อ คำอธิบาย คลิป และเอกสารของ EP เสมอ แม้เครดิตเป็น 0
- เครดิตใช้เป็นเงื่อนไขตอนส่งตรวจเท่านั้น ห้ามใช้ซ่อนส่วนสร้างหรือแก้ไข EP ของร่าง
### v0.14.285 — เปิดแบบฟอร์ม EP ตลอดเวลา

- ลบปุ่ม “ยุบทั้งหมด” และการพับราย EP ออกจากโค้ดจริง ไม่ใช้วิธีซ่อนด้วย CSS
- ชื่อ คำอธิบาย คลิป และเอกสารของทุก EP ต้องมองเห็นและกรอกต่อเนื่องได้ทันที
### v0.14.286 — เพิ่ม EP จากตะกร้าคอร์สโดยตรง

- เพิ่มปุ่ม `+ เพิ่ม EP` ที่เห็นชัด กดแล้วต่อ EP ใหม่ท้ายตะกร้าและโฟกัสช่องชื่อทันที
- จำนวน EP ในฟอร์มต้องอัปเดตตามปุ่มและยังจำกัดสูงสุด 200 EP ต่อตะกร้า
- ห้ามเพิ่มเพดานไฟล์วิดีโอโดยเดา ต้องตรวจข้อจำกัด request ของ Cloudflare และใช้ direct upload เมื่อจะรองรับไฟล์ใหญ่

### v0.14.287 — ศูนย์จัดการคอร์สเหลือพาร์ตเนอร์ 50/50 แบบเดียว

- การสร้างคอร์สใหม่เปิดเพียง `พาร์ตเนอร์ 50/50`: VisionD รับเงิน ตรวจสลิป และแบ่งรายได้ผู้สอน 50% / VisionD 50%
- ปิดเส้นทางซื้อสิทธิ์ 499 และเครดิตสำหรับการสร้างคอร์สใหม่ทั้ง UI และ API; URL เก่า `type=1` และ `type=2` ต้องพาเข้าสู่หน้าพาร์ตเนอร์แบบเดียว
- คอร์สซื้อสิทธิ์ที่สร้างไว้ก่อนแพตยังอ่านและจัดการย้อนหลังได้ พร้อมป้าย `ซื้อสิทธิ์เดิม (ปิดรับใหม่)` เพื่อไม่ทำลายข้อมูลหรือสิทธิ์เดิม
- ห้ามคืนตัวเลือกหลายแบบหรือข้อความเครดิตกลับมาในหน้า Course Center เว้นแต่ Boss สั่งเปลี่ยนนโยบายใหม่โดยตรง
